...
SUMMARY_SCHEMA = "sentigraph_controlled_production_analysis_result_creation_runtime_boundary_summary_v0_1"
SOURCE_CANDIDATE_SET_SCHEMA = (
    "sentigraph_controlled_production_analysis_result_creation_candidate_set_v0_1"
)
SOURCE_CANDIDATE_SCHEMA = "sentigraph_controlled_production_analysis_result_creation_candidate_v0_1"
PHASE = "8W-59"
APPROVAL_PHRASE = (
    "APPROVE_8W_59_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CREATION_RUNTIME_BOUNDARY_HELPER_IMPLEMENTATION"
)
SOURCE_WARN_STATUS = "production_analysis_result_creation_candidate_set_warn_manual_review_required"
WARN_STATUS = "production_analysis_result_creation_runtime_boundary_set_warn_manual_review_required"
EXPECTED_8W58_NEXT_BOUNDARY_OPTION = (
    "ready_for_8W_59_controlled_production_analysis_result_creation_runtime_boundary_helper_implementation_after_explicit_approval"
)
...
SOURCE_TRUE_FIELDS = {
    ...
    "production_analysis_result_creation_candidate_created": (
        "source_production_analysis_result_creation_candidate_created_not_true"
    ),
...
def build_controlled_production_analysis_result_creation_runtime_boundary_set(
    controlled_production_analysis_result_creation_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
    runtime_boundary_count: int = 1,
) -> dict[str, Any]:
    blockers: list[str] = []
    blockers.extend(_approval_blockers(exact_approval_phrase))
    blockers.extend(_requested_action_blockers(requested_actions))
    if runtime_boundary_count != 1:
        blockers.append("creation_runtime_boundary_count_not_one")

    source_candidates: list[dict[str, Any]] = []
    if not blockers:
        source_candidates = _safe_source_candidates(controlled_production_analysis_result_creation_candidate_set)
        blockers.extend(
            _source_candidate_set_blockers(
                controlled_production_analysis_result_creation_candidate_set,
                source_candidates,
            )
        )
    ...
    if not blockers:
        ...
        if not blockers:
            boundaries.append(_runtime_boundary_from_source(source_candidates[0]))

    status = WARN_STATUS if not blockers else _blocked_status(blockers)
    return _base_output(
        source_candidate_set=controlled_production_analysis_result_creation_candidate_set,
        status=status,
...
create_controlled_production_analysis_result_creation_runtime_boundary_set = (
    build_controlled_production_analysis_result_creation_runtime_boundary_set
)


def build_safe_controlled_production_analysis_result_creation_runtime_boundary_summary(
    controlled_production_analysis_result_creation_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
) -> dict[str, Any]:
    boundary_set = build_controlled_production_analysis_result_creation_runtime_boundary_set(
        controlled_production_analysis_result_creation_candidate_set,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_creation_runtime_boundary_set_schema": boundary_set[
            "production_analysis_result_creation_runtime_boundary_set_schema"
        ],
        "production_analysis_result_creation_runtime_boundary_set_status": boundary_set[
            "production_analysis_result_creation_runtime_boundary_set_status"
        ],
        "input_source_kind": boundary_set["input_source_kind"],
        "source_production_analysis_result_creation_candidate_set_schema": boundary_set[
            "source_production_analysis_result_creation_candidate_set_schema"
        ],
        "source_production_analysis_result_creation_candidate_schema": boundary_set[
            "source_production_analysis_result_creation_candidate_schema"
        ],
        ...
        "creation_runtime_boundary_mode": boundary_set["creation_runtime_boundary_mode"],
        "production_analysis_result_creation_runtime_boundary_count": boundary_set[
            "production_analysis_result_creation_runtime_boundary_count"
        ],
        ...
        "production_analysis_result_creation_candidate_created_upstream": boundary_set[
            "production_analysis_result_creation_candidate_created_upstream"
        ],
        "production_analysis_result_creation_runtime_boundary_created": boundary_set[
...
def _base_output(
    *,
    ...
    return {
        "production_analysis_result_creation_runtime_boundary_set_schema": RUNTIME_BOUNDARY_SET_SCHEMA,
        "phase": PHASE,
        "production_analysis_result_creation_runtime_boundary_set_status": status,
        "input_source_kind": "controlled_production_analysis_result_creation_candidate",
        "source_production_analysis_result_creation_candidate_set_schema": _safe_source_value(
            source_candidate_set,
            "production_analysis_result_creation_candidate_set_schema",
        )
        or SOURCE_CANDIDATE_SET_SCHEMA,
        "source_production_analysis_result_creation_candidate_schema": _source_candidate_schema(
            source_candidate_set
        ),
        ...
        "source_production_analysis_result_creation_or_runtime_execution_candidate_count": (
            _source_creation_or_runtime_candidate_count(source_candidate_set)
        ),
        "source_production_analysis_result_runtime_boundary_count": _source_runtime_boundary_count(
            source_candidate_set
        ),
        "source_production_analysis_result_boundary_count": _source_boundary_count(source_candidate_set),
        "source_production_analysis_result_candidate_count": _source_production_candidate_count(
            source_candidate_set
        ),
        "source_analysis_result_candidate_count": _source_analysis_result_candidate_count(source_candidate_set),
        ...
        "source_production_case_candidate_count": _source_production_case_candidate_count(source_candidate_set),
        "source_controlled_evidence_item_count": _source_controlled_evidence_item_count(source_candidate_set),
        "creation_runtime_boundary_mode": (
            "backend_only_local_production_analysis_result_creation_runtime_boundary"
        ),
        "production_analysis_result_creation_runtime_boundary_count": len(boundaries),
        "warning_count": _source_warning_count(source_candidate_set),
        "human_review_required": _truthy(
            source_candidate_set.get("human_review_required") if isinstance(source_candidate_set, dict) else None
        ),
        "no_automatic_trust_upgrade": _truthy(
            source_candidate_set.get("no_automatic_trust_upgrade")
            if isinstance(source_candidate_set, dict)
            else None
        ),
        "production_analysis_result_creation_candidate_created_upstream": _truthy(
            source_candidate_set.get("production_analysis_result_creation_candidate_created")
            if isinstance(source_candidate_set, dict)
            else None
        ),
        "production_analysis_result_creation_runtime_boundary_created": boundary_created,
        ...
        "production_analysis_result_creation_runtime_boundaries": boundaries,
        "boundary_flags": _boundary_flags(creation_runtime_boundary_only=boundary_created),
        "runtime_side_effects": _runtime_side_effects(),
        "warnings": _boundary_warnings(source_candidate_set),
        ...
        "audit_summary": {
            ...
            "production_record_effect": "none",
            "review_queue_effect": "none",
            "frontend_route_effect": "none",
...
def _runtime_boundary_from_source(source_candidate: dict[str, Any]) -> dict[str, Any]:
    source_ids = _source_candidate_ids(source_candidate)
    return {
        "production_analysis_result_creation_runtime_boundary_schema": RUNTIME_BOUNDARY_SCHEMA,
        "production_analysis_result_creation_runtime_boundary_id": _runtime_boundary_id(source_ids),
        "source_production_analysis_result_creation_candidate_ids": source_ids,
        "source_production_analysis_result_creation_candidate_count": _source_creation_candidate_count(
            source_candidate
        ),
        "source_production_analysis_result_creation_boundary_count": _source_creation_boundary_count(
        ...
        "source_production_analysis_result_creation_or_runtime_execution_candidate_count": (
            _source_creation_or_runtime_candidate_count(source_candidate)
        ),
        "source_production_analysis_result_runtime_boundary_count": _source_runtime_boundary_count(
            source_candidate
        ),
        "source_production_analysis_result_boundary_count": _source_boundary_count(source_candidate),
        "source_production_analysis_result_candidate_count": _source_production_candidate_count(source_candidate),
        "source_analysis_result_candidate_count": _source_analysis_result_candidate_count(source_candidate),
        "source_actual_analysis_execution_candidate_count": _source_actual_analysis_execution_candidate_count(
            source_candidate
        ),
        "source_production_analysis_run_candidate_count": _source_production_analysis_run_candidate_count(
        ...
        "source_production_case_candidate_count": _source_production_case_candidate_count(source_candidate),
        "source_controlled_evidence_item_count": _source_controlled_evidence_item_count(source_candidate),
        ...
        "intended_creation_runtime_boundary_labels": [
            "production_analysis_result_creation_runtime_boundary_only",
            "selected_sample_only",
            "human_review_required",
            "no_automatic_trust_upgrade",
        ],
        "intended_module_scope_labels": [
            "production_analysis_result_creation_runtime_boundary",
        ...
        "creation_runtime_gate_labels": [
            "creation_runtime_boundary_only",
            "manual_review_required",
            "warning_preserving",
            "not_creation_execution",
            "not_runtime_execution",
        ],
        "warning_count": _source_warning_count(source_candidate),
        "human_review_required": True,
        "review_status": "human_review_required",
        "trust_boundary_label": "no_automatic_trust_upgrade",
        "verification_status_summary": "needs_review",
        "redaction_status": "redacted",
        "redaction_warnings": _safe_list(source_candidate.get("redaction_warnings")),
        "warning_labels": _boundary_warnings(source_candidate),
        "blocker_codes": [],
        ...
        "creation_runtime_boundary_only": True,
        ...
        "review_queue_runtime_used": False,
        "report_ready": False,
        "boundary_flags": _boundary_flags(creation_runtime_boundary_only=True),
    }
...
def _source_candidate_set_blockers(
    ...
    if not isinstance(source_candidate_set, dict):
        return ["source_creation_candidate_set_missing"]
    blockers: list[str] = []
    for field, value in source_candidate_set.items():
        ...
        if field in FORBIDDEN_SOURCE_FIELDS and _forbidden_field_has_payload(value):
            blockers.append(f"forbidden_source_creation_candidate_set_field:{field}")
        elif _contains_forbidden_value(value):
            blockers.append(f"source_creation_candidate_set_forbidden_value:{field}")
    if source_candidate_set.get("production_analysis_result_creation_candidate_set_schema") != (
        SOURCE_CANDIDATE_SET_SCHEMA
    ):
        blockers.append("source_creation_candidate_set_schema_wrong")
    if source_candidate_set.get("production_analysis_result_creation_candidate_set_status") != SOURCE_WARN_STATUS:
        blockers.append("source_creation_candidate_set_status_wrong")
    if _source_creation_candidate_count(source_candidate_set) != (
    ...
    if source_candidate_set.get("production_analysis_result_creation_candidate_count") not in (None, 1):
        blockers.append("source_production_analysis_result_creation_candidate_count_not_one")
    if len(source_candidates) != 1:
        blockers.append("source_creation_candidate_list_count_not_one")
    if _source_creation_boundary_count(source_candidate_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_BOUNDARY_COUNT
    ):
        blockers.append("source_creation_boundary_count_not_one")
    if _source_creation_or_runtime_candidate_count(source_candidate_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_OR_RUNTIME_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_creation_or_runtime_candidate_count_not_one")
    if _source_runtime_boundary_count(source_candidate_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_RUNTIME_BOUNDARY_COUNT
    ):
        blockers.append("source_runtime_boundary_count_not_one")
    if _source_boundary_count(source_candidate_set) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_COUNT:
        blockers.append("source_production_analysis_result_boundary_count_not_one")
    if _source_production_candidate_count(source_candidate_set) != (
    ...
    if _source_analysis_result_candidate_count(source_candidate_set) != EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_analysis_result_candidate_count_not_one")
    if _source_actual_analysis_execution_candidate_count(source_candidate_set) != (
    ...
    if _source_production_analysis_run_candidate_count(source_candidate_set) != (
    ...
    if _source_production_case_candidate_count(source_candidate_set) != EXPECTED_SOURCE_PRODUCTION_CASE_CANDIDATE_COUNT:
        blockers.append("source_production_case_candidate_count_not_one")
    if _source_controlled_evidence_item_count(source_candidate_set) != EXPECTED_SOURCE_CONTROLLED_EVIDENCE_ITEM_COUNT:
    ...
    if source_candidate_set.get("8w58_decision") != "ready":
        blockers.append("source_8w58_decision_not_ready")
    if source_candidate_set.get("8w58_selected_next_boundary_option") != EXPECTED_8W58_NEXT_BOUNDARY_OPTION:
        blockers.append("source_8w58_next_boundary_option_wrong")
    ...
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if field in source_candidate_set and source_candidate_set.get(field) is not False:
            blockers.append(reason)
...
def _source_candidate_blockers(source_candidates: list[dict[str, Any]]) -> list[str]:
    if len(source_candidates) != 1:
        return ["source_production_analysis_result_creation_candidate_missing"]
    blockers: list[str] = []
    candidate = source_candidates[0]
    if not isinstance(candidate, dict):
        return ["source_production_analysis_result_creation_candidate_not_object"]
    for field, value in candidate.items():
        if field in FORBIDDEN_SOURCE_FIELDS and _forbidden_field_has_payload(value):
            blockers.append(f"forbidden_source_creation_candidate_field:{field}")
        elif _contains_forbidden_value(value):
            blockers.append(f"source_creation_candidate_forbidden_value:{field}")
    if candidate.get("production_analysis_result_creation_candidate_schema") != SOURCE_CANDIDATE_SCHEMA:
        blockers.append("source_creation_candidate_schema_wrong")
    if _source_creation_candidate_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_CANDIDATE_COUNT:
        blockers.append("source_creation_candidate_source_candidate_count_not_one")
    if _source_creation_boundary_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_BOUNDARY_COUNT:
        blockers.append("source_creation_candidate_creation_boundary_count_not_one")
    if _source_creation_or_runtime_candidate_count(candidate) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_OR_RUNTIME_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_creation_candidate_creation_or_runtime_candidate_count_not_one")
    if _source_runtime_boundary_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_RUNTIME_BOUNDARY_COUNT:
        blockers.append("source_creation_candidate_runtime_boundary_count_not_one")
    if _source_boundary_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_COUNT:
        blockers.append("source_creation_candidate_production_analysis_result_boundary_count_not_one")
    if _source_production_candidate_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_creation_candidate_production_analysis_result_candidate_count_not_one")
    if _source_analysis_result_candidate_count(candidate) != EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_creation_candidate_analysis_result_candidate_count_not_one")
    if _source_actual_analysis_execution_candidate_count(candidate) != (
        EXPECTED_SOURCE_ACTUAL_ANALYSIS_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_creation_candidate_actual_analysis_execution_candidate_count_not_one")
    if _source_production_analysis_run_candidate_count(candidate) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RUN_CANDIDATE_COUNT:
        blockers.append("source_creation_candidate_production_analysis_run_candidate_count_not_one")
    if _source_production_case_candidate_count(candidate) != EXPECTED_SOURCE_PRODUCTION_CASE_CANDIDATE_COUNT:
        blockers.append("source_creation_candidate_production_case_candidate_count_not_one")
    if _source_controlled_evidence_item_count(candidate) != EXPECTED_SOURCE_CONTROLLED_EVIDENCE_ITEM_COUNT:
        blockers.append("source_creation_candidate_controlled_evidence_item_count_not_five")
    if candidate.get("warning_count") != 1:
        blockers.append("source_creation_candidate_warning_count_not_one")
    if candidate.get("human_review_required") is not True:
        blockers.append("source_creation_candidate_human_review_required_not_true")
    if candidate.get("no_automatic_trust_upgrade") is not True:
        blockers.append("source_creation_candidate_no_automatic_trust_upgrade_not_true")
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if field in candidate and candidate.get(field) is not False:
            blockers.append(f"source_creation_candidate_{reason.removeprefix('source_')}")
    return _dedupe(blockers)
...
def _blocked_status(blockers: list[str]) -> str:
    ...
    if first.startswith("blocked_"):
        return first
    if first == "creation_runtime_boundary_count_not_one":
        return "blocked_creation_runtime_boundary_limit_violation"
    ...
    if "schema" in first or "status" in first or "count" in first or "missing" in first or "8w58" in first:
        return "blocked_invalid_source_production_analysis_result_creation_candidate"
    return "blocked_source_boundary_violation"
...
def _requested_action_status(action: str) -> str:
    ...
    if "production_analysis_result_creation" in action and "runtime" not in action:
...
def _safe_source_candidates(source_candidate_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_candidate_set, dict):
        return []
    if "production_analysis_result_creation_candidates" not in source_candidate_set:
        return [_summary_candidate_from_source_set(source_candidate_set)]
    candidates = source_candidate_set.get("production_analysis_result_creation_candidates")
    if not isinstance(candidates, list):
        return []
    return [candidate for candidate in candidates if isinstance(candidate, dict)]


def _summary_candidate_from_source_set(source_candidate_set: dict[str, Any]) -> dict[str, Any]:
    return {
        "production_analysis_result_creation_candidate_schema": SOURCE_CANDIDATE_SCHEMA,
        "production_analysis_result_creation_candidate_id": _safe_label(
            source_candidate_set.get("production_analysis_result_creation_candidate_id")
        ),
        "source_production_analysis_result_creation_candidate_count": _source_creation_candidate_count(
            source_candidate_set
        ),
        ...
        "source_production_case_candidate_count": _source_production_case_candidate_count(source_candidate_set),
        "source_controlled_evidence_item_count": _source_controlled_evidence_item_count(source_candidate_set),
        "case_id_hint": _safe_label(source_candidate_set.get("case_id_hint")),
        "case_title_or_label_redacted": _safe_label(source_candidate_set.get("case_title_or_label_redacted")),
        "input_scope_summary_redacted": _safe_label(source_candidate_set.get("input_scope_summary_redacted")),
        "warning_count": _source_warning_count(source_candidate_set),
        "human_review_required": source_candidate_set.get("human_review_required"),
        "review_status": "human_review_required",
        "trust_boundary_label": "no_automatic_trust_upgrade",
        "verification_status_summary": "needs_review",
        "redaction_status": "redacted",
        "redaction_warnings": _safe_list(source_candidate_set.get("warnings")),
        "warning_labels": _safe_list(source_candidate_set.get("warnings")),
        "no_automatic_trust_upgrade": source_candidate_set.get("no_automatic_trust_upgrade"),
        "production_analysis_result_created": source_candidate_set.get("production_analysis_result_created"),
        "production_analysis_result_creation_executed": source_candidate_set.get(
            "production_analysis_result_creation_executed"
        ),
        "production_analysis_result_runtime_used": source_candidate_set.get("production_analysis_result_runtime_used"),
        "analysis_result_generation_executed": source_candidate_set.get("analysis_result_generation_executed"),
        "analysis_result_created": source_candidate_set.get("analysis_result_created"),
        "actual_analysis_execution_started": source_candidate_set.get("actual_analysis_execution_started"),
        "analysis_execution_started": source_candidate_set.get("analysis_execution_started"),
        "production_analysis_run_created": source_candidate_set.get("production_analysis_run_created"),
        "production_case_created": source_candidate_set.get("production_case_created"),
        "production_evidence_item_created": source_candidate_set.get("production_evidence_item_created"),
        "review_queue_item_created": source_candidate_set.get("review_queue_item_created"),
        "production_review_queue_item_created": source_candidate_set.get("production_review_queue_item_created"),
        "review_queue_runtime_used": source_candidate_set.get("review_queue_runtime_used"),
        "report_ready": source_candidate_set.get("report_ready"),
        "boundary_flags": source_candidate_set.get("boundary_flags", {}),
    }


def _source_candidate_schema(source_candidate_set: dict[str, Any] | None) -> str | None:
    if not isinstance(source_candidate_set, dict):
        return None
    value = source_candidate_set.get("source_production_analysis_result_creation_candidate_schema")
    if isinstance(value, str):
        return value
    candidates = _safe_source_candidates(source_candidate_set)
    if candidates:
        candidate_schema = candidates[0].get("production_analysis_result_creation_candidate_schema")
        if isinstance(candidate_schema, str):
            return candidate_schema
    return SOURCE_CANDIDATE_SCHEMA


def _source_creation_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    explicit_count = _safe_int(
        source_candidate_set.get("source_production_analysis_result_creation_candidate_count"),
        source_candidate_set.get("production_analysis_result_creation_candidate_count"),
    )
    if explicit_count:
...
def _source_creation_boundary_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_analysis_result_creation_boundary_count"))


def _source_creation_or_runtime_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(
        source_candidate_set.get(
            "source_production_analysis_result_creation_or_runtime_execution_candidate_count"
        )
    )


def _source_runtime_boundary_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_analysis_result_runtime_boundary_count"))


def _source_boundary_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_analysis_result_boundary_count"))


def _source_production_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_analysis_result_candidate_count"))


def _source_analysis_result_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_analysis_result_candidate_count"))


def _source_actual_analysis_execution_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_actual_analysis_execution_candidate_count"))


def _source_production_analysis_run_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_analysis_run_candidate_count"))


def _source_production_case_candidate_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_production_case_candidate_count"))


def _source_controlled_evidence_item_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("source_controlled_evidence_item_count"))


def _source_warning_count(source_candidate_set: dict[str, Any] | None) -> int:
    if not isinstance(source_candidate_set, dict):
        return 0
    return _safe_int(source_candidate_set.get("warning_count"))


def _source_candidate_ids(source_candidate: dict[str, Any]) -> list[str]:
    ids = source_candidate.get("source_production_analysis_result_creation_candidate_ids")
    if isinstance(ids, list):
        safe = [_safe_label(item) for item in ids]
        return [item for item in safe if item]
    single = _safe_label(source_candidate.get("production_analysis_result_creation_candidate_id"))
    return [single] if single else []


def _runtime_boundary_id(source_ids: list[str]) -> str:
    suffix = "-".join(source_ids) if source_ids else "safe-summary"
    return f"controlled-production-analysis-result-creation-runtime-boundary-{_slug(suffix)}"
...
def _boundary_flags(*, creation_runtime_boundary_only: bool = False) -> dict[str, bool]:
    return {
        "backend_only": True,
        "local_only": True,
        "creation_runtime_boundary_only": creation_runtime_boundary_only,
        "controlled_production_analysis_result_creation_candidate_derived_only": True,
        "human_review_required": True,
...
def _boundary_warnings(source_candidate_set: dict[str, Any] | None) -> list[str]:
    ...
    if isinstance(source_candidate_set, dict):
        warnings.extend(_safe_list(source_candidate_set.get("warnings")))
