...
CANDIDATE_SET_SCHEMA = "sentigraph_controlled_production_analysis_result_candidate_set_v0_1"
CANDIDATE_SCHEMA = "sentigraph_controlled_production_analysis_result_candidate_v0_1"
SUMMARY_SCHEMA = "sentigraph_controlled_production_analysis_result_candidate_summary_v0_1"
SOURCE_CANDIDATE_SET_SCHEMA = "sentigraph_controlled_analysis_result_candidate_set_v0_1"
SOURCE_CANDIDATE_SCHEMA = "sentigraph_controlled_analysis_result_candidate_v0_1"
PHASE = "8W-43"
APPROVAL_PHRASE = "APPROVE_8W_43_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CANDIDATE_HELPER_IMPLEMENTATION"
SOURCE_WARN_STATUS = "analysis_result_candidate_set_warn_manual_review_required"
WARN_STATUS = "production_analysis_result_candidate_set_warn_manual_review_required"
...
SOURCE_TRUE_FIELDS = {
    ...
    "analysis_result_candidate_created": "source_analysis_result_candidate_created_not_true",
}
...
def build_controlled_production_analysis_result_candidate_set(
    controlled_analysis_result_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build one local production-analysis-result-candidate boundary object from an 8W-40 candidate set."""

    blockers: list[str] = []
    blockers.extend(_approval_blockers(exact_approval_phrase))
    blockers.extend(_requested_action_blockers(requested_actions))

    source_candidates = _safe_source_candidates(controlled_analysis_result_candidate_set)
    source_analysis_result_candidate_count = _source_analysis_result_candidate_count(
        controlled_analysis_result_candidate_set,
        source_candidates,
    )
    ...
    if blockers:
        return _base_output(
            ...
            warnings=[],
            candidate=None,
            source_candidate_set=controlled_analysis_result_candidate_set,
    ...
    return _base_output(
        ...
        created=True,
        blockers=[],
        warnings=_candidate_warnings(controlled_analysis_result_candidate_set),
        candidate=candidate,
        source_candidate_set=controlled_analysis_result_candidate_set,
        source_analysis_result_candidate_count=source_analysis_result_candidate_count,
        source_actual_candidate_count=source_actual_candidate_count,
...
create_controlled_production_analysis_result_candidate_set = (
    build_controlled_production_analysis_result_candidate_set
)


def build_safe_controlled_production_analysis_result_candidate_summary(
    controlled_analysis_result_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_candidate_set_schema": CANDIDATE_SET_SCHEMA,
        "production_analysis_result_candidate_set_status": candidate_set[
            "production_analysis_result_candidate_set_status"
        ],
        "source_analysis_result_candidate_set_schema": candidate_set[
            "source_analysis_result_candidate_set_schema"
        ],
        "source_analysis_result_candidate_schema": candidate_set[
            "source_analysis_result_candidate_schema"
        ],
        ...
        "production_analysis_result_candidate_count": candidate_set[
            "production_analysis_result_candidate_count"
        ],
        "production_analysis_result_candidate_created": candidate_set[
        ...
        "analysis_result_candidate_created_upstream": candidate_set[
            "analysis_result_candidate_created_upstream"
        ],
...
def _base_output(
    *,
    ...
    return {
        "production_analysis_result_candidate_set_schema": CANDIDATE_SET_SCHEMA,
        "phase": PHASE,
        "production_analysis_result_candidate_set_status": status,
        "created_at": _utc_now(),
        "input_source_kind": "controlled_analysis_result_candidate",
        "source_analysis_result_candidate_set_schema": _safe_source_value(
            source_candidate_set,
            "analysis_result_candidate_set_schema",
        ),
        "source_analysis_result_candidate_schema": _source_analysis_result_candidate_schema(
            source_candidate_set
        ),
        "source_analysis_result_candidate_count": source_analysis_result_candidate_count,
        ...
        "production_analysis_result_candidate_count": len(candidates),
        "warning_count": _source_warning_count(source_candidate_set),
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "analysis_result_candidate_created_upstream": _truthy(
            source_candidate_set.get("analysis_result_candidate_created")
            if isinstance(source_candidate_set, dict)
            else False
        ),
        "production_analysis_result_candidate_created": created,
        ...
        "production_analysis_result_candidates": candidates,
    }
...
def _candidate_from_source(
    ...
    source_ids = [
        _safe_token(candidate.get("analysis_result_candidate_id"))
        for candidate in source_candidates
        if _safe_token(candidate.get("analysis_result_candidate_id")) != "unknown"
    ]
    source = source_candidates[0] if source_candidates else {}
    return {
        "production_analysis_result_candidate_schema": CANDIDATE_SCHEMA,
        "production_analysis_result_candidate_id": _production_analysis_result_candidate_id(source_ids),
        "source_analysis_result_candidate_id": source_ids[0] if source_ids else None,
        "source_analysis_result_candidate_ids": source_ids,
        "source_analysis_result_candidate_count": source_analysis_result_candidate_count,
        "source_actual_analysis_execution_candidate_count": source_actual_candidate_count,
        "source_production_analysis_run_candidate_count": source_analysis_run_candidate_count,
        "source_production_case_candidate_count": source_case_candidate_count,
        "source_controlled_evidence_item_count": source_controlled_evidence_item_count,
        "case_id_hint": _safe_label(source.get("case_id_hint")),
        "case_title_or_label_redacted": _safe_label(source.get("case_title_or_label_redacted")),
        "input_scope_summary_redacted": _safe_label(source.get("input_scope_summary_redacted")),
        ...
        "warning_count": _source_warning_count(source_candidate_set),
        "human_review_required": True,
        "review_status": _safe_label(source.get("review_status")) or "human_review_required",
        "trust_boundary_label": _safe_label(source.get("trust_boundary_label")) or "no_automatic_trust_upgrade",
        "verification_status_summary": _safe_label(source.get("verification_status_summary")) or "needs_review",
        "redaction_status": _safe_label(source.get("redaction_status")) or "redacted",
        "redaction_warnings": _merged_warning_labels(source_candidate_set, source_candidates),
...
def _source_candidate_set_blockers(
    ...
    if not isinstance(source_candidate_set, dict):
        return ["source_analysis_result_candidate_set_missing_or_not_object"]

    blockers: list[str] = []
    expected = {
        "analysis_result_candidate_set_schema": (
            SOURCE_CANDIDATE_SET_SCHEMA,
            "source_analysis_result_candidate_set_schema_wrong",
        ),
    ...
    if len(source_candidates) != EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_analysis_result_candidates_count_not_one")
    ...
    for field, value in source_candidate_set.items():
        if field == "analysis_result_candidates":
            continue
        if field in FORBIDDEN_SOURCE_FIELDS:
            if field in SOURCE_FALSE_FIELDS and value is False:
                continue
            blockers.append(f"forbidden_source_analysis_result_candidate_set_field:{field}")
        elif _contains_forbidden_value(value):
            blockers.append(f"source_analysis_result_candidate_set_forbidden_value:{field}")
    return _dedupe(blockers)
...
def _source_candidate_blockers(source_candidates: list[dict[str, Any]]) -> list[str]:
    ...
    for candidate in source_candidates:
        if not isinstance(candidate, dict):
            blockers.append("source_analysis_result_candidate_not_object")
            continue
        for field, value in candidate.items():
            if field in FORBIDDEN_SOURCE_FIELDS:
                blockers.append(f"forbidden_source_analysis_result_candidate_field:{field}")
            elif _contains_forbidden_value(value):
                blockers.append(f"source_analysis_result_candidate_forbidden_value:{field}")
        if candidate.get("analysis_result_candidate_schema") != SOURCE_CANDIDATE_SCHEMA:
            blockers.append("source_analysis_result_candidate_schema_wrong")
        if candidate.get("source_analysis_result_candidate_count") not in (None, 1):
        ...
        if candidate.get("human_review_required") is not True:
            blockers.append("source_analysis_result_candidate_human_review_required_not_true")
        if candidate.get("no_automatic_trust_upgrade") is not True:
            blockers.append("source_analysis_result_candidate_no_automatic_trust_upgrade_not_true")
        if candidate.get("analysis_result_candidate_only") is not True:
            blockers.append("source_analysis_result_candidate_only_not_true")
...
def _safe_source_candidates(source_candidate_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_candidate_set, dict):
        return []
    candidates = source_candidate_set.get("analysis_result_candidates")
...
def _source_analysis_result_candidate_count(
    ...
    if isinstance(source_candidate_set, dict):
        count = source_candidate_set.get("analysis_result_candidate_count")
...
def _source_analysis_result_candidate_schema(source_candidate_set: dict[str, Any] | None) -> str | None:
    candidates = _safe_source_candidates(source_candidate_set)
    if candidates:
        return _safe_label(candidates[0].get("analysis_result_candidate_schema"))
    return None
...
def _blocked_status(blockers: list[str]) -> str:
    if not blockers:
        return "blocked_invalid_source_analysis_result_candidate"
    first = blockers[0]
    if first in {
    ...
    if first.startswith("source_analysis_result_candidate_count") or first.startswith(
        "source_analysis_result_candidates_count"
    ):
        return "blocked_candidate_limit_violation"
    if first.startswith("source_actual_analysis_execution_candidate_count"):
        return "blocked_candidate_limit_violation"
    ...
    if first.startswith("source_controlled_evidence_item_count"):
        return "blocked_candidate_limit_violation"
    if first.startswith("forbidden_source") or first.startswith("source_analysis_result_candidate_forbidden"):
...
def _production_analysis_result_candidate_id(source_ids: list[str]) -> str:
    first_id = source_ids[0] if source_ids else "unknown"
    return f"controlled-production-analysis-result-candidate-001-{first_id}"
