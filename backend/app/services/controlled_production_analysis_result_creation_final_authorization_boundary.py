...
SOURCE_WARN_STATUS = "production_analysis_result_creation_execution_boundary_set_warn_manual_review_required"
WARN_STATUS = "production_analysis_result_creation_final_authorization_boundary_set_warn_manual_review_required"
EXPECTED_8W62_DECISION = "ready"
EXPECTED_8W62_NEXT_BOUNDARY_OPTION = (
...
EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_EXECUTION_BOUNDARY_COUNT = 1
...
SOURCE_TRUE_FIELDS = {
    ...
    "production_analysis_result_creation_execution_boundary_created": (
        "source_production_analysis_result_creation_execution_boundary_created_not_true"
    ),
...
def build_controlled_production_analysis_result_creation_final_authorization_boundary_set(
    controlled_production_analysis_result_creation_execution_boundary_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    decision_8w62: str | None,
    selected_next_boundary_option_8w62: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
    final_authorization_boundary_count: int = 1,
) -> dict[str, Any]:
    blockers: list[str] = []
    blockers.extend(_approval_blockers(exact_approval_phrase))
    blockers.extend(_requested_action_blockers(requested_actions))
    if final_authorization_boundary_count != 1:
        blockers.append("final_authorization_boundary_count_not_one")

    source_boundaries: list[dict[str, Any]] = []
    if not blockers:
        source_boundaries = _safe_source_execution_boundaries(
            controlled_production_analysis_result_creation_execution_boundary_set
        )
        blockers.extend(
            _source_execution_boundary_set_blockers(
                controlled_production_analysis_result_creation_execution_boundary_set,
                source_boundaries,
                decision_8w62=decision_8w62,
                selected_next_boundary_option_8w62=selected_next_boundary_option_8w62,
            )
        )
    ...
    if not blockers:
        blockers.extend(
            _source_execution_boundary_blockers(
                controlled_production_analysis_result_creation_execution_boundary_set,
                source_boundaries,
            )
        )
        if not blockers:
            boundaries.append(
                _final_authorization_boundary_from_source(
                    source_boundaries[0],
                    controlled_production_analysis_result_creation_execution_boundary_set,
                )
            )
    ...
    return _base_output(
        source_execution_boundary_set=controlled_production_analysis_result_creation_execution_boundary_set,
        status=status,
...
create_controlled_production_analysis_result_creation_final_authorization_boundary_set = (
    build_controlled_production_analysis_result_creation_final_authorization_boundary_set
)


def build_safe_controlled_production_analysis_result_creation_final_authorization_boundary_summary(
    controlled_production_analysis_result_creation_execution_boundary_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    decision_8w62: str | None,
    ...
    boundary_set = build_controlled_production_analysis_result_creation_final_authorization_boundary_set(
        controlled_production_analysis_result_creation_execution_boundary_set,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_creation_final_authorization_boundary_set_schema": boundary_set[
            "production_analysis_result_creation_final_authorization_boundary_set_schema"
        ],
        "production_analysis_result_creation_final_authorization_boundary_set_status": boundary_set[
        ...
        "source_production_analysis_result_creation_execution_boundary_set_schema": boundary_set[
            "source_production_analysis_result_creation_execution_boundary_set_schema"
        ],
        "source_production_analysis_result_creation_execution_boundary_schema": boundary_set[
            "source_production_analysis_result_creation_execution_boundary_schema"
        ],
        "source_production_analysis_result_creation_execution_boundary_count": boundary_set[
        ...
        "final_authorization_boundary_mode": boundary_set["final_authorization_boundary_mode"],
        "production_analysis_result_creation_final_authorization_boundary_count": boundary_set[
            "production_analysis_result_creation_final_authorization_boundary_count"
        ],
        ...
        "production_analysis_result_creation_execution_boundary_created_upstream": boundary_set[
            "production_analysis_result_creation_execution_boundary_created_upstream"
        ],
        "production_analysis_result_creation_final_authorization_boundary_created": boundary_set[
...
def _base_output(
    *,
    source_execution_boundary_set: dict[str, Any] | None,
    status: str,
    boundaries: list[dict[str, Any]],
    blockers: list[str],
) -> dict[str, Any]:
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_creation_final_authorization_boundary_set_status": status,
        "input_source_kind": "controlled_production_analysis_result_creation_execution_boundary",
        "source_production_analysis_result_creation_execution_boundary_set_schema": _safe_source_value(
            source_execution_boundary_set,
            "production_analysis_result_creation_execution_boundary_set_schema",
        )
        or SOURCE_EXECUTION_BOUNDARY_SET_SCHEMA,
        "source_production_analysis_result_creation_execution_boundary_schema": (
            _source_execution_boundary_schema(source_execution_boundary_set)
        ),
        "source_production_analysis_result_creation_execution_boundary_count": (
            _source_creation_execution_boundary_count(source_execution_boundary_set)
        ),
        "source_production_analysis_result_creation_runtime_boundary_count": (
            _source_creation_runtime_boundary_count(source_execution_boundary_set)
        ),
        ...
        "source_production_analysis_result_creation_or_runtime_execution_candidate_count": (
            _source_creation_or_runtime_candidate_count(source_execution_boundary_set)
        ),
        "source_production_analysis_result_runtime_boundary_count": _source_runtime_boundary_count(
            source_execution_boundary_set
        ),
        "source_production_analysis_result_boundary_count": _source_boundary_count(
            source_execution_boundary_set
        ),
        ...
        "production_analysis_result_creation_final_authorization_boundary_count": len(boundaries),
        "warning_count": _source_warning_count(source_execution_boundary_set),
        "human_review_required": bool(
            isinstance(source_execution_boundary_set, dict)
            and source_execution_boundary_set.get("human_review_required") is True
        ),
        "no_automatic_trust_upgrade": bool(
            isinstance(source_execution_boundary_set, dict)
            and source_execution_boundary_set.get("no_automatic_trust_upgrade") is True
        ),
        "production_analysis_result_creation_execution_boundary_created_upstream": bool(
            isinstance(source_execution_boundary_set, dict)
            and source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_created")
            is True
        ),
        "production_analysis_result_creation_final_authorization_boundary_created": boundary_created,
        ...
        "production_analysis_result_creation_final_authorization_boundaries": boundaries,
        "boundary_flags": _boundary_flags(final_authorization_boundary_only=boundary_created),
        "runtime_side_effects": _runtime_side_effects(),
        "warnings": _boundary_warnings(source_execution_boundary_set),
...
def _final_authorization_boundary_from_source(
    source_execution_boundary: dict[str, Any],
    source_execution_boundary_set: dict[str, Any] | None,
) -> dict[str, Any]:
    source_ids = _source_execution_boundary_ids(source_execution_boundary)
    return {
        ...
        "production_analysis_result_creation_final_authorization_boundary_id": (
            _final_authorization_boundary_id(source_ids)
        ),
        "source_production_analysis_result_creation_execution_boundary_ids": source_ids,
        "source_production_analysis_result_creation_execution_boundary_count": (
            _source_creation_execution_boundary_count(source_execution_boundary_set)
        ),
        "source_production_analysis_result_creation_runtime_boundary_count": (
        ...
        "case_id_hint": _safe_label(source_execution_boundary.get("case_id_hint")),
        "case_title_or_label_redacted": _safe_label(
            source_execution_boundary.get("case_title_or_label_redacted")
        ),
        "input_scope_summary_redacted": _safe_label(
            source_execution_boundary.get("input_scope_summary_redacted")
        ),
        "intended_final_authorization_boundary_labels": [
            "final_authorization_boundary_only",
            "selected_sample_only",
            "human_review_required",
            "no_automatic_trust_upgrade",
        ],
        "intended_module_scope_labels": [
            "production_analysis_result_creation_final_authorization_boundary",
        ...
        "final_authorization_gate_labels": [
            "final_authorization_boundary_gate_only",
            "manual_review_required",
            "not_final_authorization_performed",
        ...
        "redaction_warnings": _safe_list(source_execution_boundary.get("redaction_warnings"))
        or _boundary_warnings(source_execution_boundary_set),
        "warning_labels": _safe_list(source_execution_boundary.get("warning_labels"))
        or _boundary_warnings(source_execution_boundary_set),
        "blocker_codes": [],
        ...
        "review_queue_runtime_used": False,
        "report_ready": False,
        "boundary_flags": _boundary_flags(final_authorization_boundary_only=True),
    }
...
def _source_execution_boundary_set_blockers(
    source_execution_boundary_set: dict[str, Any] | None,
    source_boundaries: list[dict[str, Any]],
    *,
    decision_8w62: str | None,
    ...
    blockers: list[str] = []
    if decision_8w62 != EXPECTED_8W62_DECISION:
        blockers.append("8w62_decision_not_ready")
    if selected_next_boundary_option_8w62 != EXPECTED_8W62_NEXT_BOUNDARY_OPTION:
        blockers.append("8w62_selected_next_boundary_option_not_ready")
    if not isinstance(source_execution_boundary_set, dict):
        blockers.append("source_execution_boundary_set_missing")
        return blockers
    if _contains_forbidden_value(source_execution_boundary_set):
        blockers.append("forbidden_source_payload_detected")
        return blockers
    if _source_execution_boundary_set_schema(source_execution_boundary_set) != SOURCE_EXECUTION_BOUNDARY_SET_SCHEMA:
        blockers.append("source_execution_boundary_set_schema_mismatch")
    if source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_set_status") != (
        SOURCE_WARN_STATUS
    ):
        blockers.append("source_execution_boundary_set_status_mismatch")
    if _source_execution_boundary_schema(source_execution_boundary_set) != SOURCE_EXECUTION_BOUNDARY_SCHEMA:
        blockers.append("source_execution_boundary_schema_mismatch")
    if _source_creation_execution_boundary_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_EXECUTION_BOUNDARY_COUNT
    ):
        blockers.append("source_creation_execution_boundary_count_not_one")
    if (
        "production_analysis_result_creation_execution_boundary_count" in source_execution_boundary_set
        and source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_count") != 1
    ):
        blockers.append("production_analysis_result_creation_execution_boundary_count_not_one")
    if _source_creation_runtime_boundary_count(source_execution_boundary_set) != (
    ...
    if _source_creation_candidate_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_CANDIDATE_COUNT
    ):
        blockers.append("source_creation_candidate_count_not_one")
    if _source_creation_boundary_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_BOUNDARY_COUNT
    ):
        blockers.append("source_creation_boundary_count_not_one")
    if _source_creation_or_runtime_candidate_count(source_execution_boundary_set) != (
    ...
    if _source_runtime_boundary_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_RUNTIME_BOUNDARY_COUNT
    ):
        blockers.append("source_runtime_boundary_count_not_one")
    if _source_boundary_count(source_execution_boundary_set) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_COUNT:
        blockers.append("source_boundary_count_not_one")
    if _source_production_candidate_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CANDIDATE_COUNT
    ):
        blockers.append("source_production_candidate_count_not_one")
    if _source_analysis_result_candidate_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT
    ):
        blockers.append("source_analysis_result_candidate_count_not_one")
    if _source_actual_analysis_execution_candidate_count(source_execution_boundary_set) != (
    ...
    if _source_production_analysis_run_candidate_count(source_execution_boundary_set) != (
    ...
    if _source_production_case_candidate_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_PRODUCTION_CASE_CANDIDATE_COUNT
    ):
        blockers.append("source_production_case_candidate_count_not_one")
    if _source_controlled_evidence_item_count(source_execution_boundary_set) != (
        EXPECTED_SOURCE_CONTROLLED_EVIDENCE_ITEM_COUNT
    ):
        blockers.append("source_controlled_evidence_item_count_not_five")
    if _source_warning_count(source_execution_boundary_set) != 1:
        blockers.append("source_warning_count_not_one")
    for field, reason in SOURCE_TRUE_FIELDS.items():
        if source_execution_boundary_set.get(field) is not True:
            blockers.append(reason)
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if field in source_execution_boundary_set and source_execution_boundary_set.get(field) is not False:
            blockers.append(reason)
    blockers.extend(_runtime_side_effect_blockers(source_execution_boundary_set.get("runtime_side_effects")))
    if source_boundaries == []:
        blockers.append("source_execution_boundary_missing_or_malformed")
    return _dedupe(blockers)


def _source_execution_boundary_blockers(
    source_execution_boundary_set: dict[str, Any] | None,
    source_boundaries: list[dict[str, Any]],
) -> list[str]:
    if not source_boundaries:
        return ["source_execution_boundary_missing_or_malformed"]
    boundary = source_boundaries[0]
    blockers: list[str] = []
    if _contains_forbidden_value(boundary):
        return ["forbidden_source_boundary_payload_detected"]
    if boundary.get("production_analysis_result_creation_execution_boundary_schema") != SOURCE_EXECUTION_BOUNDARY_SCHEMA:
        blockers.append("source_execution_boundary_item_schema_mismatch")
    if _source_creation_execution_boundary_count(source_execution_boundary_set) != 1:
        blockers.append("source_execution_boundary_count_not_one")
    if _source_creation_runtime_boundary_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_RUNTIME_BOUNDARY_COUNT:
        blockers.append("source_execution_boundary_creation_runtime_boundary_count_not_one")
    if _source_creation_candidate_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_CANDIDATE_COUNT:
        blockers.append("source_execution_boundary_creation_candidate_count_not_one")
    if _source_creation_boundary_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_BOUNDARY_COUNT:
        blockers.append("source_execution_boundary_creation_boundary_count_not_one")
    if _source_creation_or_runtime_candidate_count(boundary) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CREATION_OR_RUNTIME_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_execution_boundary_creation_or_runtime_execution_candidate_count_not_one")
    if _source_runtime_boundary_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_RUNTIME_BOUNDARY_COUNT:
        blockers.append("source_execution_boundary_runtime_boundary_count_not_one")
    if _source_boundary_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_COUNT:
        blockers.append("source_execution_boundary_boundary_count_not_one")
    if _source_production_candidate_count(boundary) != EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_execution_boundary_production_candidate_count_not_one")
    if _source_analysis_result_candidate_count(boundary) != EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_execution_boundary_analysis_result_candidate_count_not_one")
    if _source_actual_analysis_execution_candidate_count(boundary) != (
        EXPECTED_SOURCE_ACTUAL_ANALYSIS_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_execution_boundary_actual_analysis_execution_candidate_count_not_one")
    if _source_production_analysis_run_candidate_count(boundary) != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RUN_CANDIDATE_COUNT
    ):
        blockers.append("source_execution_boundary_production_analysis_run_candidate_count_not_one")
    if _source_production_case_candidate_count(boundary) != EXPECTED_SOURCE_PRODUCTION_CASE_CANDIDATE_COUNT:
        blockers.append("source_execution_boundary_production_case_candidate_count_not_one")
    if _source_controlled_evidence_item_count(boundary) != EXPECTED_SOURCE_CONTROLLED_EVIDENCE_ITEM_COUNT:
        blockers.append("source_execution_boundary_controlled_evidence_item_count_not_five")
    if boundary.get("warning_count") != 1:
        blockers.append("source_execution_boundary_warning_count_not_one")
    if boundary.get("human_review_required") is not True:
        blockers.append("source_execution_boundary_human_review_required_not_true")
    if boundary.get("no_automatic_trust_upgrade") is not True:
        blockers.append("source_execution_boundary_no_automatic_trust_upgrade_not_true")
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if field in boundary and boundary.get(field) is not False:
            blockers.append(f"source_execution_boundary_{reason.removeprefix('source_')}")
    return _dedupe(blockers)
...
def _blocked_status(blockers: list[str]) -> str:
    ...
    if first.startswith("blocked_"):
        return first
    if first == "final_authorization_boundary_count_not_one":
        return "blocked_final_authorization_boundary_limit_violation"
    ...
    if "schema" in first or "status" in first or "count" in first or "missing" in first or "8w62" in first:
        return "blocked_invalid_source_production_analysis_result_creation_execution_boundary"
    return "blocked_source_boundary_violation"


def _requested_action_status(action: str) -> str:
    if "production_analysis_result_creation_final_authorization" in action:
        return "blocked_unapproved_production_analysis_result_creation_final_authorization_request"
...
def _safe_source_execution_boundaries(source_execution_boundary_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_execution_boundary_set, dict):
        return []
    if "production_analysis_result_creation_execution_boundaries" not in source_execution_boundary_set:
        return [_summary_execution_boundary_from_source_set(source_execution_boundary_set)]
    boundaries = source_execution_boundary_set.get("production_analysis_result_creation_execution_boundaries")
    if not isinstance(boundaries, list):
        return []
    return [boundary for boundary in boundaries if isinstance(boundary, dict)]


def _summary_execution_boundary_from_source_set(source_execution_boundary_set: dict[str, Any]) -> dict[str, Any]:
    return {
        "production_analysis_result_creation_execution_boundary_schema": SOURCE_EXECUTION_BOUNDARY_SCHEMA,
        "production_analysis_result_creation_execution_boundary_id": _safe_label(
            source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_id")
        ),
        "source_production_analysis_result_creation_runtime_boundary_count": (
            _source_creation_runtime_boundary_count(source_execution_boundary_set)
        ),
        ...
        "source_production_analysis_result_boundary_count": _source_boundary_count(source_execution_boundary_set),
        "source_production_analysis_result_candidate_count": _source_production_candidate_count(
            source_execution_boundary_set
        ),
        "source_analysis_result_candidate_count": _source_analysis_result_candidate_count(
        ...
        "case_id_hint": _safe_label(source_execution_boundary_set.get("case_id_hint")),
        "case_title_or_label_redacted": _safe_label(
            source_execution_boundary_set.get("case_title_or_label_redacted")
        ),
        "input_scope_summary_redacted": _safe_label(
            source_execution_boundary_set.get("input_scope_summary_redacted")
        ),
        "warning_count": _source_warning_count(source_execution_boundary_set),
        "human_review_required": source_execution_boundary_set.get("human_review_required"),
        "review_status": "human_review_required",
        "trust_boundary_label": "no_automatic_trust_upgrade",
        "verification_status_summary": "needs_review",
        "redaction_status": "redacted",
        "redaction_warnings": _safe_list(source_execution_boundary_set.get("warnings")),
        "warning_labels": _safe_list(source_execution_boundary_set.get("warnings")),
        "no_automatic_trust_upgrade": source_execution_boundary_set.get("no_automatic_trust_upgrade"),
        "production_analysis_result_creation_final_authorization_performed": source_execution_boundary_set.get(
            "production_analysis_result_creation_final_authorization_performed"
        ),
        "production_analysis_result_created": source_execution_boundary_set.get("production_analysis_result_created"),
        "production_analysis_result_creation_executed": source_execution_boundary_set.get(
            "production_analysis_result_creation_executed"
        ),
        "production_analysis_result_runtime_used": source_execution_boundary_set.get(
            "production_analysis_result_runtime_used"
        ),
        "analysis_result_generation_executed": source_execution_boundary_set.get(
            "analysis_result_generation_executed"
        ),
        "analysis_result_created": source_execution_boundary_set.get("analysis_result_created"),
        "actual_analysis_execution_started": source_execution_boundary_set.get(
            "actual_analysis_execution_started"
        ),
        "analysis_execution_started": source_execution_boundary_set.get("analysis_execution_started"),
        "production_analysis_run_created": source_execution_boundary_set.get("production_analysis_run_created"),
        "production_case_created": source_execution_boundary_set.get("production_case_created"),
        "production_evidence_item_created": source_execution_boundary_set.get("production_evidence_item_created"),
        "review_queue_item_created": source_execution_boundary_set.get("review_queue_item_created"),
        "production_review_queue_item_created": source_execution_boundary_set.get(
            "production_review_queue_item_created"
        ),
        "review_queue_runtime_used": source_execution_boundary_set.get("review_queue_runtime_used"),
        "report_ready": source_execution_boundary_set.get("report_ready"),
        "boundary_flags": source_execution_boundary_set.get("boundary_flags", {}),
    }


def _source_execution_boundary_set_schema(source_execution_boundary_set: dict[str, Any] | None) -> str | None:
    if not isinstance(source_execution_boundary_set, dict):
        return None
    value = source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_set_schema")
    if isinstance(value, str):
        return value
    return _safe_source_value(
        source_execution_boundary_set,
        "source_production_analysis_result_creation_execution_boundary_set_schema",
    )


def _source_execution_boundary_schema(source_execution_boundary_set: dict[str, Any] | None) -> str | None:
    if not isinstance(source_execution_boundary_set, dict):
        return None
    value = source_execution_boundary_set.get("source_production_analysis_result_creation_execution_boundary_schema")
    if isinstance(value, str):
        return value
    boundaries = _safe_source_execution_boundaries(source_execution_boundary_set)
    if boundaries:
        boundary_schema = boundaries[0].get("production_analysis_result_creation_execution_boundary_schema")
        if isinstance(boundary_schema, str):
            return boundary_schema
    return SOURCE_EXECUTION_BOUNDARY_SCHEMA


def _source_creation_execution_boundary_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    explicit_count = _safe_int(
        source_execution_boundary_set.get("source_production_analysis_result_creation_execution_boundary_count"),
        source_execution_boundary_set.get("production_analysis_result_creation_execution_boundary_count"),
    )
    if explicit_count:
...
def _source_creation_runtime_boundary_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_creation_runtime_boundary_count"))


def _source_creation_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_creation_candidate_count"))


def _source_creation_boundary_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_creation_boundary_count"))


def _source_creation_or_runtime_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(
        source_execution_boundary_set.get(
            "source_production_analysis_result_creation_or_runtime_execution_candidate_count"
        )
    )


def _source_runtime_boundary_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_runtime_boundary_count"))


def _source_boundary_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_boundary_count"))


def _source_production_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_result_candidate_count"))


def _source_analysis_result_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_analysis_result_candidate_count"))


def _source_actual_analysis_execution_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_actual_analysis_execution_candidate_count"))


def _source_production_analysis_run_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_analysis_run_candidate_count"))


def _source_production_case_candidate_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_production_case_candidate_count"))


def _source_controlled_evidence_item_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("source_controlled_evidence_item_count"))


def _source_warning_count(source_execution_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_execution_boundary_set, dict):
        return 0
    return _safe_int(source_execution_boundary_set.get("warning_count"))


def _source_execution_boundary_ids(source_execution_boundary: dict[str, Any]) -> list[str]:
    ids = source_execution_boundary.get("source_production_analysis_result_creation_execution_boundary_ids")
    if isinstance(ids, list):
        safe = [_safe_label(item) for item in ids]
        return [item for item in safe if item]
    single = _safe_label(source_execution_boundary.get("production_analysis_result_creation_execution_boundary_id"))
    return [single] if single else []


def _final_authorization_boundary_id(source_ids: list[str]) -> str:
    suffix = "-".join(source_ids) if source_ids else "safe-summary"
    return f"controlled-production-analysis-result-creation-final-authorization-boundary-{_slug(suffix)}"
...
def _boundary_flags(*, final_authorization_boundary_only: bool = False) -> dict[str, bool]:
    return {
        "backend_only": True,
        "local_only": True,
        "final_authorization_boundary_only": final_authorization_boundary_only,
        "controlled_production_analysis_result_creation_execution_boundary_derived_only": True,
        "human_review_required": True,
...
def _boundary_warnings(source_execution_boundary_set: dict[str, Any] | None) -> list[str]:
    warnings = ["manual_review_required", "selected_sample_only"]
    if isinstance(source_execution_boundary_set, dict):
        warnings.extend(_safe_list(source_execution_boundary_set.get("warnings")))
