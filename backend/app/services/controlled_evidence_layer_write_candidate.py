...
SUMMARY_SCHEMA = "sentigraph_controlled_evidence_layer_write_candidate_summary_v0_1"
...
SOURCE_PHASE = "8W-16"
PHASE = "8W-19"
APPROVAL_PHRASE = "批准 8W-19 Controlled Evidence Layer Write Candidate Helper Implementation"
SOURCE_WARN_STATUS = "evidence_layer_import_candidate_set_warn_manual_review_required"
...
REQUIRED_SOURCE_IMPORT_CANDIDATE_FIELDS = [
    "evidence_layer_import_candidate_schema",
    "evidence_layer_import_candidate_id",
...
OPTIONAL_SAFE_SOURCE_IMPORT_CANDIDATE_FIELDS = [
    "source_review_queue_candidate_schema",
    "preview_hash",
    "platform",
    "evidence_type",
    "coarse_created_at",
...
FORBIDDEN_SOURCE_IMPORT_CANDIDATE_FIELDS = {
    "evidence_item_id",
...
def build_controlled_evidence_layer_write_candidate_set(
    controlled_evidence_layer_import_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    candidate_limit: int | None = None,
    requested_actions: list[str] | dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Create local, redacted, human-review-only evidence-layer-write-candidate-shaped objects."""

    blockers: list[str] = []
    blockers.extend(_approval_blockers(exact_approval_phrase))
    blockers.extend(_requested_action_blockers(requested_actions))

    source_candidates = _safe_source_candidates(controlled_evidence_layer_import_candidate_set)
    source_candidate_count = _source_evidence_layer_import_candidate_count(
        controlled_evidence_layer_import_candidate_set,
        source_candidates,
    )
    ...
    blockers.extend(
        _source_evidence_layer_import_candidate_set_blockers(
            controlled_evidence_layer_import_candidate_set,
            source_candidates,
        )
    )
    ...
    if blockers:
        return _base_output(
            ...
            blockers=blockers,
            write_candidates=[],
            source_candidate_set=controlled_evidence_layer_import_candidate_set,
            source_candidate_count=source_candidate_count,
        )

    status = WARN_STATUS if _source_warning_count(controlled_evidence_layer_import_candidate_set) else READY_STATUS
    return _base_output(
        status=status,
        created=True,
        blockers=[],
        warnings=_write_candidate_warnings(controlled_evidence_layer_import_candidate_set),
...
create_controlled_evidence_layer_write_candidate_set = build_controlled_evidence_layer_write_candidate_set


def build_safe_controlled_evidence_layer_write_candidate_summary(
    controlled_evidence_layer_import_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    candidate_limit: int | None = None,
    ...
    return {
        ...
        "source_evidence_layer_import_candidate_count": candidate_set[
            "source_evidence_layer_import_candidate_count"
        ],
...
def _base_output(
    *,
    ...
    return {
        ...
        "created_at": _utc_now(),
        "input_source_kind": "controlled_evidence_layer_import_candidate_set",
        "source_evidence_layer_import_candidate_set_schema": _safe_source_value(
            source_candidate_set,
            "evidence_layer_import_candidate_set_schema",
        ),
        "source_evidence_layer_import_candidate_set_status": _safe_source_value(
            source_candidate_set,
            "evidence_layer_import_candidate_set_status",
        ),
        "source_evidence_layer_import_candidate_count": source_candidate_count,
...
def _source_evidence_layer_import_candidate_set_blockers(
    source_candidate_set: dict[str, Any] | None,
    source_candidates: list[dict[str, Any]],
) -> list[str]:
    if not isinstance(source_candidate_set, dict):
        return ["source_evidence_layer_import_candidate_set_missing_or_not_object"]

    blockers: list[str] = []
    expected = {
        "evidence_layer_import_candidate_set_schema": (
            SOURCE_EVIDENCE_LAYER_IMPORT_CANDIDATE_SET_SCHEMA,
            "source_evidence_layer_import_candidate_set_schema_wrong",
        ),
        "phase": (SOURCE_PHASE, "source_evidence_layer_import_candidate_set_phase_wrong"),
        "evidence_layer_import_candidate_set_status": (
            SOURCE_WARN_STATUS,
            "source_evidence_layer_import_candidate_set_status_not_warn_manual_review",
        ),
    }
    ...
    true_fields = {
        ...
        "evidence_layer_import_candidate_helper_implementation_approved": (
            "source_evidence_layer_import_candidate_helper_not_approved"
        ),
        "evidence_layer_import_candidate_created": "source_evidence_layer_import_candidate_created_not_true",
    }
    ...
    candidate_count = source_candidate_set.get("evidence_layer_import_candidate_count")
    source_review_queue_candidate_count = source_candidate_set.get("source_review_queue_candidate_count")
    if not isinstance(candidate_count, int) or isinstance(candidate_count, bool):
        blockers.append("source_evidence_layer_import_candidate_count_missing_or_invalid")
    elif candidate_count != len(source_candidates):
        blockers.append("source_evidence_layer_import_candidate_count_inconsistent")
    elif candidate_count < 1:
        blockers.append("source_evidence_layer_import_candidates_missing")
    elif candidate_count > HARD_CANDIDATE_BOUND:
        blockers.append("source_evidence_layer_import_candidate_count_too_high")

    if not isinstance(source_review_queue_candidate_count, int) or isinstance(source_review_queue_candidate_count, bool):
        ...
        if source_review_queue_candidate_count < candidate_count:
            blockers.append("source_review_queue_candidate_count_less_than_evidence_layer_import_candidate_count")
        if source_review_queue_candidate_count > HARD_CANDIDATE_BOUND:
...
def _candidate_limit_blockers(candidate_limit: Any, source_candidate_count: int) -> list[str]:
    ...
    if source_candidate_count >= 0 and candidate_limit > source_candidate_count:
        blockers.append("blocked_candidate_limit_exceeds_source_evidence_layer_import_candidate_count")
    return blockers
...
def _build_write_candidates(
    ...
    for index, source_candidate in enumerate(source_candidates, start=1):
        candidate_blockers = _source_evidence_layer_import_candidate_blockers(source_candidate)
...
def _source_evidence_layer_import_candidate_blockers(source_candidate: dict[str, Any]) -> list[str]:
    if not isinstance(source_candidate, dict):
        return ["source_evidence_layer_import_candidate_not_object"]
    blockers: list[str] = []
    for field in source_candidate:
        if field in FORBIDDEN_SOURCE_IMPORT_CANDIDATE_FIELDS:
            blockers.append(f"forbidden_source_evidence_layer_import_candidate_field:{field}")
    for field in REQUIRED_SOURCE_IMPORT_CANDIDATE_FIELDS:
        value = source_candidate.get(field)
        if value is None or value == "":
            blockers.append(f"source_evidence_layer_import_candidate_missing_required_field:{field}")
    if source_candidate.get("evidence_layer_import_candidate_schema") != SOURCE_EVIDENCE_LAYER_IMPORT_CANDIDATE_SCHEMA:
        blockers.append("source_evidence_layer_import_candidate_schema_wrong")
    if source_candidate.get("human_review_required") is not True:
        blockers.append("source_evidence_layer_import_candidate_human_review_required_not_true")
    if source_candidate.get("preview_only") is not True:
        blockers.append("source_evidence_layer_import_candidate_preview_only_not_true")
    if source_candidate.get("import_candidate_only") is not True:
        blockers.append("source_evidence_layer_import_candidate_import_candidate_only_not_true")

    boundary_flags = source_candidate.get("boundary_flags")
    if not isinstance(boundary_flags, dict):
        blockers.append("source_evidence_layer_import_candidate_boundary_flags_missing_or_invalid")
    else:
        required_true_flags = {
            "preview_only": "source_evidence_layer_import_candidate_boundary_preview_only_not_true",
            "human_review_required": "source_evidence_layer_import_candidate_boundary_human_review_required_not_true",
            "import_candidate_only": "source_evidence_layer_import_candidate_boundary_import_candidate_only_not_true",
            "not_evidence_item": "source_evidence_layer_import_candidate_boundary_not_evidence_item_not_true",
            "not_production_evidence_item": (
                "source_evidence_layer_import_candidate_boundary_not_production_evidence_item_not_true"
            ),
            "no_evidence_layer_write": (
                "source_evidence_layer_import_candidate_boundary_no_evidence_layer_write_not_true"
            ),
            "not_review_queue_item": "source_evidence_layer_import_candidate_boundary_not_review_queue_item_not_true",
            "not_production_review_queue_item": (
                "source_evidence_layer_import_candidate_boundary_not_production_review_queue_item_not_true"
            ),
            "not_production_case": "source_evidence_layer_import_candidate_boundary_not_production_case_not_true",
            "not_production_analysis_run": (
                "source_evidence_layer_import_candidate_boundary_not_production_analysis_run_not_true"
            ),
        }
    ...
    for field in safe_fields:
        ...
        if _contains_forbidden_value(value):
            blockers.append(f"source_evidence_layer_import_candidate_forbidden_value:{field}")
    return _dedupe(blockers)
...
def _write_candidate_from_source(source_candidate: dict[str, Any], *, index: int) -> dict[str, Any]:
    ...
    source_import_candidate_id = _safe_token(source_candidate.get("evidence_layer_import_candidate_id"))
    ...
    return {
        ...
        "evidence_layer_write_candidate_id": f"evidence-layer-write-candidate-{index:03d}-{evidence_hash}",
        "source_evidence_layer_import_candidate_id": source_import_candidate_id,
        "source_evidence_layer_import_candidate_set_schema": SOURCE_EVIDENCE_LAYER_IMPORT_CANDIDATE_SET_SCHEMA,
        "source_evidence_layer_import_candidate_schema": SOURCE_EVIDENCE_LAYER_IMPORT_CANDIDATE_SCHEMA,
...
def _safe_source_candidates(source_candidate_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_candidate_set, dict):
        return []
    candidates = source_candidate_set.get("evidence_layer_import_candidates")
    if not isinstance(candidates, list):
        return []
    return [candidate for candidate in candidates if isinstance(candidate, dict)]


def _source_evidence_layer_import_candidate_count(
    source_candidate_set: dict[str, Any] | None,
    source_candidates: list[dict[str, Any]],
) -> int:
    if isinstance(source_candidate_set, dict):
        count = source_candidate_set.get("evidence_layer_import_candidate_count")
...
def _blocked_status(blockers: list[str]) -> str:
    if not blockers:
        return "blocked_invalid_source_evidence_layer_import_candidate_set"
    first = blockers[0]
    ...
    if first.startswith("forbidden_source_evidence_layer_import_candidate_field") or first.startswith(
        "source_evidence_layer_import_candidate_forbidden_value"
    ):
        return "blocked_forbidden_field_detected"
    if first.startswith("source_evidence_layer_import_candidate_set") or first.startswith(
        "source_evidence_layer_import_candidate_count"
    ):
        return "blocked_invalid_source_evidence_layer_import_candidate_set"
    if first.startswith("source_evidence_layer_import_candidate") or first == (
        "source_evidence_layer_import_candidate_not_object"
    ):
        return "blocked_invalid_source_evidence_layer_import_candidate_set"
