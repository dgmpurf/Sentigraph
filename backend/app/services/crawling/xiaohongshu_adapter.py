...
XIAOHONGSHU_API_APPROVAL_STATUS = "developer_access_obtained_permission_unverified"
XIAOHONGSHU_DEVELOPER_ACCESS_STATUS = "obtained"
XIAOHONGSHU_COMMENT_API_STATUS = "unknown_or_not_confirmed"
XIAOHONGSHU_REAL_MODE_BLOCKER = "permission_not_verified"
XIAOHONGSHU_MOCK_POST_LIMIT = 100
XIAOHONGSHU_MOCK_COMMENT_LIMIT = 500
...
class XiaohongshuCredentials:
    ...
    @classmethod
    def from_env(cls) -> "XiaohongshuCredentials | None":
        client_id = os.getenv("XIAOHONGSHU_CLIENT_ID", "").strip()
        client_secret = os.getenv("XIAOHONGSHU_CLIENT_SECRET", "").strip()
        access_token = os.getenv("XIAOHONGSHU_ACCESS_TOKEN", "").strip()
...
class XiaohongshuAdapter(BasePlatformAdapter):
    ...
    def __init__(
        ...
        *,
        ...
        credentials: XiaohongshuCredentials | None = None,
    ) -> None:
    ...
    def get_required_credentials(cls) -> tuple[str, ...]:
        return XIAOHONGSHU_REQUIRED_CREDENTIALS

    def health_check(self) -> AdapterHealth:
        if self.requested_mode == "real":
            if self.has_required_credentials():
                message = (
                    "Xiaohongshu official API mode is planned but disabled until note/comment "
                    "API availability and permission are verified and implementation is added."
                )
            else:
                message = "Xiaohongshu official API mode was requested, but credentials are missing; using mock data."
        else:
            message = "Xiaohongshu adapter mock mode is active."
    ...
    def search_posts(
        ...
        safe_limit = self.clamp_limit(limit, default=20, maximum=XIAOHONGSHU_MOCK_POST_LIMIT)
        return [
            self.normalize_post(raw)
            for raw in _mock_xiaohongshu_posts(keyword=keyword, sort=sort, date_range=date_range)[:safe_limit]
        ]

    def fetch_comments(self, post_id: str, limit: int = 100) -> list[RawComment]:
        safe_limit = self.clamp_limit(limit, default=100, maximum=XIAOHONGSHU_MOCK_COMMENT_LIMIT)
        return [self.normalize_comment(raw) for raw in _mock_xiaohongshu_comments(post_id)[:safe_limit]]

    def normalize_post(self, raw: Mapping[str, Any]) -> RawPost:
        payload = _note_payload(raw)
        ...
        post_id = self.safe_text(
            payload.get("note_id") or payload.get("post_id") or payload.get("id"),
            default="xiaohongshu_unknown_note",
        )
        author_name = self.safe_text(
            payload.get("creator_name") or author.get("name") or author.get("nickname") or payload.get("source"),
            default="mock_xiaohongshu_creator",
        )
        ...
        return RawPost(
            ...
            title=self.safe_text(payload.get("title") or payload.get("topic_summary"), default="Xiaohongshu note discussion"),
            content=self.safe_text(
                payload.get("content") or payload.get("description") or payload.get("caption") or payload.get("note_text"),
                default="Mock Xiaohongshu note content.",
            ),
            ...
            url=self.safe_text(payload.get("url") or payload.get("share_url"), default=f"https://www.xiaohongshu.com/explore/{post_id}"),
    ...
    def normalize_comment(self, raw: Mapping[str, Any]) -> RawComment:
        ...
        return RawComment(
            ...
            share_count=0,
            created_at=self.to_utc_iso(payload.get("created_at") or payload.get("create_time")),
            url=self.safe_text(payload.get("url"), default=f"https://www.xiaohongshu.com/explore/{post_id}#comment-{comment_id}"),
            raw_data=self.sanitize_raw_data(payload),
        )


def _mock_xiaohongshu_posts(
    *,
    keyword: str,
    sort: str,
    ...
    return [
        {
            ...
            "note_id": "xiaohongshu_mock_note_001",
            "title": f"{topic} community note",
            "content": "Mock Xiaohongshu note collecting lifestyle-community reactions about product trust, service tone, and user experience.",
            "creator_id": "xiaohongshu_mock_creator_001",
            "creator_name": "Mock Xiaohongshu Observer",
            "created_at": "2026-05-15T08:40:00Z",
            "like_count": 2460,
            "reply_count": 138,
            "share_count": 64,
            "url": "https://www.xiaohongshu.com/explore/xiaohongshu_mock_note_001",
            "sort": sort,
        ...
        },
        {
            "mode": "mock",
            "platform": "xiaohongshu",
            "note_id": "xiaohongshu_mock_note_002",
            "title": f"{topic} experience comparison",
            "content": "Mock note comparing community feedback, complaint themes, and whether the official response feels useful.",
            "creator_id": "xiaohongshu_mock_creator_002",
            "creator_name": "Mock Community Notes",
            "created_at": "2026-05-15T09:25:00Z",
            "like_count": 1740,
            "reply_count": 97,
            "share_count": 43,
            "url": "https://www.xiaohongshu.com/explore/xiaohongshu_mock_note_002",
            "sort": sort,
            "date_range": date_range,
        },
        {
            "mode": "mock",
            "platform": "xiaohongshu",
            "note_id": "xiaohongshu_mock_note_003",
            "title": f"{topic} trust signal summary",
            "content": "Mock Xiaohongshu creator summary of trust signals, repeated phrasing, and high-risk comment concerns.",
            "creator_id": "xiaohongshu_mock_creator_003",
            "creator_name": "Mock Lifestyle Watch",
            "created_at": "2026-05-15T10:05:00Z",
            "like_count": 1320,
            "reply_count": 73,
            "share_count": 31,
            "url": "https://www.xiaohongshu.com/explore/xiaohongshu_mock_note_003",
            "sort": sort,
            "date_range": date_range,
        },
    ]
...
def _mock_xiaohongshu_comments(post_id: str) -> list[dict[str, Any]]:
    target_post_id = post_id or "xiaohongshu_mock_note_001"
    return [
        {
            "mode": "mock",
            "platform": "xiaohongshu",
            ...
            "author_id": "xiaohongshu_mock_commenter_001",
            "author_name": "mock_xiaohongshu_user_a",
            "content": "Mock comment: users want a clearer explanation before they decide whether to keep recommending it.",
            "created_at": "2026-05-15T08:52:00Z",
            "like_count": 204,
            "reply_count": 11,
            "url": f"https://www.xiaohongshu.com/explore/{target_post_id}#comment001",
        },
        {
            "mode": "mock",
            "platform": "xiaohongshu",
            ...
            "author_id": "xiaohongshu_mock_commenter_002",
            "author_name": "mock_xiaohongshu_user_b",
            "content": "Mock comment: similar notes appearing at the same time should be checked for repeated wording.",
            "created_at": "2026-05-15T09:01:00Z",
            "like_count": 129,
            "reply_count": 4,
            "url": f"https://www.xiaohongshu.com/explore/{target_post_id}#comment002",
        },
        {
            "mode": "mock",
            "platform": "xiaohongshu",
            ...
            "author_id": "xiaohongshu_mock_commenter_003",
            "author_name": "mock_xiaohongshu_user_c",
            "content": "Mock comment: a concise official note with next steps would reduce speculation in the comment area.",
            "created_at": "2026-05-15T09:14:00Z",
            "like_count": 88,
            "reply_count": 2,
            "url": f"https://www.xiaohongshu.com/explore/{target_post_id}#comment003",
        },
    ]


def _note_payload(raw: Mapping[str, Any]) -> Mapping[str, Any]:
    data = raw.get("data")
...
def _adapter_mode_from_env() -> AdapterMode:
    return _normalize_adapter_mode(os.getenv("XIAOHONGSHU_ADAPTER_MODE", "mock"))
...
def _credential_presence() -> dict[str, bool]:
    ...
    return {
        ...
        for credential_name in XIAOHONGSHU_REQUIRED_CREDENTIALS
    }
