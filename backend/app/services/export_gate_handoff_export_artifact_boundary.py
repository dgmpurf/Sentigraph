...
INPUT_HANDOFF_SOURCE_KIND = "finalsummaryreport_boundary_adapter"
INPUT_HANDOFF_MODE = "backend_only_local_export_gate_handoff_readiness_smoke"
...
READY_STATUS = "export_artifact_boundary_ready_for_manual_review"
...
BLOCKED_EXPORT_ARTIFACT_RUNTIME_STATUS = "blocked_export_artifact_runtime_side_effect_risk"
...
EXPORT_ARTIFACT_RUNTIME_REQUEST_FIELDS = {
...
def build_export_gate_handoff_export_artifact_boundary(
    export_gate_handoff: dict[str, Any],
    *,
    created_by: str = "sentigraph_internal_operator",
) -> dict[str, Any]:
    safe_handoff = export_gate_handoff if isinstance(export_gate_handoff, dict) else {}
    blockers = _collect_blockers(safe_handoff)
    status = _boundary_status(blockers, safe_handoff)
    return _boundary_object(
        safe_handoff,
        created_by=created_by,
        export_artifact_boundary_status=status,
        export_artifact_boundary_created=not blockers,
        blockers=blockers,
    )


def create_export_gate_handoff_export_artifact_boundary(
    export_gate_handoff: dict[str, Any],
    *,
    created_by: str = "sentigraph_internal_operator",
) -> dict[str, Any]:
    return build_export_gate_handoff_export_artifact_boundary(
        export_gate_handoff,
        created_by=created_by,
    )


def build_safe_export_gate_handoff_export_artifact_boundary_summary(
    boundary: dict[str, Any],
) -> dict[str, Any]:
    safe_boundary = boundary if isinstance(boundary, dict) else {}
    return {
        "schema": "sentigraph_export_gate_handoff_export_artifact_boundary_summary_v0_1",
        "export_artifact_boundary_id": _safe_value(safe_boundary, "export_artifact_boundary_id"),
        "export_artifact_boundary_schema": _safe_value(safe_boundary, "export_artifact_boundary_schema"),
        "export_artifact_boundary_status": _safe_value(safe_boundary, "export_artifact_boundary_status"),
        "export_artifact_boundary_created": bool(safe_boundary.get("export_artifact_boundary_created")),
        "created_local_export_artifact_boundary": bool(safe_boundary.get("created_local_export_artifact_boundary")),
...
def _boundary_object(
    ...
    *,
    created_by: str,
    export_artifact_boundary_status: str,
    export_artifact_boundary_created: bool,
    blockers: list[dict[str, str]],
) -> dict[str, Any]:
    export_gate_handoff_id = _safe_value(handoff, "export_gate_handoff_id")
    return {
        "export_artifact_boundary_id": _boundary_id(export_gate_handoff_id),
        "export_artifact_boundary_schema": EXPORT_ARTIFACT_BOUNDARY_SCHEMA,
        "export_artifact_boundary_status": export_artifact_boundary_status,
        "export_artifact_boundary_created": bool(export_artifact_boundary_created),
        "created_local_export_artifact_boundary": bool(export_artifact_boundary_created),
        "created_at": _utc_now(),
        "created_by": _safe_label(created_by) or "sentigraph_internal_operator",
        "export_gate_handoff_id": export_gate_handoff_id,
        "adapter_id": _safe_value(handoff, "adapter_id"),
        "final_summary_report_id": _safe_value(handoff, "final_summary_report_id"),
        "source11_governance_handoff_id": _safe_value(handoff, "source11_governance_handoff_id"),
        "final_report_boundary_id": _safe_value(handoff, "final_report_boundary_id"),
        "report_candidate_id": _safe_value(handoff, "report_candidate_id"),
        "integration_id": _safe_value(handoff, "integration_id"),
        "execution_id": _safe_value(handoff, "execution_id"),
        "bridge_id": _safe_value(handoff, "bridge_id"),
        "staging_candidate_id": _safe_value(handoff, "staging_candidate_id"),
        "provider_result_id": _safe_value(handoff, "provider_result_id"),
        "request_id": _safe_value(handoff, "request_id"),
        "case_id_hint": _safe_value(handoff, "case_id_hint"),
        "package_name": _safe_package_name(handoff.get("package_name")),
        "input_source_kind": INPUT_SOURCE_KIND,
        "boundary_mode": BOUNDARY_MODE,
        "export_gate_handoff_schema": _safe_value(handoff, "export_gate_handoff_schema")
        or INPUT_EXPORT_GATE_HANDOFF_SCHEMA,
        "export_gate_handoff_status": _safe_value(handoff, "export_gate_handoff_status")
        or BLOCKED_METADATA_STATUS,
        "export_gate_handoff_created": bool(handoff.get("export_gate_handoff_created")),
        "created_local_export_gate_handoff": bool(handoff.get("created_local_export_gate_handoff")),
        "adapter_schema": _safe_value(handoff, "adapter_schema"),
        "adapter_status": _safe_value(handoff, "adapter_status"),
        "final_summary_report_schema": FINAL_SUMMARY_REPORT_SCHEMA if export_artifact_boundary_created else None,
        "final_summary_report_created": bool(export_artifact_boundary_created),
        "final_summary_report_created_local_only": bool(export_artifact_boundary_created),
        "local_final_summary_report_only": bool(export_artifact_boundary_created),
        "source_and_scope": _source_and_scope(),
        "export_gate_handoff_summary": _export_gate_handoff_summary(handoff, enabled=export_artifact_boundary_created),
        "export_artifact_boundary_readiness_summary": _export_artifact_boundary_readiness_summary(
            export_artifact_boundary_created
        ),
        ...
        "human_review_required": True,
        **_output_false_flags(),
        "boundary_flags": _boundary_flags(export_artifact_boundary_created),
        "runtime_side_effects": _runtime_side_effects(export_artifact_boundary_created),
        "audit_refs": _safe_audit_refs(handoff.get("audit_refs")),
        "downstream_policy": _downstream_policy(export_artifact_boundary_created),
    }


def _collect_blockers(value: dict[str, Any]) -> list[dict[str, str]]:
    blockers: list[dict[str, str]] = []
    if _safe_value(value, "export_gate_handoff_id") is None:
        blockers.append(_blocker("missing_export_gate_handoff_id", "metadata_contract"))
    ...
    if _safe_value(value, "input_source_kind") != INPUT_HANDOFF_SOURCE_KIND:
        blockers.append(_blocker("wrong_export_gate_handoff_input_source_kind", "metadata_contract"))
    if _safe_value(value, "handoff_mode") != INPUT_HANDOFF_MODE:
        blockers.append(_blocker("wrong_export_gate_handoff_mode", "metadata_contract"))
    ...
    if not isinstance(value.get("final_summary_report_boundary_summary"), dict):
        blockers.append(_blocker("missing_final_summary_report_boundary_summary", "metadata_contract"))
    if not isinstance(value.get("export_gate_readiness_summary"), dict):
        blockers.append(_blocker("missing_export_gate_readiness_summary", "metadata_contract"))
    ...
    if not isinstance(runtime_side_effects, dict):
        ...
        for key, flag_value in runtime_side_effects.items():
            if key == "created_local_export_gate_handoff" and flag_value is True:
                continue
    ...
    for field in sorted(REQUIRED_FALSE_FIELDS):
        if value.get(field, False) is not False:
            blockers.append(_blocker(f"export_gate_handoff_flag_not_false:{field}", "side_effect"))
    ...
    for field_name in sorted(_find_export_artifact_runtime_requests(value)):
        blockers.append(_blocker(f"requested_export_artifact_runtime_or_file:{field_name}", "export_artifact_runtime"))
...
def _boundary_status(blockers: list[dict[str, str]], value: dict[str, Any]) -> str:
    ...
    if "export_artifact_runtime" in categories:
        return BLOCKED_EXPORT_ARTIFACT_RUNTIME_STATUS
    ...
    if _safe_value(value, "export_gate_handoff_status") == MANUAL_REVIEW_STATUS:
...
def _source_and_scope() -> dict[str, bool | str]:
    return {
        ...
        "scope_note": "selected public sample only; export-artifact boundary marker only",
    }


def _export_gate_handoff_summary(value: dict[str, Any], *, enabled: bool) -> dict[str, Any]:
    return {
        "export_gate_handoff_id": _safe_value(value, "export_gate_handoff_id"),
        "export_gate_handoff_schema": _safe_value(value, "export_gate_handoff_schema"),
        "export_gate_handoff_status": _safe_value(value, "export_gate_handoff_status"),
        "export_gate_handoff_created": bool(value.get("export_gate_handoff_created") and enabled),
        "created_local_export_gate_handoff": bool(value.get("created_local_export_gate_handoff") and enabled),
...
def _export_artifact_boundary_readiness_summary(enabled: bool) -> dict[str, Any]:
    return {
        "eligible_for_later_manual_export_artifact_runtime_review": bool(enabled),
...
def _warnings(handoff: dict[str, Any]) -> list[str]:
    warnings = _safe_string_list(handoff.get("warnings"))
    required = [
        "selected_sample_only",
        "not_full_web",
        "not_full_platform",
        ...
        "markdown_pdf_deck_appendix_not_generated",
...
def _boundary_flags(created: bool) -> dict[str, bool]:
    return {
        ...
        "export_artifact_boundary_only": True,
        "export_artifact_runtime_not_used": True,
        "export_artifact_record_not_created": True,
        "markdown_file_not_generated": True,
        "pdf_file_not_generated": True,
        "briefing_deck_not_generated": True,
        "evidence_appendix_package_not_generated": True,
...
def _runtime_side_effects(created: bool) -> dict[str, bool]:
    return {
        ...
        "auto_executed": False,
        "created_local_export_artifact_boundary": bool(created),
    }


def _downstream_policy(created: bool) -> dict[str, Any]:
    return {
        "export_artifact_boundary_ready_for_manual_review": bool(created),
        "export_artifact_runtime_ready": False,
        "final_summary_report_export_artifact_ready": False,
        ...
        "final_summary_report_export_artifact_creation_requires_separate_decision": True,
        "markdown_pdf_deck_appendix_generation_requires_separate_decision": True,
...
def _safe_blockers(value: Any) -> list[dict[str, str]]:
    ...
    for item in value:
        if isinstance(item, dict):
            ...
            category = _safe_value(item, "category") or "export_artifact_boundary_blocker"
        else:
            reason = str(item)
            category = "export_artifact_boundary_blocker"
...
def _find_export_artifact_runtime_requests(value: Any, path: str = "") -> set[str]:
    found: set[str] = set()
    if isinstance(value, dict):
        for key, nested_value in value.items():
            lowered = str(key).lower()
            child_path = f"{path}.{lowered}" if path else lowered
            if (
                lowered in EXPORT_ARTIFACT_RUNTIME_REQUEST_FIELDS
                and _truthy(nested_value)
                and not _is_allowed_false_flag(child_path, lowered, nested_value)
            ):
                found.add(str(key))
            found.update(_find_export_artifact_runtime_requests(nested_value, child_path))
    elif isinstance(value, list):
        for item in value:
            found.update(_find_export_artifact_runtime_requests(item, path))
    return found
...
def _is_allowed_false_flag(path: str, normalized_field: str, value: Any) -> bool:
    ...
    return (
        ...
        or path.endswith(f"export_gate_handoff_summary.{normalized_field}")
        or path.endswith(f"export_artifact_boundary_readiness_summary.{normalized_field}")
    )
...
def _boundary_id(export_gate_handoff_id: str | None) -> str:
    return f"export_gate_handoff_export_artifact_boundary_{_safe_identifier(export_gate_handoff_id)}"
