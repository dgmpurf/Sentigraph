...
INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_SCHEMA = (
    "sentigraph_export_artifact_boundary_download_public_access_boundary_v0_1"
)
INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_STATUS = "download_public_access_boundary_ready_for_manual_review"
INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_SOURCE_KIND = "export_artifact_boundary"
INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_MODE = "backend_only_local_download_public_access_boundary_readiness_smoke"
...
BOUNDARY_MODE = "backend_only_local_final_delivery_boundary_readiness_smoke"

READY_STATUS = "final_delivery_boundary_ready_for_manual_review"
...
BLOCKED_FINAL_DELIVERY_RUNTIME_STATUS = "blocked_final_delivery_runtime_side_effect_risk"
...
FINAL_DELIVERY_RUNTIME_REQUEST_FIELDS = {
...
def build_download_public_access_boundary_final_delivery_boundary(
    download_public_access_boundary: dict[str, Any],
    *,
    created_by: str = "sentigraph_internal_operator",
) -> dict[str, Any]:
    safe_boundary = download_public_access_boundary if isinstance(download_public_access_boundary, dict) else {}
    blockers = _collect_blockers(safe_boundary)
    status = _boundary_status(blockers, safe_boundary)
    return _boundary_object(
        safe_boundary,
        created_by=created_by,
        final_delivery_boundary_status=status,
        final_delivery_boundary_created=not blockers,
        blockers=blockers,
    )


def create_download_public_access_boundary_final_delivery_boundary(
    download_public_access_boundary: dict[str, Any],
    *,
    created_by: str = "sentigraph_internal_operator",
) -> dict[str, Any]:
    return build_download_public_access_boundary_final_delivery_boundary(
        download_public_access_boundary,
        created_by=created_by,
    )


def build_safe_download_public_access_boundary_final_delivery_boundary_summary(
    boundary: dict[str, Any],
) -> dict[str, Any]:
    safe_boundary = boundary if isinstance(boundary, dict) else {}
    return {
        "schema": "sentigraph_download_public_access_boundary_final_delivery_boundary_summary_v0_1",
        "final_delivery_boundary_id": _safe_value(safe_boundary, "final_delivery_boundary_id"),
        "final_delivery_boundary_schema": _safe_value(safe_boundary, "final_delivery_boundary_schema"),
        "final_delivery_boundary_status": _safe_value(safe_boundary, "final_delivery_boundary_status"),
        "final_delivery_boundary_created": bool(safe_boundary.get("final_delivery_boundary_created")),
        "created_local_final_delivery_boundary": bool(safe_boundary.get("created_local_final_delivery_boundary")),
...
def _boundary_object(
    ...
    *,
    created_by: str,
    final_delivery_boundary_status: str,
    final_delivery_boundary_created: bool,
    blockers: list[dict[str, str]],
) -> dict[str, Any]:
    download_public_access_boundary_id = _safe_value(
        download_public_access_boundary, "download_public_access_boundary_id"
    )
    return {
        "final_delivery_boundary_id": _boundary_id(download_public_access_boundary_id),
        "final_delivery_boundary_schema": FINAL_DELIVERY_BOUNDARY_SCHEMA,
        "final_delivery_boundary_status": final_delivery_boundary_status,
        "final_delivery_boundary_created": bool(final_delivery_boundary_created),
        "created_local_final_delivery_boundary": bool(final_delivery_boundary_created),
        "created_at": _utc_now(),
        "created_by": _safe_label(created_by) or "sentigraph_internal_operator",
        "download_public_access_boundary_id": download_public_access_boundary_id,
        "export_artifact_boundary_id": _safe_value(download_public_access_boundary, "export_artifact_boundary_id"),
        "export_gate_handoff_id": _safe_value(download_public_access_boundary, "export_gate_handoff_id"),
        "adapter_id": _safe_value(download_public_access_boundary, "adapter_id"),
        "final_summary_report_id": _safe_value(download_public_access_boundary, "final_summary_report_id"),
        "source11_governance_handoff_id": _safe_value(
            download_public_access_boundary, "source11_governance_handoff_id"
        ),
        "final_report_boundary_id": _safe_value(download_public_access_boundary, "final_report_boundary_id"),
        "report_candidate_id": _safe_value(download_public_access_boundary, "report_candidate_id"),
        "integration_id": _safe_value(download_public_access_boundary, "integration_id"),
        "execution_id": _safe_value(download_public_access_boundary, "execution_id"),
        "bridge_id": _safe_value(download_public_access_boundary, "bridge_id"),
        "staging_candidate_id": _safe_value(download_public_access_boundary, "staging_candidate_id"),
        "provider_result_id": _safe_value(download_public_access_boundary, "provider_result_id"),
        "request_id": _safe_value(download_public_access_boundary, "request_id"),
        "case_id_hint": _safe_value(download_public_access_boundary, "case_id_hint"),
        "package_name": _safe_package_name(download_public_access_boundary.get("package_name")),
        "input_source_kind": INPUT_SOURCE_KIND,
        "boundary_mode": BOUNDARY_MODE,
        "download_public_access_boundary_schema": _safe_value(
            download_public_access_boundary, "download_public_access_boundary_schema"
        )
        or INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_SCHEMA,
        "download_public_access_boundary_status": _safe_value(
            download_public_access_boundary, "download_public_access_boundary_status"
        )
        or BLOCKED_METADATA_STATUS,
        "download_public_access_boundary_created": bool(
            download_public_access_boundary.get("download_public_access_boundary_created")
        ),
        "created_local_download_public_access_boundary": bool(
            download_public_access_boundary.get("created_local_download_public_access_boundary")
        ),
        "export_artifact_boundary_schema": _safe_value(
            download_public_access_boundary, "export_artifact_boundary_schema"
        ),
        "export_artifact_boundary_status": _safe_value(
            download_public_access_boundary, "export_artifact_boundary_status"
        ),
        "export_artifact_boundary_created": bool(download_public_access_boundary.get("export_artifact_boundary_created")),
        "created_local_export_artifact_boundary": bool(
            download_public_access_boundary.get("created_local_export_artifact_boundary")
        ),
        "export_gate_handoff_schema": _safe_value(download_public_access_boundary, "export_gate_handoff_schema"),
        "export_gate_handoff_status": _safe_value(download_public_access_boundary, "export_gate_handoff_status"),
        "export_gate_handoff_created": bool(download_public_access_boundary.get("export_gate_handoff_created")),
        "created_local_export_gate_handoff": bool(
            download_public_access_boundary.get("created_local_export_gate_handoff")
        ),
        "final_summary_report_created": bool(final_delivery_boundary_created),
        "final_summary_report_created_local_only": bool(final_delivery_boundary_created),
        "local_final_summary_report_only": bool(final_delivery_boundary_created),
        "source_and_scope": _source_and_scope(),
        "download_public_access_boundary_summary": _download_public_access_boundary_summary(
            download_public_access_boundary,
            enabled=final_delivery_boundary_created,
        ),
        "final_delivery_boundary_readiness_summary": _final_delivery_boundary_readiness_summary(
            final_delivery_boundary_created
        ),
        "boundary_block": _boundary_block(),
        "coverage_limitations": _coverage_limitations(),
        "warnings": _warnings(download_public_access_boundary),
        "blockers": _safe_blockers(blockers),
        "human_review_status": "required",
        "human_review_required": True,
        **_output_false_flags(),
        "boundary_flags": _boundary_flags(final_delivery_boundary_created),
        "runtime_side_effects": _runtime_side_effects(final_delivery_boundary_created),
        "audit_refs": _safe_audit_refs(download_public_access_boundary.get("audit_refs")),
        "downstream_policy": _downstream_policy(final_delivery_boundary_created),
    }


def _collect_blockers(value: dict[str, Any]) -> list[dict[str, str]]:
    blockers: list[dict[str, str]] = []
    if _safe_value(value, "download_public_access_boundary_id") is None:
        blockers.append(_blocker("missing_download_public_access_boundary_id", "metadata_contract"))
    if _safe_value(value, "download_public_access_boundary_schema") != INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_SCHEMA:
        blockers.append(_blocker("wrong_download_public_access_boundary_schema", "metadata_contract"))
    if _safe_value(value, "download_public_access_boundary_status") != INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_STATUS:
        blockers.append(_blocker("download_public_access_boundary_status_not_ready", "metadata_contract"))
    if value.get("download_public_access_boundary_created") is not True:
        blockers.append(_blocker("download_public_access_boundary_not_created", "metadata_contract"))
    if value.get("created_local_download_public_access_boundary") is not True:
        blockers.append(_blocker("created_local_download_public_access_boundary_missing", "metadata_contract"))
    if _safe_value(value, "input_source_kind") != INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_SOURCE_KIND:
        blockers.append(_blocker("wrong_download_public_access_boundary_input_source_kind", "metadata_contract"))
    if _safe_value(value, "boundary_mode") != INPUT_DOWNLOAD_PUBLIC_ACCESS_BOUNDARY_MODE:
        blockers.append(_blocker("wrong_download_public_access_boundary_mode", "metadata_contract"))
    ...
    if not isinstance(value.get("export_artifact_boundary_summary"), dict):
        blockers.append(_blocker("missing_export_artifact_boundary_summary", "metadata_contract"))
    if not isinstance(value.get("download_public_access_boundary_readiness_summary"), dict):
        blockers.append(_blocker("missing_download_public_access_boundary_readiness_summary", "metadata_contract"))
    ...
    if not isinstance(runtime_side_effects, dict):
        ...
        for key, flag_value in runtime_side_effects.items():
            if key == "created_local_download_public_access_boundary" and flag_value is True:
                continue
    ...
    for field in sorted(REQUIRED_FALSE_FIELDS):
        if value.get(field, False) is not False:
            blockers.append(_blocker(f"download_public_access_boundary_flag_not_false:{field}", "side_effect"))
    ...
    for field_name in sorted(_find_final_delivery_runtime_requests(value)):
        blockers.append(_blocker(f"requested_final_delivery_runtime_or_delivery:{field_name}", "final_delivery_runtime"))
...
def _boundary_status(blockers: list[dict[str, str]], value: dict[str, Any]) -> str:
    ...
    if "final_delivery_runtime" in categories:
        return BLOCKED_FINAL_DELIVERY_RUNTIME_STATUS
    ...
    if _safe_value(value, "download_public_access_boundary_status") == MANUAL_REVIEW_STATUS:
...
def _source_and_scope() -> dict[str, bool | str]:
    return {
        ...
        "scope_note": "selected public sample only; final-delivery boundary marker only",
    }


def _download_public_access_boundary_summary(value: dict[str, Any], *, enabled: bool) -> dict[str, Any]:
    return {
        "download_public_access_boundary_id": _safe_value(value, "download_public_access_boundary_id"),
        "download_public_access_boundary_schema": _safe_value(value, "download_public_access_boundary_schema"),
        "download_public_access_boundary_status": _safe_value(value, "download_public_access_boundary_status"),
        "download_public_access_boundary_created": bool(
            value.get("download_public_access_boundary_created") and enabled
        ),
        "created_local_download_public_access_boundary": bool(
            value.get("created_local_download_public_access_boundary") and enabled
        ),
        ...
        "human_review_required": True,
        "final_delivery_boundary_only": True,
        "final_delivery_runtime_used": False,
        "download_package_runtime_used": False,
        "public_access_runtime_used": False,
        "external_delivery_runtime_used": False,
...
def _final_delivery_boundary_readiness_summary(enabled: bool) -> dict[str, Any]:
    return {
        "eligible_for_later_manual_final_delivery_runtime_review": bool(enabled),
...
def _boundary_flags(created: bool) -> dict[str, bool]:
    return {
        ...
        "final_delivery_runtime_not_used": True,
        ...
        "final_delivery_not_performed": True,
        "customer_delivery_not_created": True,
        "object_storage_not_uploaded": True,
        "email_not_sent": True,
        "portal_not_published": True,
...
def _runtime_side_effects(created: bool) -> dict[str, bool]:
    return {
        ...
        "performed_final_delivery": False,
        "created_customer_delivery": False,
        ...
        "auto_executed": False,
        "created_local_final_delivery_boundary": bool(created),
    }


def _downstream_policy(created: bool) -> dict[str, Any]:
    return {
        "final_delivery_boundary_ready_for_manual_review": bool(created),
        "final_delivery_runtime_ready": False,
        ...
        "route_ready": False,
        "production_ready": False,
        "final_delivery_runtime_requires_separate_decision": True,
        "customer_delivery_requires_separate_decision": True,
        ...
        "object_storage_upload_requires_separate_decision": True,
        "email_sending_requires_separate_decision": True,
        "portal_publication_requires_separate_decision": True,
...
def _safe_blockers(value: Any) -> list[dict[str, str]]:
    ...
    for item in value:
        if isinstance(item, dict):
            ...
            category = _safe_value(item, "category") or "final_delivery_boundary_blocker"
        else:
            reason = str(item)
            category = "final_delivery_boundary_blocker"
...
def _find_final_delivery_runtime_requests(value: Any, path: str = "") -> set[str]:
    found: set[str] = set()
    if isinstance(value, dict):
        for key, nested_value in value.items():
            lowered = str(key).lower()
            child_path = f"{path}.{lowered}" if path else lowered
            if (
                lowered in FINAL_DELIVERY_RUNTIME_REQUEST_FIELDS
                and _truthy(nested_value)
                and not _is_allowed_false_flag(child_path, lowered, nested_value)
            ):
                found.add(str(key))
            found.update(_find_final_delivery_runtime_requests(nested_value, child_path))
    elif isinstance(value, list):
        for item in value:
            found.update(_find_final_delivery_runtime_requests(item, path))
    return found
...
def _is_allowed_false_flag(path: str, normalized_field: str, value: Any) -> bool:
    ...
    return (
        ...
        or path.endswith(f"download_public_access_boundary_summary.{normalized_field}")
        or path.endswith(f"final_delivery_boundary_readiness_summary.{normalized_field}")
    )
...
def _boundary_id(download_public_access_boundary_id: str | None) -> str:
    return f"download_public_access_boundary_final_delivery_boundary_{_safe_identifier(download_public_access_boundary_id)}"
