...
SOURCE_RUNTIME_BOUNDARY_SET_SCHEMA = "sentigraph_controlled_production_analysis_result_runtime_boundary_set_v0_1"
SOURCE_RUNTIME_BOUNDARY_SCHEMA = "sentigraph_controlled_production_analysis_result_runtime_boundary_v0_1"
PHASE = "8W-52"
APPROVAL_PHRASE = (
    "APPROVE_8W_52_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CREATION_OR_RUNTIME_EXECUTION_CANDIDATE_HELPER_IMPLEMENTATION"
)
SOURCE_WARN_STATUS = "production_analysis_result_runtime_boundary_set_warn_manual_review_required"
WARN_STATUS = (
    "production_analysis_result_creation_or_runtime_execution_candidate_set_warn_manual_review_required"
)
EXPECTED_NEXT_BOUNDARY_OPTION = (
...
SOURCE_TRUE_FIELDS = {
    ...
    "production_analysis_result_runtime_boundary_created": (
        "source_production_analysis_result_runtime_boundary_created_not_true"
    ),
}
...
def build_controlled_production_analysis_result_creation_or_runtime_execution_candidate_set(
    controlled_production_analysis_result_runtime_boundary_set: dict[str, Any] | None,
    *,
    ...
    if candidate_count != 1:
        blockers.append("candidate_count_not_one")

    source_boundaries: list[dict[str, Any]] = []
    if not blockers:
        source_boundaries = _safe_source_runtime_boundaries(
            controlled_production_analysis_result_runtime_boundary_set
        )
        blockers.extend(
    ...
    if not blockers:
        blockers.extend(_source_runtime_boundary_blockers(source_boundaries))
        if not blockers:
            candidates.append(_candidate_from_source(source_boundaries[0]))

    status = WARN_STATUS if not blockers else _blocked_status(blockers)
    return _base_output(
        source_boundary_set=controlled_production_analysis_result_runtime_boundary_set,
        status=status,
...
create_controlled_production_analysis_result_creation_or_runtime_execution_candidate_set = (
    build_controlled_production_analysis_result_creation_or_runtime_execution_candidate_set
)


def build_safe_controlled_production_analysis_result_creation_or_runtime_execution_candidate_summary(
    controlled_production_analysis_result_runtime_boundary_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
) -> dict[str, Any]:
    candidate_set = build_controlled_production_analysis_result_creation_or_runtime_execution_candidate_set(
        controlled_production_analysis_result_runtime_boundary_set,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_creation_or_runtime_execution_candidate_set_schema": candidate_set[
            "production_analysis_result_creation_or_runtime_execution_candidate_set_schema"
        ],
        "production_analysis_result_creation_or_runtime_execution_candidate_set_status": candidate_set[
            "production_analysis_result_creation_or_runtime_execution_candidate_set_status"
        ],
        "input_source_kind": candidate_set["input_source_kind"],
        "source_production_analysis_result_runtime_boundary_set_schema": candidate_set[
            "source_production_analysis_result_runtime_boundary_set_schema"
        ],
        "source_production_analysis_result_runtime_boundary_schema": candidate_set[
            "source_production_analysis_result_runtime_boundary_schema"
        ],
        ...
        "candidate_mode": candidate_set["candidate_mode"],
        "production_analysis_result_creation_or_runtime_execution_candidate_count": candidate_set[
            "production_analysis_result_creation_or_runtime_execution_candidate_count"
        ],
        ...
        "production_analysis_result_runtime_boundary_created_upstream": candidate_set[
            "production_analysis_result_runtime_boundary_created_upstream"
        ],
        "production_analysis_result_creation_or_runtime_execution_candidate_created": candidate_set[
...
def _base_output(
    *,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_result_creation_or_runtime_execution_candidate_set_status": status,
        "input_source_kind": "controlled_production_analysis_result_runtime_boundary",
        "source_production_analysis_result_runtime_boundary_set_schema": _safe_source_value(
            source_boundary_set,
            "production_analysis_result_runtime_boundary_set_schema",
        )
        or SOURCE_RUNTIME_BOUNDARY_SET_SCHEMA,
        "source_production_analysis_result_runtime_boundary_schema": _source_runtime_boundary_schema(
            source_boundary_set
        ),
        ...
        "candidate_mode": (
            "backend_only_local_production_analysis_result_creation_or_runtime_execution_candidate"
        ),
        "production_analysis_result_creation_or_runtime_execution_candidate_count": len(candidates),
        ...
        "production_analysis_result_runtime_boundary_created_upstream": _truthy(
            source_boundary_set.get("production_analysis_result_runtime_boundary_created")
            if isinstance(source_boundary_set, dict)
            else None
        ),
        "production_analysis_result_creation_or_runtime_execution_candidate_created": candidate_created,
        ...
        "production_analysis_result_creation_or_runtime_execution_candidates": candidates,
        ...
        "audit_summary": {
            "audit_schema": (
                "sentigraph_controlled_production_analysis_result_creation_or_runtime_execution_candidate_audit_summary_v0_1"
            ),
            "phase": PHASE,
...
def _candidate_from_source(source_boundary: dict[str, Any]) -> dict[str, Any]:
    source_ids = _source_runtime_boundary_ids(source_boundary)
    return {
        "production_analysis_result_creation_or_runtime_execution_candidate_schema": CANDIDATE_SCHEMA,
        "production_analysis_result_creation_or_runtime_execution_candidate_id": _candidate_id(source_ids),
        "source_production_analysis_result_runtime_boundary_id": source_ids[0] if source_ids else None,
        "source_production_analysis_result_runtime_boundary_ids": source_ids,
        ...
        "intended_candidate_boundary_labels": [
            "candidate_only",
        ...
        "intended_module_scope_labels": _dedupe(
            _safe_list(source_boundary.get("intended_module_scope_labels"))
            + [
                "production_analysis_result_creation_or_runtime_execution_candidate",
        ...
        "creation_or_runtime_execution_gate_labels": [
            "candidate_gate_only",
        ...
        "redaction_warnings": _dedupe(
            _safe_list(source_boundary.get("redaction_warnings")) + ["manual_review_required"]
        ),
        "warning_labels": _dedupe(
            _safe_list(source_boundary.get("warning_labels")) + ["manual_review_required", "selected_sample_only"]
        ),
        ...
        "production_analysis_result_runtime_execution_readiness_blockers": [
            "production_analysis_result_runtime_execution_not_approved",
            "human_review_required",
        ],
        ...
        "review_queue_runtime_used": False,
        "report_ready": False,
        "boundary_flags": _boundary_flags(candidate_only=True),
    }
...
def _source_boundary_set_blockers(
    ...
    if not isinstance(source_boundary_set, dict):
        return ["source_production_analysis_result_runtime_boundary_set_missing"]
    if source_boundary_set.get("production_analysis_result_runtime_boundary_set_schema") != (
        SOURCE_RUNTIME_BOUNDARY_SET_SCHEMA
    ):
        blockers.append("source_runtime_boundary_set_schema_wrong")
    if source_boundary_set.get("production_analysis_result_runtime_boundary_set_status") != SOURCE_WARN_STATUS:
        blockers.append("source_runtime_boundary_set_status_wrong")
    if _source_runtime_boundary_schema(source_boundary_set) != SOURCE_RUNTIME_BOUNDARY_SCHEMA:
        blockers.append("source_runtime_boundary_schema_wrong")
    ...
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if source_boundary_set.get(field) is not False:
            blockers.append(reason)
    if source_boundary_set.get("8w50_decision") != "ready":
        blockers.append("source_8w50_decision_not_ready")
    if source_boundary_set.get("8w51_decision") != "ready":
        blockers.append("source_8w51_decision_not_ready")
    if source_boundary_set.get("8w51_selected_next_boundary_option") != EXPECTED_NEXT_BOUNDARY_OPTION:
        blockers.append("source_8w51_selected_next_boundary_option_wrong")
    ...
    for field, value in source_boundary_set.items():
        if field == "production_analysis_result_runtime_boundaries":
            continue
    ...
    if len(source_boundaries) != 1:
        blockers.append("source_production_analysis_result_runtime_boundaries_count_not_one")
    return _dedupe(blockers)


def _source_runtime_boundary_blockers(source_boundaries: list[dict[str, Any]]) -> list[str]:
    blockers: list[str] = []
    if len(source_boundaries) != 1:
        return ["source_production_analysis_result_runtime_boundaries_count_not_one"]
    boundary = source_boundaries[0]
    if not isinstance(boundary, dict):
        return ["source_production_analysis_result_runtime_boundary_not_object"]
    for field, value in boundary.items():
        if field in FORBIDDEN_SOURCE_FIELDS:
            blockers.append(f"forbidden_source_runtime_boundary_field:{field}")
        elif _contains_forbidden_value(value):
            blockers.append(f"source_runtime_boundary_forbidden_value:{field}")
    if boundary.get("production_analysis_result_runtime_boundary_schema") != SOURCE_RUNTIME_BOUNDARY_SCHEMA:
        blockers.append("source_production_analysis_result_runtime_boundary_schema_wrong")
    if boundary.get("source_production_analysis_result_runtime_boundary_count") not in (None, 1):
        blockers.append("source_runtime_boundary_source_runtime_boundary_count_not_one")
    if boundary.get("source_production_analysis_result_boundary_count") != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_COUNT
    ):
        blockers.append("source_runtime_boundary_production_analysis_result_boundary_count_not_one")
    if boundary.get("source_production_analysis_result_candidate_count") != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RESULT_CANDIDATE_COUNT
    ):
        blockers.append("source_runtime_boundary_production_analysis_result_candidate_count_not_one")
    if boundary.get("source_analysis_result_candidate_count") != EXPECTED_SOURCE_ANALYSIS_RESULT_CANDIDATE_COUNT:
        blockers.append("source_runtime_boundary_analysis_result_candidate_count_not_one")
    if boundary.get("source_actual_analysis_execution_candidate_count") != (
        EXPECTED_SOURCE_ACTUAL_ANALYSIS_EXECUTION_CANDIDATE_COUNT
    ):
        blockers.append("source_runtime_boundary_actual_analysis_execution_candidate_count_not_one")
    if boundary.get("source_production_analysis_run_candidate_count") != (
        EXPECTED_SOURCE_PRODUCTION_ANALYSIS_RUN_CANDIDATE_COUNT
    ):
        blockers.append("source_runtime_boundary_production_analysis_run_candidate_count_not_one")
    if boundary.get("source_production_case_candidate_count") != EXPECTED_SOURCE_PRODUCTION_CASE_CANDIDATE_COUNT:
        blockers.append("source_runtime_boundary_production_case_candidate_count_not_one")
    if boundary.get("source_controlled_evidence_item_count") != EXPECTED_SOURCE_CONTROLLED_EVIDENCE_ITEM_COUNT:
        blockers.append("source_runtime_boundary_controlled_evidence_item_count_not_five")
    if boundary.get("warning_count") != 1:
        blockers.append("source_runtime_boundary_warning_count_not_one")
    if boundary.get("human_review_required") is not True:
        blockers.append("source_runtime_boundary_human_review_required_not_true")
    if boundary.get("no_automatic_trust_upgrade") is not True:
        blockers.append("source_runtime_boundary_no_automatic_trust_upgrade_not_true")
    for field, reason in SOURCE_FALSE_FIELDS.items():
        if field in boundary and boundary.get(field) is not False:
            blockers.append(f"source_runtime_boundary_{reason.removeprefix('source_')}")
    return _dedupe(blockers)
...
def _blocked_status(blockers: list[str]) -> str:
    ...
    if first.startswith("blocked_"):
        return first
    if first == "candidate_count_not_one":
        return "blocked_candidate_boundary_limit_violation"
    ...
    if ("runtime_boundary" in first or "runtime_boundaries" in first) and (
        "schema" in first or "status" in first or "count" in first
    ):
        return "blocked_invalid_source_production_analysis_result_runtime_boundary"
...
def _safe_source_runtime_boundaries(source_boundary_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_boundary_set, dict):
        return []
    boundaries = source_boundary_set.get("production_analysis_result_runtime_boundaries")
    if not isinstance(boundaries, list):
        return []
    return [boundary for boundary in boundaries if isinstance(boundary, dict)]


def _source_runtime_boundary_schema(source_boundary_set: dict[str, Any] | None) -> str | None:
    if not isinstance(source_boundary_set, dict):
        return None
    value = source_boundary_set.get("source_production_analysis_result_runtime_boundary_schema")
    if isinstance(value, str):
        return value
    boundaries = _safe_source_runtime_boundaries(source_boundary_set)
    if boundaries:
        boundary_schema = boundaries[0].get("production_analysis_result_runtime_boundary_schema")
...
def _source_runtime_boundary_count(source_boundary_set: dict[str, Any] | None) -> int:
    if not isinstance(source_boundary_set, dict):
        return 0
    return _safe_int(
        source_boundary_set.get("source_production_analysis_result_runtime_boundary_count"),
        source_boundary_set.get("production_analysis_result_runtime_boundary_count"),
        len(_safe_source_runtime_boundaries(source_boundary_set)),
    )
...
def _source_runtime_boundary_ids(source_boundary: dict[str, Any]) -> list[str]:
    ids = source_boundary.get("source_production_analysis_result_runtime_boundary_ids")
    if isinstance(ids, list):
        safe = [_safe_label(item) for item in ids]
        return [item for item in safe if item]
    single = _safe_label(source_boundary.get("production_analysis_result_runtime_boundary_id"))
    return [single] if single else []


def _candidate_id(source_ids: list[str]) -> str:
    suffix = "-".join(source_ids) if source_ids else "safe-summary"
    return f"controlled-production-analysis-result-creation-runtime-execution-candidate-{_slug(suffix)}"
...
def _boundary_flags(*, candidate_only: bool = False) -> dict[str, bool]:
    return {
        "backend_only": True,
        "local_only": True,
        "candidate_only": candidate_only,
        "controlled_production_analysis_result_runtime_boundary_derived_only": True,
        "human_review_required": True,
