...
SUMMARY_SCHEMA = "sentigraph_controlled_production_evidence_import_candidate_summary_v0_1"
SOURCE_EVIDENCE_LAYER_WRITE_CANDIDATE_SET_SCHEMA = (
    "sentigraph_controlled_evidence_layer_write_candidate_set_v0_1"
)
SOURCE_EVIDENCE_LAYER_WRITE_CANDIDATE_SCHEMA = "sentigraph_controlled_evidence_layer_write_candidate_v0_1"
SOURCE_PHASE = "8W-19"
PHASE = "8W-22"
APPROVAL_PHRASE = "批准 8W-22 Controlled Production Evidence Import Candidate Helper Implementation"
SOURCE_WARN_STATUS = "evidence_layer_write_candidate_set_warn_manual_review_required"
READY_STATUS = "production_evidence_import_candidate_set_ready_for_manual_review"
WARN_STATUS = "production_evidence_import_candidate_set_warn_manual_review_required"
...
REQUIRED_SOURCE_WRITE_CANDIDATE_FIELDS = [
...
OPTIONAL_SAFE_SOURCE_WRITE_CANDIDATE_FIELDS = [
    "preview_hash",
    "platform",
    "evidence_type",
    "coarse_created_at",
...
FORBIDDEN_SOURCE_WRITE_CANDIDATE_FIELDS = {
    "evidence_item_id",
...
def build_controlled_production_evidence_import_candidate_set(
    ...
    *,
    ...
    """Create local, redacted, human-review-only production-import-candidate-shaped objects."""
    ...
    source_candidate_count = _source_evidence_layer_write_candidate_count(
    ...
    blockers.extend(
        _source_evidence_layer_write_candidate_set_blockers(
            controlled_evidence_layer_write_candidate_set,
            source_candidates,
        )
    )
    ...
    status = WARN_STATUS if _source_warning_count(controlled_evidence_layer_write_candidate_set) else READY_STATUS
    return _base_output(
        status=status,
        created=True,
        blockers=[],
        warnings=_import_candidate_warnings(controlled_evidence_layer_write_candidate_set),
...
def build_safe_controlled_production_evidence_import_candidate_summary(
    controlled_evidence_layer_write_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    candidate_limit: int | None = None,
    ...
    candidate_set = build_controlled_production_evidence_import_candidate_set(
        controlled_evidence_layer_write_candidate_set,
    ...
    return {
        ...
        "phase": PHASE,
        "production_evidence_import_candidate_set_schema": PRODUCTION_EVIDENCE_IMPORT_CANDIDATE_SET_SCHEMA,
        "production_evidence_import_candidate_set_status": candidate_set[
            "production_evidence_import_candidate_set_status"
        ],
        "production_evidence_import_candidate_count": candidate_set["production_evidence_import_candidate_count"],
        "source_evidence_layer_write_candidate_count": candidate_set[
...
def _base_output(
    *,
    ...
    return {
        "production_evidence_import_candidate_set_schema": PRODUCTION_EVIDENCE_IMPORT_CANDIDATE_SET_SCHEMA,
        "phase": PHASE,
        "production_evidence_import_candidate_set_status": status,
        "created_at": _utc_now(),
        "input_source_kind": "controlled_evidence_layer_write_candidate_set",
        "source_evidence_layer_write_candidate_set_schema": _safe_source_value(
            source_candidate_set,
            "evidence_layer_write_candidate_set_schema",
        ),
        "source_evidence_layer_write_candidate_set_status": _safe_source_value(
            source_candidate_set,
            "evidence_layer_write_candidate_set_status",
        ),
        "source_evidence_layer_write_candidate_count": source_candidate_count,
        "production_evidence_import_candidate_mode": PRODUCTION_EVIDENCE_IMPORT_CANDIDATE_MODE,
        "production_evidence_import_candidate_count": len(import_candidates),
        ...
        "production_evidence_import_candidate_helper_implementation_approved": created,
        "production_evidence_import_candidate_created": created,
        "evidence_item_created": False,
        "evidence_items_created": False,
        "production_evidence_item_created": False,
        "evidence_layer_write": False,
        ...
        "production_evidence_import_candidates": import_candidates,
...
def _source_evidence_layer_write_candidate_set_blockers(
    source_candidate_set: dict[str, Any] | None,
    ...
    true_fields = {
        ...
        "evidence_layer_write_candidate_created": "source_evidence_layer_write_candidate_created_not_true",
    }
    ...
    source_import_candidate_count = source_candidate_set.get("source_evidence_layer_import_candidate_count")
    ...
    if not isinstance(source_import_candidate_count, int) or isinstance(source_import_candidate_count, bool):
        ...
        if source_import_candidate_count < candidate_count:
            blockers.append("source_evidence_layer_import_candidate_count_less_than_evidence_layer_write_candidate_count")
        if source_import_candidate_count > HARD_CANDIDATE_BOUND:
...
def _candidate_limit_blockers(candidate_limit: Any, source_candidate_count: int) -> list[str]:
    ...
    if source_candidate_count >= 0 and candidate_limit > source_candidate_count:
        blockers.append("blocked_candidate_limit_exceeds_source_evidence_layer_write_candidate_count")
    return blockers
...
def _build_import_candidates(
    ...
    for index, source_candidate in enumerate(source_candidates, start=1):
        candidate_blockers = _source_evidence_layer_write_candidate_blockers(source_candidate)
...
def _source_evidence_layer_write_candidate_blockers(source_candidate: dict[str, Any]) -> list[str]:
    if not isinstance(source_candidate, dict):
        return ["source_evidence_layer_write_candidate_not_object"]
    blockers: list[str] = []
    for field in source_candidate:
        if field in FORBIDDEN_SOURCE_WRITE_CANDIDATE_FIELDS:
            blockers.append(f"forbidden_source_evidence_layer_write_candidate_field:{field}")
    for field in REQUIRED_SOURCE_WRITE_CANDIDATE_FIELDS:
    ...
    if source_candidate.get("evidence_layer_write_candidate_schema") != SOURCE_EVIDENCE_LAYER_WRITE_CANDIDATE_SCHEMA:
    ...
    if not isinstance(boundary_flags, dict):
        ...
        required_true_flags = {
            ...
            "human_review_required": "source_evidence_layer_write_candidate_boundary_human_review_required_not_true",
            ...
            "not_review_queue_item": "source_evidence_layer_write_candidate_boundary_not_review_queue_item_not_true",
            "not_production_review_queue_item": (
                "source_evidence_layer_write_candidate_boundary_not_production_review_queue_item_not_true"
            ),
            "not_production_case": "source_evidence_layer_write_candidate_boundary_not_production_case_not_true",
...
def _import_candidate_from_source(source_candidate: dict[str, Any], *, index: int) -> dict[str, Any]:
    ...
    return {
        "production_evidence_import_candidate_schema": PRODUCTION_EVIDENCE_IMPORT_CANDIDATE_SCHEMA,
        "production_evidence_import_candidate_id": f"production-evidence-import-candidate-{index:03d}-{evidence_hash}",
...
def _source_evidence_layer_write_candidate_count(
    source_candidate_set: dict[str, Any] | None,
    source_candidates: list[dict[str, Any]],
) -> int:
    if isinstance(source_candidate_set, dict):
