...
CANDIDATE_SET_SCHEMA = "sentigraph_controlled_production_analysis_run_candidate_set_v0_1"
CANDIDATE_SCHEMA = "sentigraph_controlled_production_analysis_run_candidate_v0_1"
SUMMARY_SCHEMA = "sentigraph_controlled_production_analysis_run_candidate_summary_v0_1"
SOURCE_CANDIDATE_SET_SCHEMA = "sentigraph_controlled_production_case_candidate_set_v0_1"
SOURCE_CANDIDATE_SCHEMA = "sentigraph_controlled_production_case_candidate_v0_1"
PHASE = "8W-34"
APPROVAL_PHRASE = "APPROVE_8W_34_CONTROLLED_PRODUCTION_ANALYSIS_RUN_CANDIDATE_HELPER_IMPLEMENTATION"
SOURCE_WARN_STATUS = "production_case_candidate_set_warn_manual_review_required"
WARN_STATUS = "production_analysis_run_candidate_set_warn_manual_review_required"
EXPECTED_CASE_CANDIDATE_COUNT = 1
...
SOURCE_TRUE_FIELDS = {
    ...
    "production_case_candidate_created": "source_production_case_candidate_created_not_true",
}
...
def build_controlled_production_analysis_run_candidate_set(
    controlled_production_case_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
) -> dict[str, Any]:
    """Build one local analysis-run-candidate-shaped boundary object from an 8W-31 case candidate set."""

    blockers: list[str] = []
    blockers.extend(_approval_blockers(exact_approval_phrase))
    blockers.extend(_requested_action_blockers(requested_actions))

    source_candidates = _safe_source_candidates(controlled_production_case_candidate_set)
    source_case_candidate_count = _source_case_candidate_count(
        controlled_production_case_candidate_set,
        source_candidates,
    )
    ...
    if blockers:
        return _base_output(
            ...
            warnings=[],
            candidate=None,
            source_candidate_set=controlled_production_case_candidate_set,
    ...
    return _base_output(
        ...
        created=True,
        blockers=[],
        warnings=_candidate_warnings(controlled_production_case_candidate_set),
        candidate=candidate,
        source_candidate_set=controlled_production_case_candidate_set,
        source_case_candidate_count=source_case_candidate_count,
        source_controlled_evidence_item_count=source_controlled_evidence_item_count,
...
create_controlled_production_analysis_run_candidate_set = build_controlled_production_analysis_run_candidate_set


def build_safe_controlled_production_analysis_run_candidate_summary(
    controlled_production_case_candidate_set: dict[str, Any] | None,
    *,
    exact_approval_phrase: str | None,
    requested_actions: list[str] | dict[str, Any] | None = None,
) -> dict[str, Any]:
    candidate_set = build_controlled_production_analysis_run_candidate_set(
        controlled_production_case_candidate_set,
    ...
    return {
        ...
        "phase": PHASE,
        "production_analysis_run_candidate_set_schema": CANDIDATE_SET_SCHEMA,
        "production_analysis_run_candidate_set_status": candidate_set[
            "production_analysis_run_candidate_set_status"
        ],
        "source_production_case_candidate_set_schema": candidate_set[
        ...
        "production_analysis_run_candidate_count": candidate_set["production_analysis_run_candidate_count"],
        "production_analysis_run_candidate_created": candidate_set["production_analysis_run_candidate_created"],
        ...
        "production_case_candidate_created_upstream": candidate_set[
            "production_case_candidate_created_upstream"
        ],
...
def _base_output(
    *,
    ...
    return {
        "production_analysis_run_candidate_set_schema": CANDIDATE_SET_SCHEMA,
        "phase": PHASE,
        "production_analysis_run_candidate_set_status": status,
        "created_at": _utc_now(),
        "input_source_kind": "controlled_production_case_candidate",
        "source_production_case_candidate_set_schema": _safe_source_value(
            source_candidate_set,
            "production_case_candidate_set_schema",
        ),
        "source_production_case_candidate_schema": _source_production_case_candidate_schema(source_candidate_set),
        ...
        "production_analysis_run_candidate_count": len(candidates),
        "warning_count": _source_warning_count(source_candidate_set),
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "production_case_candidate_created_upstream": _truthy(
            source_candidate_set.get("production_case_candidate_created")
        ...
        "production_analysis_run_candidates": candidates,
    }
...
def _candidate_from_source(
    ...
    source_ids = [_safe_token(candidate.get("production_case_candidate_id")) for candidate in source_candidates]
    ...
    return {
        "production_analysis_run_candidate_schema": CANDIDATE_SCHEMA,
        "production_analysis_run_candidate_id": _analysis_run_candidate_id(source_ids),
        "source_production_case_candidate_id": source_ids[0] if source_ids else "unknown",
        "source_production_case_candidate_ids": source_ids,
        "source_production_case_candidate_count": source_case_candidate_count,
        "source_controlled_evidence_item_count": source_controlled_evidence_item_count,
        ...
        "verification_status_summary": _safe_label(first_source.get("verification_status")) or "needs_review",
...
def _source_candidate_set_blockers(
    ...
    if not isinstance(source_candidate_set, dict):
        return ["source_production_case_candidate_set_missing_or_not_object"]

    blockers: list[str] = []
    expected = {
        "production_case_candidate_set_schema": (
            SOURCE_CANDIDATE_SET_SCHEMA,
            "source_production_case_candidate_set_schema_wrong",
        ),
        "production_case_candidate_set_status": (
            SOURCE_WARN_STATUS,
            "source_production_case_candidate_set_status_not_warn_manual_review",
        ),
    }
    ...
    if source_case_candidate_count != EXPECTED_CASE_CANDIDATE_COUNT:
        blockers.append("source_production_case_candidate_count_not_one")
    if len(source_candidates) != EXPECTED_CASE_CANDIDATE_COUNT:
        blockers.append("source_production_case_candidates_count_not_one")
    ...
    for field, value in source_candidate_set.items():
        if field == "production_case_candidates":
            continue
        if field in FORBIDDEN_SOURCE_FIELDS:
            if field in SOURCE_FALSE_FIELDS and value is False:
                continue
            blockers.append(f"forbidden_source_production_case_candidate_set_field:{field}")
        elif _contains_forbidden_value(value):
            blockers.append(f"source_production_case_candidate_set_forbidden_value:{field}")
    return _dedupe(blockers)
...
def _source_candidate_blockers(source_candidates: list[dict[str, Any]]) -> list[str]:
    ...
    for candidate in source_candidates:
        if not isinstance(candidate, dict):
            blockers.append("source_production_case_candidate_not_object")
            continue
        for field, value in candidate.items():
            if field in FORBIDDEN_SOURCE_FIELDS:
                blockers.append(f"forbidden_source_production_case_candidate_field:{field}")
            elif _contains_forbidden_value(value):
                blockers.append(f"source_production_case_candidate_forbidden_value:{field}")
        if candidate.get("production_case_candidate_schema") != SOURCE_CANDIDATE_SCHEMA:
        ...
        if candidate.get("human_review_required") is not True:
            blockers.append("source_production_case_candidate_human_review_required_not_true")
        if candidate.get("no_automatic_trust_upgrade") is not True:
            blockers.append("source_production_case_candidate_no_automatic_trust_upgrade_not_true")
        if candidate.get("production_case_candidate_only") is not True:
            blockers.append("source_production_case_candidate_only_not_true")
        if candidate.get("production_case_ready") is not False:
            blockers.append("source_production_case_candidate_production_case_ready_true")
        if candidate.get("production_analysis_run_ready") is not False:
            blockers.append("source_production_case_candidate_analysis_run_ready_true")
        if candidate.get("analysis_ready") is not False:
            blockers.append("source_production_case_candidate_analysis_ready_true")
        if candidate.get("report_ready") is not False:
            blockers.append("source_production_case_candidate_report_ready_true")
    return _dedupe(blockers)
...
def _safe_source_candidates(source_candidate_set: dict[str, Any] | None) -> list[dict[str, Any]]:
    if not isinstance(source_candidate_set, dict):
        return []
    candidates = source_candidate_set.get("production_case_candidates")
...
def _source_case_candidate_count(
    ...
    if isinstance(source_candidate_set, dict):
        count = source_candidate_set.get("production_case_candidate_count")
...
def _source_production_case_candidate_schema(source_candidate_set: dict[str, Any] | None) -> str | None:
    candidates = _safe_source_candidates(source_candidate_set)
    if candidates:
        return _safe_label(candidates[0].get("production_case_candidate_schema"))
    return None
...
def _blocked_status(blockers: list[str]) -> str:
    if not blockers:
        return "blocked_invalid_source_production_case_candidate"
    first = blockers[0]
    if first in {
    ...
    if first.startswith("requested_action_blocked"):
        ...
        if "analysis_execution" in first:
            return "blocked_unapproved_analysis_execution_request"
    ...
    if first.startswith("source_production_case_candidate_count") or first.startswith(
        "source_production_case_candidates_count"
    ):
        return "blocked_candidate_limit_violation"
    if first.startswith("source_controlled_evidence_item_count"):
        return "blocked_candidate_limit_violation"
    if first.startswith("forbidden_source") or first.startswith("source_production_case_candidate_forbidden_value"):
...
def _analysis_run_candidate_id(source_ids: list[str]) -> str:
    first_id = source_ids[0] if source_ids else "unknown"
    return f"controlled-production-analysis-run-candidate-001-{first_id}"
