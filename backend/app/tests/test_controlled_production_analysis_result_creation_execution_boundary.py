...
from app.services import controlled_production_analysis_result_creation_execution_boundary as module
from app.services.controlled_production_analysis_result_creation_execution_boundary import (
    APPROVAL_PHRASE,
    build_controlled_production_analysis_result_creation_execution_boundary_set,
    build_safe_controlled_production_analysis_result_creation_execution_boundary_summary,
)
...
NON_ASCII_APPROVAL_PHRASE = (
    "\u6279\u51c6 8W-61 Controlled Production Analysis Result Creation Execution Boundary Helper Implementation"
)
MOJIBAKE_APPROVAL_PHRASE = (
    "\u93b5\u7470\u567f 8W-61 Controlled Production Analysis Result Creation Execution Boundary Helper Implementation"
)
...
def _source_runtime_boundary(**overrides: object) -> dict[str, object]:
    boundary: dict[str, object] = {
        "production_analysis_result_creation_runtime_boundary_schema": (
            "sentigraph_controlled_production_analysis_result_creation_runtime_boundary_v0_1"
        ),
        "production_analysis_result_creation_runtime_boundary_id": (
            "controlled-production-analysis-result-creation-runtime-boundary-001-hash-001"
        ),
        ...
        "input_scope_summary_redacted": (
            "controlled_local_production_analysis_result_creation_runtime_boundary_summary_only"
        ),
...
def _valid_runtime_boundary_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "production_analysis_result_creation_runtime_boundary_set_status": (
            "production_analysis_result_creation_runtime_boundary_set_warn_manual_review_required"
        ),
        ...
        "source_production_case_candidate_count": 1,
        "source_controlled_evidence_item_count": 5,
        "creation_runtime_boundary_mode": "backend_only_local_production_analysis_result_creation_runtime_boundary",
        "production_analysis_result_creation_runtime_boundary_count": 1,
        "warning_count": 1,
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "production_analysis_result_creation_candidate_created": True,
        "production_analysis_result_creation_candidate_created_upstream": True,
        "production_analysis_result_creation_boundary_created": True,
        ...
        "production_analysis_result_creation_runtime_boundaries": [_source_runtime_boundary()],
        "boundary_flags": {
            "backend_only": True,
            "local_only": True,
            "creation_runtime_boundary_only": True,
...
def _build_ready(source: dict[str, object] | None = None) -> dict[str, object]:
    return build_controlled_production_analysis_result_creation_execution_boundary_set(
        source or _valid_runtime_boundary_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        decision_8w60=EXPECTED_8W60_DECISION,
        selected_next_boundary_option_8w60=EXPECTED_8W60_NEXT,
    )
...
def test_ready_path_builds_one_warning_preserving_creation_execution_boundary() -> None:
    result = _build_ready()

    assert result["production_analysis_result_creation_execution_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_execution_boundary_set_v0_1"
    )
    assert result["production_analysis_result_creation_execution_boundary_set_status"] == (
        "production_analysis_result_creation_execution_boundary_set_warn_manual_review_required"
    )
    assert result["input_source_kind"] == "controlled_production_analysis_result_creation_runtime_boundary"
    assert result["creation_execution_boundary_mode"] == (
        "backend_only_local_production_analysis_result_creation_execution_boundary"
    )
    assert result["production_analysis_result_creation_execution_boundary_count"] == 1
    assert result["source_production_analysis_result_creation_runtime_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_runtime_boundary_set_v0_1"
    )
    assert result["source_production_analysis_result_creation_runtime_boundary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_runtime_boundary_v0_1"
    )
    ...
    assert result["production_analysis_result_creation_runtime_boundary_created_upstream"] is True
    assert result["production_analysis_result_creation_execution_boundary_created"] is True
    ...
    boundaries = result["production_analysis_result_creation_execution_boundaries"]
    assert len(boundaries) == 1
    boundary = boundaries[0]
    assert boundary["production_analysis_result_creation_execution_boundary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_execution_boundary_v0_1"
    )
    assert boundary["creation_execution_boundary_only"] is True
    ...
    assert boundary["boundary_flags"]["not_production_analysis_result_creation_execution"] is True
    assert boundary["boundary_flags"]["not_production_analysis_result_runtime_use"] is True
    _assert_no_forbidden_payload(result)


def test_safe_summary_without_runtime_boundary_list_builds_one_creation_execution_boundary() -> None:
    source = _valid_runtime_boundary_set()
    source.pop("production_analysis_result_creation_runtime_boundaries")
    ...
    assert result["production_analysis_result_creation_execution_boundary_count"] == 1
    assert result["production_analysis_result_creation_execution_boundary_created"] is True
    assert result["production_analysis_result_creation_execution_boundaries"][0][
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "APPROVE_8W_61_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CREATION_EXECUTION_BOUNDARY_HELPER_IMPLEMENTATI0N",
...
def test_exact_approval_phrase_required_before_creation_execution_boundary_construction(
    phrase: str | None,
    expected_status: str,
    ...
    result = build_controlled_production_analysis_result_creation_execution_boundary_set(
    ...
    assert result["production_analysis_result_creation_execution_boundary_set_status"] == expected_status
    assert result["production_analysis_result_creation_execution_boundary_count"] == 0
    assert result["production_analysis_result_creation_execution_boundary_created"] is False
    assert result["production_analysis_result_creation_execution_boundaries"] == []


@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("production_analysis_result_creation_runtime_boundary_set_schema", "wrong"),
        ("source_production_analysis_result_creation_runtime_boundary_schema", "wrong"),
        ("production_analysis_result_creation_runtime_boundary_set_status", "wrong"),
        ("source_production_analysis_result_creation_runtime_boundary_count", 2),
        ("production_analysis_result_creation_runtime_boundary_count", 2),
        ...
        ("production_analysis_result_creation_candidate_created", False),
        ("production_analysis_result_creation_boundary_created", False),
...
def test_source_runtime_boundary_validation_blocks_unsafe_inputs(field: str, value: object) -> None:
...
@pytest.mark.parametrize(
    ...
    [
        (None, EXPECTED_8W60_NEXT),
        ("needs_fix", EXPECTED_8W60_NEXT),
        (EXPECTED_8W60_DECISION, None),
        (EXPECTED_8W60_DECISION, "pause"),
    ],
)
def test_missing_or_wrong_8w60_gate_status_blocks(decision: str | None, selected_next: str | None) -> None:
    result = build_controlled_production_analysis_result_creation_execution_boundary_set(
        _valid_runtime_boundary_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        decision_8w60=decision,
        selected_next_boundary_option_8w60=selected_next,
    )

    assert result["production_analysis_result_creation_execution_boundary_set_status"] == (
        "blocked_invalid_source_production_analysis_result_creation_runtime_boundary"
    )
    assert result["production_analysis_result_creation_execution_boundary_count"] == 0
...
def test_requested_side_effect_actions_block(requested_action: str) -> None:
    ...
    assert result["production_analysis_result_creation_execution_boundary_set_status"].startswith("blocked_")
    assert result["production_analysis_result_creation_execution_boundary_count"] == 0
...
def test_missing_source_creation_runtime_boundary_blocks() -> None:
    result = _build_ready(_valid_runtime_boundary_set(production_analysis_result_creation_runtime_boundaries=[]))
...
def test_multiple_execution_boundary_request_blocks() -> None:
    result = build_controlled_production_analysis_result_creation_execution_boundary_set(
        _valid_runtime_boundary_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        decision_8w60=EXPECTED_8W60_DECISION,
        selected_next_boundary_option_8w60=EXPECTED_8W60_NEXT,
        execution_boundary_count=2,
    )

    assert result["production_analysis_result_creation_execution_boundary_set_status"] == (
        "blocked_creation_execution_boundary_limit_violation"
    )
    assert result["production_analysis_result_creation_execution_boundary_count"] == 0


def test_safe_summary_is_aggregate_only() -> None:
    summary = build_safe_controlled_production_analysis_result_creation_execution_boundary_summary(
        _valid_runtime_boundary_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        decision_8w60=EXPECTED_8W60_DECISION,
        selected_next_boundary_option_8w60=EXPECTED_8W60_NEXT,
    ...
    assert summary["production_analysis_result_creation_execution_boundary_count"] == 1
    ...
    assert "production_analysis_result_creation_execution_boundaries" not in summary
