...
from app.services.controlled_production_analysis_run_candidate import (
    APPROVAL_PHRASE,
    build_controlled_production_analysis_run_candidate_set,
    build_safe_controlled_production_analysis_run_candidate_summary,
)


EXPECTED_APPROVAL_PHRASE = "APPROVE_8W_34_CONTROLLED_PRODUCTION_ANALYSIS_RUN_CANDIDATE_HELPER_IMPLEMENTATION"
NON_ASCII_APPROVAL_PHRASE = "\u6279\u51c6 8W-34 Controlled Production Analysis Run Candidate Helper Implementation"
MOJIBAKE_APPROVAL_PHRASE = "\u9395\u7470\u567f 8W-34 Controlled Production Analysis Run Candidate Helper Implementation"
...
def _source_case_candidate(**overrides: object) -> dict[str, object]:
    candidate: dict[str, object] = {
        "production_case_candidate_schema": "sentigraph_controlled_production_case_candidate_v0_1",
        "production_case_candidate_id": "controlled-production-case-candidate-001-hash-001",
...
def _valid_source_case_candidate_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "production_case_candidate_created": True,
        "production_case_candidate_only": True,
        "controlled_evidenceitem_created_upstream": True,
        "evidence_item_created_upstream": True,
        "evidence_layer_write_upstream": True,
        ...
        "production_case_candidates": [_source_case_candidate()],
    }
...
def _assert_blocked(candidate_set: dict[str, object], expected_reason: str) -> None:
    assert str(candidate_set["production_analysis_run_candidate_set_status"]).startswith("blocked_")
    assert expected_reason in candidate_set["blockers"]
    assert candidate_set["production_analysis_run_candidate_created"] is False
    assert candidate_set["production_analysis_run_candidate_count"] == 0
    assert candidate_set["production_analysis_run_candidates"] == []
...
def test_ready_path_builds_one_controlled_production_analysis_run_candidate() -> None:
    ...
    candidate_set = build_controlled_production_analysis_run_candidate_set(
        _valid_source_case_candidate_set(),
    ...
    assert candidate_set["phase"] == "8W-34"
    assert candidate_set["production_analysis_run_candidate_set_status"] == (
        "production_analysis_run_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["input_source_kind"] == "controlled_production_case_candidate"
    assert candidate_set["source_production_case_candidate_set_schema"] == (
        "sentigraph_controlled_production_case_candidate_set_v0_1"
    )
    assert candidate_set["source_production_case_candidate_schema"] == (
    ...
    assert candidate_set["production_case_candidate_created_upstream"] is True
    ...
    allowed_fields = {
        ...
        "source_production_case_candidate_id",
        "source_production_case_candidate_ids",
        ...
        "production_analysis_run_candidate_only",
    ...
    assert candidate["production_analysis_run_candidate_schema"] == (
    ...
    assert candidate["production_analysis_run_candidate_only"] is True
    ...
    assert "production_case_not_created" in candidate["production_analysis_run_readiness_blockers"]
    assert "analysis_execution_not_approved" in candidate["analysis_execution_readiness_blockers"]
    assert "analysis_result_not_created" in candidate["report_readiness_blockers"]
    assert candidate["boundary_flags"]["production_analysis_run_candidate_only"] is True
    assert candidate["boundary_flags"]["not_production_analysis_run"] is True
    assert candidate["boundary_flags"]["not_analysis_execution"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("APPROVE_8W_34_CONTROLLED_PRODUCTION_ANALYSIS_RUN_CANDIDATE_HELPER_IMPLEMENTATI0N", "blocked_wrong_exact_approval"),
    ],
)
def test_exact_ascii_approval_required_before_candidate_construction_and_file_access(
    ...
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-34 must not open files")
...
@pytest.mark.parametrize(
    ("field", "value", "reason"),
    [
        ("production_case_candidate_set_schema", "wrong", "source_production_case_candidate_set_schema_wrong"),
        (
            "production_case_candidate_set_status",
            "production_case_candidate_set_ready",
            "source_production_case_candidate_set_status_not_warn_manual_review",
        ),
        ("production_case_candidate_count", 2, "source_production_case_candidate_count_not_one"),
        ...
        ("production_case_candidate_created", False, "source_production_case_candidate_created_not_true"),
...
def test_source_validation_blocks_unsafe_candidate_set_fields(field: str, value: object, reason: str) -> None:
    candidate_set = build_controlled_production_analysis_run_candidate_set(
        _valid_source_case_candidate_set(**{field: value}),
...
def test_source_runtime_side_effects_block(runtime_flag: str, reason: str) -> None:
    source = _valid_source_case_candidate_set()
...
def test_candidate_count_blocks_when_source_candidates_absent_or_malformed() -> None:
    absent = build_controlled_production_analysis_run_candidate_set(
        _valid_source_case_candidate_set(production_case_candidates=[]),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(absent, "source_production_case_candidates_count_not_one")

    malformed = build_controlled_production_analysis_run_candidate_set(
        _valid_source_case_candidate_set(production_case_candidates=[{"wrong": "shape"}]),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(malformed, "source_production_case_candidate_schema_wrong")


def test_forbidden_fields_in_source_candidate_block_or_never_emit() -> None:
    unsafe_candidate = _source_case_candidate(
        ...
        analysis_execution_id="analysis-execution-should-never-appear",
    ...
    candidate_set = build_controlled_production_analysis_run_candidate_set(
        _valid_source_case_candidate_set(production_case_candidates=[unsafe_candidate]),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    _assert_blocked(candidate_set, "forbidden_source_production_case_candidate_field:production_analysis_run_id")
...
def test_requested_side_effects_block_and_keep_flags_false(requested_action: str) -> None:
    ...
    assert candidate_set["production_analysis_run_candidate_set_status"].startswith("blocked_")
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_production_analysis_run_candidate_summary(
        _valid_source_case_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == "sentigraph_controlled_production_analysis_run_candidate_summary_v0_1"
    assert summary["phase"] == "8W-34"
    assert summary["production_analysis_run_candidate_set_schema"] == (
        "sentigraph_controlled_production_analysis_run_candidate_set_v0_1"
    )
    assert summary["production_analysis_run_candidate_set_status"] == (
        "production_analysis_run_candidate_set_warn_manual_review_required"
    )
    assert summary["production_analysis_run_candidate_count"] == 1
    ...
    assert summary["production_analysis_run_candidate_created"] is True
    ...
    assert "production_analysis_run_candidates" not in summary
    assert "source_production_case_candidate_id" not in _serialized(summary)
