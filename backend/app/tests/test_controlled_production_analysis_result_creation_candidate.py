...
from app.services import controlled_production_analysis_result_creation_candidate as module
from app.services.controlled_production_analysis_result_creation_candidate import (
    APPROVAL_PHRASE,
...
NON_ASCII_APPROVAL_PHRASE = (
    "\u6279\u51c6 8W-57 Controlled Production Analysis Result Creation Candidate Helper Implementation"
)
MOJIBAKE_APPROVAL_PHRASE = (
    "\u9395\u7470\u567f 8W-57 Controlled Production Analysis Result Creation Candidate Helper Implementation"
)
...
def _source_creation_boundary(**overrides: object) -> dict[str, object]:
    boundary: dict[str, object] = {
        "production_analysis_result_creation_boundary_schema": (
            "sentigraph_controlled_production_analysis_result_creation_boundary_v0_1"
        ),
        "production_analysis_result_creation_boundary_id": (
            "controlled-production-analysis-result-creation-boundary-001-hash-001"
        ),
        ...
        "input_scope_summary_redacted": (
            "controlled_local_production_analysis_result_creation_boundary_summary_only"
        ),
...
def _valid_creation_boundary_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "input_source_kind": (
            "controlled_production_analysis_result_creation_or_runtime_execution_candidate"
        ),
        ...
        "production_analysis_result_creation_boundary_count": 1,
        "warning_count": 1,
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "8w56_decision": "ready",
        "8w56_selected_next_boundary_option": (
        ...
        "production_analysis_result_creation_boundaries": [_source_creation_boundary()],
        "boundary_flags": {
            "backend_only": True,
            "local_only": True,
            "creation_boundary_only": True,
...
def _build_ready(source: dict[str, object] | None = None) -> dict[str, object]:
    return build_controlled_production_analysis_result_creation_candidate_set(
        source or _valid_creation_boundary_set(),
...
def test_ready_path_builds_one_warning_preserving_creation_candidate() -> None:
    result = _build_ready()

    assert result["production_analysis_result_creation_candidate_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_candidate_set_v0_1"
    )
    assert result["production_analysis_result_creation_candidate_set_status"] == (
        "production_analysis_result_creation_candidate_set_warn_manual_review_required"
    )
    assert result["production_analysis_result_creation_candidate_count"] == 1
    assert result["source_production_analysis_result_creation_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_boundary_set_v0_1"
    )
    assert result["source_production_analysis_result_creation_boundary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_boundary_v0_1"
    )
    ...
    assert result["production_analysis_result_creation_boundary_created_upstream"] is True
    assert result["production_analysis_result_creation_candidate_created"] is True
    ...
    candidates = result["production_analysis_result_creation_candidates"]
    assert len(candidates) == 1
    candidate = candidates[0]
    assert candidate["production_analysis_result_creation_candidate_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_candidate_v0_1"
    )
    assert candidate["creation_candidate_only"] is True
    assert candidate["human_review_required"] is True
    assert candidate["production_analysis_result_created"] is False
    assert candidate["production_analysis_result_creation_executed"] is False
    ...
    assert candidate["boundary_flags"]["not_production_analysis_result_creation"] is True
    _assert_no_forbidden_payload(result)


def test_safe_summary_without_boundary_list_builds_one_creation_candidate() -> None:
    source = _valid_creation_boundary_set()
    source.pop("production_analysis_result_creation_boundaries")
    ...
    assert result["production_analysis_result_creation_candidate_count"] == 1
    assert result["production_analysis_result_creation_candidate_created"] is True
    assert result["production_analysis_result_creation_candidates"][0][
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("APPROVE_8W_57_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CREATION_CANDIDATE_HELPER_IMPLEMENTATI0N", "blocked_wrong_exact_approval"),
...
def test_exact_approval_phrase_required_before_candidate_construction(
    ...
    result = build_controlled_production_analysis_result_creation_candidate_set(
        _valid_creation_boundary_set(),
        exact_approval_phrase=phrase,
    )

    assert result["production_analysis_result_creation_candidate_set_status"] == expected_status
    assert result["production_analysis_result_creation_candidate_count"] == 0
    assert result["production_analysis_result_creation_candidate_created"] is False
    assert result["production_analysis_result_creation_candidates"] == []


@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("production_analysis_result_creation_boundary_set_schema", "wrong"),
        ("source_production_analysis_result_creation_boundary_schema", "wrong"),
        ("production_analysis_result_creation_boundary_set_status", "wrong"),
        ("production_analysis_result_creation_boundary_count", 2),
        ...
        ("8w56_decision", "needs_fix"),
        ("8w56_selected_next_boundary_option", "pause"),
        ("production_analysis_result_creation_boundary_created", False),
        ("production_analysis_result_creation_or_runtime_execution_candidate_created", False),
...
def test_source_creation_boundary_validation_blocks_unsafe_inputs(field: str, value: object) -> None:
    result = _build_ready(_valid_creation_boundary_set(**{field: value}))
...
def test_runtime_side_effect_flags_true_block(flag: str) -> None:
    result = _build_ready(_valid_creation_boundary_set(runtime_side_effects=_runtime_side_effects(**{flag: True})))
...
def test_requested_side_effect_actions_block(requested_action: str) -> None:
    ...
    assert result["production_analysis_result_creation_candidate_set_status"].startswith("blocked_")
    assert result["production_analysis_result_creation_candidate_count"] == 0
...
def test_forbidden_fields_block_and_are_not_emitted(field: str, value: object) -> None:
    source = _valid_creation_boundary_set(**{field: value})
...
def test_missing_source_creation_boundary_blocks() -> None:
    result = _build_ready(_valid_creation_boundary_set(production_analysis_result_creation_boundaries=[]))

    assert result["production_analysis_result_creation_candidate_set_status"] == (
        "blocked_invalid_source_production_analysis_result_creation_boundary"
    )
    assert result["production_analysis_result_creation_candidate_count"] == 0
...
def test_multiple_candidate_request_blocks() -> None:
    ...
    assert result["production_analysis_result_creation_candidate_set_status"] == (
        "blocked_creation_candidate_limit_violation"
    )
    assert result["production_analysis_result_creation_candidate_count"] == 0


def test_safe_summary_is_aggregate_only() -> None:
    summary = build_safe_controlled_production_analysis_result_creation_candidate_summary(
        _valid_creation_boundary_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_candidate_summary_v0_1"
    )
    assert summary["production_analysis_result_creation_candidate_count"] == 1
    ...
    assert "production_analysis_result_creation_candidates" not in summary
