...
from app.services import controlled_production_analysis_result_boundary as module
from app.services.controlled_production_analysis_result_boundary import (
    APPROVAL_PHRASE,
    build_controlled_production_analysis_result_boundary_set,
    build_safe_controlled_production_analysis_result_boundary_summary,
)
...
NON_ASCII_APPROVAL_PHRASE = (
    "\u6279\u51c6 8W-46 Controlled Production Analysis Result Boundary Helper Implementation"
)
MOJIBAKE_APPROVAL_PHRASE = (
    "\u9395\u7470\u567f 8W-46 Controlled Production Analysis Result Boundary Helper Implementation"
)
...
def _source_production_analysis_result_candidate(**overrides: object) -> dict[str, object]:
    candidate: dict[str, object] = {
        ...
        "source_analysis_result_candidate_id": "controlled-analysis-result-candidate-001-hash-001",
        "source_analysis_result_candidate_ids": ["controlled-analysis-result-candidate-001-hash-001"],
        ...
        "input_scope_summary_redacted": "controlled_local_production_analysis_result_candidate_summary_only",
        ...
        "intended_module_scope_labels": [
            "production_analysis_result_candidate_boundary",
        ...
        "production_analysis_result_candidate_only": True,
        ...
        "boundary_flags": {
            "production_analysis_result_candidate_only": True,
            "controlled_analysis_result_candidate_derived_only": True,
            "human_review_required": True,
...
def _valid_source_production_analysis_result_candidate_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "input_source_kind": "controlled_analysis_result_candidate",
        "source_analysis_result_candidate_set_schema": (
            "sentigraph_controlled_analysis_result_candidate_set_v0_1"
        ),
        "source_analysis_result_candidate_schema": "sentigraph_controlled_analysis_result_candidate_v0_1",
        "source_analysis_result_candidate_count": 1,
        "source_actual_analysis_execution_candidate_count": 1,
        "source_production_analysis_run_candidate_count": 1,
        "source_production_case_candidate_count": 1,
        "source_controlled_evidence_item_count": 5,
        "production_analysis_result_candidate_mode": (
        ...
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "analysis_result_candidate_created_upstream": True,
        "production_analysis_result_candidate_created": True,
        "production_analysis_result_candidate_only": True,
        "production_analysis_result_created": False,
        "production_analysis_result_runtime_used": False,
        "analysis_result_generation_executed": False,
        ...
        "warnings": ["manual_review_required", "selected_sample_only", "production_analysis_result_candidate_only"],
        "blockers": [],
        "audit_summary": {
            "audit_schema": "sentigraph_controlled_production_analysis_result_candidate_audit_summary_v0_1",
            "phase": "8W-43",
        ...
        "production_analysis_result_candidates": [_source_production_analysis_result_candidate()],
    }
...
def _valid_source_summary(**overrides: object) -> dict[str, object]:
    summary = dict(_valid_source_production_analysis_result_candidate_set())
    summary.pop("production_analysis_result_candidates")
    summary["summary_schema"] = "sentigraph_controlled_production_analysis_result_candidate_summary_v0_1"
...
def _assert_blocked(boundary_set: dict[str, object], expected_reason: str) -> None:
    assert str(boundary_set["production_analysis_result_boundary_set_status"]).startswith("blocked_")
    assert expected_reason in boundary_set["blockers"]
    assert boundary_set["production_analysis_result_boundary_created"] is False
    assert boundary_set["production_analysis_result_boundary_count"] == 0
    assert boundary_set["production_analysis_result_boundaries"] == []
...
def test_ready_path_builds_one_controlled_production_analysis_result_boundary() -> None:
    ...
    boundary_set = build_controlled_production_analysis_result_boundary_set(
        _valid_source_production_analysis_result_candidate_set(),
        exact_approval_phrase=EXPECTED_APPROVAL_PHRASE,
    )

    assert boundary_set["production_analysis_result_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_boundary_set_v0_1"
    )
    assert boundary_set["phase"] == "8W-46"
    assert boundary_set["production_analysis_result_boundary_set_status"] == (
        "production_analysis_result_boundary_set_warn_manual_review_required"
    )
    assert boundary_set["input_source_kind"] == "controlled_production_analysis_result_candidate"
    assert boundary_set["source_production_analysis_result_candidate_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_candidate_set_v0_1"
    )
    assert boundary_set["source_production_analysis_result_candidate_schema"] == (
        "sentigraph_controlled_production_analysis_result_candidate_v0_1"
    )
    ...
    assert boundary_set["boundary_mode"] == "backend_only_local_production_analysis_result_boundary"
    assert boundary_set["production_analysis_result_boundary_count"] == 1
    ...
    assert boundary_set["production_analysis_result_candidate_created_upstream"] is True
    assert boundary_set["production_analysis_result_boundary_created"] is True
    assert boundary_set["production_analysis_result_created"] is False
    assert boundary_set["production_analysis_result_runtime_used"] is False
    assert boundary_set["analysis_result_generation_executed"] is False
    assert boundary_set["analysis_result_created"] is False
    ...
    assert boundary_set["blockers"] == []

    boundaries = boundary_set["production_analysis_result_boundaries"]
    ...
    allowed_fields = {
        ...
        "source_production_analysis_result_candidate_id",
    ...
    assert set(boundary) == allowed_fields
    assert boundary["production_analysis_result_boundary_schema"] == (
    ...
    assert boundary["trust_boundary_label"] == "no_automatic_trust_upgrade"
    assert boundary["verification_status_summary"] == "needs_review"
    ...
    assert "manual_review_required" in boundary["warning_labels"]
    assert "production_analysis_result_creation_not_approved" in boundary[
        "production_analysis_result_creation_readiness_blockers"
    ]
    assert "production_analysis_result_runtime_not_approved" in boundary["production_runtime_blockers"]
    assert "analysis_result_not_created" in boundary["report_readiness_blockers"]
    assert "production_analysis_run_creation_not_approved" in boundary[
        "production_record_creation_blockers"
    ]
    assert boundary["boundary_flags"]["boundary_only"] is True
    assert boundary["boundary_flags"]["not_production_analysis_result"] is True
    assert boundary["boundary_flags"]["not_production_analysis_result_runtime"] is True
    assert boundary["boundary_flags"]["not_analysis_result_generation"] is True
    assert boundary["boundary_flags"]["not_report_ready"] is True
    _assert_all_side_effects_false(boundary_set)
    _assert_safe_output(boundary_set)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "APPROVE_8W_46_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_BOUNDARY_HELPER_IMPLEMENTATI0N",
...
def test_exact_ascii_approval_required_before_boundary_construction_and_file_access(
    phrase: str | None,
    expected_reason: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-46 must not open files")

    def blocked_boundary(*args, **kwargs):
        raise AssertionError("8W-46 must not construct boundaries on wrong approval")

    monkeypatch.setattr(builtins, "open", blocked_open)
    monkeypatch.setattr(Path, "open", blocked_open)
    monkeypatch.setattr(module, "_boundary_from_source", blocked_boundary)
    ...
    _assert_blocked(boundary_set, expected_reason)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "production_analysis_result_candidate_set_status",
            "production_analysis_result_candidate_set_ready",
            "source_production_analysis_result_candidate_set_status_not_warn_manual_review",
        ),
        (
            "production_analysis_result_candidate_count",
            2,
        ...
        (
            "production_analysis_result_runtime_used",
            True,
            "source_production_analysis_result_runtime_used_true",
        ),
...
def test_source_validation_blocks_unsafe_boundary_set_fields(field: str, value: object, reason: str) -> None:
    boundary_set = build_controlled_production_analysis_result_boundary_set(
        _valid_source_production_analysis_result_candidate_set(**{field: value}),
...
def test_source_runtime_side_effects_block(runtime_flag: str, reason: str) -> None:
    source = _valid_source_production_analysis_result_candidate_set()
...
def test_boundary_count_blocks_when_source_candidates_absent_or_malformed() -> None:
    absent = build_controlled_production_analysis_result_boundary_set(
        _valid_source_production_analysis_result_candidate_set(production_analysis_result_candidates=[]),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(absent, "source_production_analysis_result_candidates_count_not_one")

    malformed = build_controlled_production_analysis_result_boundary_set(
        _valid_source_production_analysis_result_candidate_set(
            production_analysis_result_candidates=[{"wrong": "shape"}]
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(malformed, "source_production_analysis_result_candidate_schema_wrong")


def test_forbidden_fields_in_source_candidate_block_or_never_emit() -> None:
    unsafe_candidate = _source_production_analysis_result_candidate(
    ...
    boundary_set = build_controlled_production_analysis_result_boundary_set(
        _valid_source_production_analysis_result_candidate_set(
            production_analysis_result_candidates=[unsafe_candidate]
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    _assert_blocked(boundary_set, "forbidden_source_production_analysis_result_candidate_field:production_analysis_result_id")
...
def test_requested_side_effects_block_and_keep_flags_false(requested_action: str) -> None:
    ...
    assert boundary_set["production_analysis_result_boundary_set_status"].startswith("blocked_")
    assert f"requested_action_blocked:{requested_action}" in boundary_set["blockers"]
    assert boundary_set["production_analysis_result_created"] is False
    assert boundary_set["production_analysis_result_runtime_used"] is False
    assert boundary_set["analysis_result_generation_executed"] is False
    assert boundary_set["analysis_result_created"] is False
...
def test_safe_summary_input_can_build_boundary_without_exposing_candidate_details() -> None:
    boundary_set = build_controlled_production_analysis_result_boundary_set(
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_production_analysis_result_boundary_summary(
        _valid_source_production_analysis_result_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == (
        "sentigraph_controlled_production_analysis_result_boundary_summary_v0_1"
    )
    assert summary["phase"] == "8W-46"
    assert summary["production_analysis_result_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_boundary_set_v0_1"
    )
    assert summary["production_analysis_result_boundary_set_status"] == (
        "production_analysis_result_boundary_set_warn_manual_review_required"
    )
    assert summary["production_analysis_result_boundary_count"] == 1
    ...
    assert summary["production_analysis_result_boundary_created"] is True
    ...
    assert "source_production_analysis_result_candidate_id" not in _serialized(summary)
