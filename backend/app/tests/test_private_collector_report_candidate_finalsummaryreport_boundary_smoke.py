...
from app.services import final_report_boundary_source11_governance_handoff
...
ROW_SENTINEL = "ROW_SENTINEL_SHOULD_NOT_BE_READ_OR_EXPOSED_8X12"
RAW_IDENTITY_SENTINEL = "RAW_IDENTITY_SHOULD_NOT_BE_EXPOSED_8X12"
PROFILE_URL_SENTINEL = "PROFILE_URL_SENTINEL_SHOULD_NOT_BE_EXPOSED_8X12"
SECRET_SENTINEL = "SECRET_SENTINEL_SHOULD_NOT_BE_EXPOSED_8X12"
ROW_LIKE_FILES = {
...
def _provider_result_payload(package_name: str = "synthetic_package") -> dict:
    return {
        ...
        "provider_result_id": "provider_result_8x12_final_boundary",
        "provider_job_id": "provider_job_8x12_final_boundary",
        "request_id": "analysis_request_8x12_final_boundary",
...
def _build_review_only_staging_summary(provider_result_path: Path, export_root: Path) -> dict:
    ...
    candidate = create_review_only_staging_candidate(handoff_summary, requested_by="8x12_final_boundary_smoke")
...
def _build_report_candidate(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> dict:
    ...
    def guarded_read_text(self: Path, *args, **kwargs):
        if self.name in ROW_LIKE_FILES:
            raise AssertionError(f"{self.name} must not be opened or parsed in 8X-12 final boundary smoke")
        return original_read_text(self, *args, **kwargs)

    monkeypatch.setattr(Path, "read_text", guarded_read_text)
    staging_summary = _build_review_only_staging_summary(provider_result_path, export_root)
    bridge = build_staging_candidate_generated_run_bridge(staging_summary, created_by="8x12_final_boundary_smoke")
    bridge["minimum_real_run_input_candidate"]["fixture_metadata"]["stage_id"] = "T0"
    execution = execute_minimum_real_run_from_bridge_candidate(bridge, created_by="8x12_final_boundary_smoke")
    integration = integrate_generated_run_with_dense_graph_from_execution(
        execution,
        created_by="8x12_final_boundary_smoke",
    )
    return build_dense_graph_report_candidate_from_integration(
        integration,
        created_by="8x12_final_boundary_smoke",
    )
...
def _install_downstream_side_effect_guards(monkeypatch: pytest.MonkeyPatch) -> None:
    def fail_downstream(*args, **kwargs):
        raise AssertionError("downstream FinalSummaryReport/export/public delivery must not be called in 8X-12")

    def fail_network(*args, **kwargs):
        raise AssertionError("network access must not occur in 8X-12")

    for module in (
        final_report_boundary_source11_governance_handoff,
...
def test_report_candidate_reaches_local_finalsummaryreport_boundary_without_runtime_or_export(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    _install_downstream_side_effect_guards(monkeypatch)

    report_candidate = _build_report_candidate(tmp_path, monkeypatch)
    ...
    encoded = _encoded({"candidate": report_candidate, "boundary": boundary, "summary": summary})

    assert report_candidate["report_candidate_schema"] == "sentigraph_dense_graph_report_candidate_v0_1"
    assert report_candidate["report_candidate_status"] == "candidate_ready"
    assert report_candidate["candidate_mode"] == "backend_only_local_report_candidate"
    assert report_candidate["report_candidate_created"] is True
    assert report_candidate["final_report_created"] is False
    assert report_candidate["b_end_report_runtime_generated"] is False
    assert report_candidate["sandbox_public_event_generated"] is False
    assert report_candidate["export_artifact_created"] is False
    assert report_candidate["frontend_ready"] is False
    assert report_candidate["route_ready"] is False
    assert report_candidate["production_ready"] is False
    assert report_candidate["customer_ready"] is False
    assert report_candidate["boundary_flags"]["human_review_required"] is True
    assert report_candidate["boundary_flags"]["not_full_web"] is True
    assert report_candidate["boundary_flags"]["not_full_platform"] is True
    assert report_candidate["boundary_flags"]["not_official_verification"] is True
    assert report_candidate["boundary_flags"]["not_causal_proof"] is True
    assert report_candidate["boundary_flags"]["not_prediction"] is True
    assert report_candidate["boundary_flags"]["not_production_score"] is True
    _assert_runtime_side_effects_false(report_candidate)
    ...
    assert boundary["sandbox_public_event_generated"] is False
    ...
    assert boundary["generated_response_text"] is False
    ...
    assert boundary["runtime_side_effects"]["wrote_evidence_layer"] is False
    assert boundary["runtime_side_effects"]["created_production_case"] is False
    assert boundary["runtime_side_effects"]["created_production_analysis_run"] is False
    assert boundary["runtime_side_effects"]["used_source11_final_summary_report_runtime"] is False
    assert boundary["runtime_side_effects"]["generated_final_summary_report"] is False
    assert boundary["runtime_side_effects"]["generated_b_end_report_runtime"] is False
    assert boundary["runtime_side_effects"]["generated_sandbox_runtime"] is False
    assert boundary["runtime_side_effects"]["generated_public_event_runtime"] is False
    assert boundary["runtime_side_effects"]["generated_export_artifact"] is False
    ...
    assert boundary["runtime_side_effects"]["generated_response_text"] is False
    assert boundary["runtime_side_effects"]["created_public_route"] is False
    assert boundary["downstream_policy"]["source11_ready"] is False
    assert boundary["downstream_policy"]["export_ready"] is False
    assert boundary["downstream_policy"]["download_ready"] is False
    assert boundary["downstream_policy"]["public_access_ready"] is False
    assert boundary["downstream_policy"]["external_delivery_ready"] is False
    assert boundary["downstream_policy"]["b_end_ready"] is False
    assert boundary["downstream_policy"]["sandbox_ready"] is False
    assert boundary["downstream_policy"]["public_event_ready"] is False
    ...
    assert not (FORBIDDEN_OUTPUT_KEYS & _walk_keys(boundary))
...
def test_wrong_or_missing_report_candidate_blocks_boundary(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    report_candidate = _build_report_candidate(tmp_path, monkeypatch)
    variants = [
        dict(report_candidate, report_candidate_schema="unknown"),
        dict(report_candidate, report_candidate_status="blocked_metadata_contract"),
        dict(report_candidate, report_candidate_summary=None),
    ]

    for value in variants:
        boundary = build_report_candidate_final_report_boundary(value)
...
def test_readiness_flags_block_boundary(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    for flag in ("frontend_ready", "route_ready", "production_ready", "customer_ready", "export_ready", "public_ready"):
        report_candidate = _build_report_candidate(tmp_path / flag, monkeypatch)
        report_candidate[flag] = True

        boundary = build_report_candidate_final_report_boundary(report_candidate)
