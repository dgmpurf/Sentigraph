...
from app.services import controlled_production_analysis_result_creation_boundary as module
from app.services.controlled_production_analysis_result_creation_boundary import (
    APPROVAL_PHRASE,
    build_controlled_production_analysis_result_creation_boundary_set,
    build_safe_controlled_production_analysis_result_creation_boundary_summary,
)
...
NON_ASCII_APPROVAL_PHRASE = (
    "\u6279\u51c6 8W-55 Controlled Production Analysis Result Creation Boundary Helper Implementation"
)
MOJIBAKE_APPROVAL_PHRASE = (
    "\u9395\u7470\u567f 8W-55 Controlled Production Analysis Result Creation Boundary Helper Implementation"
)
...
def _source_creation_candidate(**overrides: object) -> dict[str, object]:
    candidate: dict[str, object] = {
        "production_analysis_result_creation_or_runtime_execution_candidate_schema": (
            "sentigraph_controlled_production_analysis_result_creation_or_runtime_execution_candidate_v0_1"
        ),
        "production_analysis_result_creation_or_runtime_execution_candidate_id": (
            "controlled-production-analysis-result-creation-runtime-candidate-001-hash-001"
        ),
        ...
        "input_scope_summary_redacted": (
            "controlled_local_production_analysis_result_creation_or_runtime_execution_candidate_summary_only"
        ),
...
def _valid_creation_candidate_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "production_analysis_result_creation_or_runtime_execution_candidate_count": 1,
        "warning_count": 1,
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "production_analysis_result_runtime_boundary_created_upstream": True,
        "production_analysis_result_creation_or_runtime_execution_candidate_created": True,
        "production_analysis_result_created": False,
        "production_analysis_result_runtime_used": False,
        "analysis_result_generation_executed": False,
        "analysis_result_created": False,
        ...
        "8w53_decision": "ready",
        "8w54_decision": "ready",
        "8w54_selected_next_boundary_option": (
        ...
        "production_analysis_result_creation_or_runtime_execution_candidates": [_source_creation_candidate()],
        "boundary_flags": {
            "backend_only": True,
            "local_only": True,
            "candidate_only": True,
...
def _build_ready(source: dict[str, object] | None = None) -> dict[str, object]:
    return build_controlled_production_analysis_result_creation_boundary_set(
        source or _valid_creation_candidate_set(),
...
def test_ready_path_builds_one_warning_preserving_creation_boundary() -> None:
    result = _build_ready()

    assert result["production_analysis_result_creation_boundary_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_boundary_set_v0_1"
    )
    assert result["production_analysis_result_creation_boundary_set_status"] == (
        "production_analysis_result_creation_boundary_set_warn_manual_review_required"
    )
    assert result["production_analysis_result_creation_boundary_count"] == 1
    assert result["source_production_analysis_result_creation_or_runtime_execution_candidate_set_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_or_runtime_execution_candidate_set_v0_1"
    )
    assert result["source_production_analysis_result_creation_or_runtime_execution_candidate_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_or_runtime_execution_candidate_v0_1"
    )
    ...
    assert result["production_analysis_result_creation_boundary_created"] is True
    ...
    boundaries = result["production_analysis_result_creation_boundaries"]
    assert len(boundaries) == 1
    boundary = boundaries[0]
    assert boundary["production_analysis_result_creation_boundary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_boundary_v0_1"
    )
    assert boundary["creation_boundary_only"] is True
...
def test_safe_summary_without_candidate_list_builds_one_creation_boundary() -> None:
    source = _valid_creation_candidate_set()
    source.pop("production_analysis_result_creation_or_runtime_execution_candidates")
    ...
    assert result["production_analysis_result_creation_boundary_count"] == 1
    assert result["production_analysis_result_creation_boundary_created"] is True
    assert result["production_analysis_result_creation_boundaries"][0][
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("APPROVE_8W_55_CONTROLLED_PRODUCTION_ANALYSIS_RESULT_CREATION_BOUNDARY_HELPER_IMPLEMENTATI0N", "blocked_wrong_exact_approval"),
        (NON_ASCII_APPROVAL_PHRASE, "blocked_non_ascii_approval"),
        (MOJIBAKE_APPROVAL_PHRASE, "blocked_non_ascii_approval"),
    ],
)
def test_exact_approval_phrase_required_before_boundary_construction(
    phrase: str | None,
    expected_status: str,
    ...
    result = build_controlled_production_analysis_result_creation_boundary_set(
        _valid_creation_candidate_set(),
        exact_approval_phrase=phrase,
    )

    assert result["production_analysis_result_creation_boundary_set_status"] == expected_status
    assert result["production_analysis_result_creation_boundary_count"] == 0
    assert result["production_analysis_result_creation_boundary_created"] is False
    assert result["production_analysis_result_creation_boundaries"] == []


@pytest.mark.parametrize(
    ("field", "value"),
    [
        ("production_analysis_result_creation_or_runtime_execution_candidate_set_schema", "wrong"),
        ("production_analysis_result_creation_or_runtime_execution_candidate_set_status", "wrong"),
        ("production_analysis_result_creation_or_runtime_execution_candidate_count", 2),
        ...
        ("8w53_decision", "needs_fix"),
        ("8w54_decision", "needs_fix"),
        ("8w54_selected_next_boundary_option", "pause"),
...
def test_source_candidate_validation_blocks_unsafe_inputs(field: str, value: object) -> None:
    result = _build_ready(_valid_creation_candidate_set(**{field: value}))
...
def test_runtime_side_effect_flags_true_block(flag: str) -> None:
    result = _build_ready(_valid_creation_candidate_set(runtime_side_effects=_runtime_side_effects(**{flag: True})))
...
def test_requested_side_effect_actions_block(requested_action: str) -> None:
    ...
    assert result["production_analysis_result_creation_boundary_set_status"].startswith("blocked_")
    assert result["production_analysis_result_creation_boundary_count"] == 0
...
def test_forbidden_fields_block_and_are_not_emitted(field: str, value: object) -> None:
    source = _valid_creation_candidate_set(**{field: value})
...
def test_missing_source_candidate_blocks() -> None:
    result = _build_ready(
        _valid_creation_candidate_set(production_analysis_result_creation_or_runtime_execution_candidates=[])
    )

    assert result["production_analysis_result_creation_boundary_set_status"] == (
        "blocked_invalid_source_production_analysis_result_creation_or_runtime_execution_candidate"
    )
    assert result["production_analysis_result_creation_boundary_count"] == 0


def test_multiple_boundary_request_blocks() -> None:
    result = build_controlled_production_analysis_result_creation_boundary_set(
        _valid_creation_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        boundary_count=2,
    ...
    assert result["production_analysis_result_creation_boundary_set_status"] == (
        "blocked_creation_boundary_limit_violation"
    )
    assert result["production_analysis_result_creation_boundary_count"] == 0


def test_safe_summary_is_aggregate_only() -> None:
    summary = build_safe_controlled_production_analysis_result_creation_boundary_summary(
        _valid_creation_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == (
        "sentigraph_controlled_production_analysis_result_creation_boundary_summary_v0_1"
    )
    assert summary["production_analysis_result_creation_boundary_count"] == 1
    ...
    assert "production_analysis_result_creation_boundaries" not in summary
