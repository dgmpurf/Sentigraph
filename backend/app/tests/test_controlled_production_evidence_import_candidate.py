...
from app.services.controlled_production_evidence_import_candidate import (
    APPROVAL_PHRASE,
...
EXPECTED_APPROVAL_PHRASE = "批准 8W-22 Controlled Production Evidence Import Candidate Helper Implementation"
MOJIBAKE_APPROVAL_PHRASE = "鎵瑰噯 8W-22 Controlled Production Evidence Import Candidate Helper Implementation"
ALT_MOJIBAKE_APPROVAL_PHRASE = "閹电懓鍣?8W-22 Controlled Production Evidence Import Candidate Helper Implementation"
...
def _write_candidate(index: int, **overrides: object) -> dict[str, object]:
    candidate: dict[str, object] = {
        ...
        "evidence_layer_write_candidate_id": f"evidence-layer-write-candidate-{index:03d}-hash-{index:03d}",
        "source_evidence_layer_import_candidate_id": f"evidence-layer-import-candidate-{index:03d}-hash-{index:03d}",
...
def _valid_write_candidate_set(**overrides: object) -> dict[str, object]:
    ...
    candidate_set: dict[str, object] = {
        ...
        "source_evidence_layer_import_candidate_set_schema": "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1",
        "source_evidence_layer_import_candidate_set_status": "evidence_layer_import_candidate_set_warn_manual_review_required",
        "source_evidence_layer_import_candidate_count": 5,
...
def _assert_blocked(candidate_set: dict[str, object], expected_reason: str) -> None:
    assert str(candidate_set["production_evidence_import_candidate_set_status"]).startswith("blocked_")
    assert candidate_set["production_evidence_import_candidate_helper_implementation_approved"] is False
    assert candidate_set["production_evidence_import_candidate_created"] is False
    assert candidate_set["production_evidence_import_candidate_count"] == 0
    assert candidate_set["production_evidence_import_candidates"] == []
...
def test_ready_path_builds_local_production_evidence_import_candidates_without_production_side_effects() -> None:
    ...
    assert candidate_set["production_evidence_import_candidate_set_schema"] == (
        "sentigraph_controlled_production_evidence_import_candidate_set_v0_1"
    )
    assert candidate_set["phase"] == "8W-22"
    assert candidate_set["production_evidence_import_candidate_set_status"] == (
        "production_evidence_import_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["input_source_kind"] == "controlled_evidence_layer_write_candidate_set"
    assert candidate_set["source_evidence_layer_write_candidate_set_schema"] == (
        "sentigraph_controlled_evidence_layer_write_candidate_set_v0_1"
    )
    assert candidate_set["source_evidence_layer_write_candidate_set_status"] == (
        "evidence_layer_write_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["source_evidence_layer_write_candidate_count"] == 5
    assert candidate_set["production_evidence_import_candidate_mode"] == (
        "backend_only_local_production_evidence_import_candidate_boundary"
    )
    assert candidate_set["production_evidence_import_candidate_count"] == 5
    ...
    assert candidate_set["production_import_candidate_only"] is True
    assert candidate_set["production_evidence_import_candidate_helper_implementation_approved"] is True
    assert candidate_set["production_evidence_import_candidate_created"] is True
    assert candidate_set["evidence_item_created"] is False
    assert candidate_set["evidence_items_created"] is False
    assert candidate_set["production_evidence_item_created"] is False
    assert candidate_set["evidence_layer_write"] is False
    ...
    for candidate in candidate_set["production_evidence_import_candidates"]:
...
def test_exact_approval_required_before_candidate_creation_and_file_access(
    ...
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-22 must not open files")
...
@pytest.mark.parametrize(
    ("field", "value", "reason"),
    [
        ("evidence_layer_write_candidate_set_schema", "wrong", "source_evidence_layer_write_candidate_set_schema_wrong"),
        ...
        (
            ...
            4,
            "source_evidence_layer_import_candidate_count_less_than_evidence_layer_write_candidate_count",
        ),
        ...
        ("evidence_layer_write_candidate_created", False, "source_evidence_layer_write_candidate_created_not_true"),
        ("production_evidence_import_candidate_created", True, "source_production_evidence_import_candidate_created_true"),
...
def test_source_validation_blocks_unsafe_write_candidate_set_fields(field: str, value: object, reason: str) -> None:
    candidate_set = build_controlled_production_evidence_import_candidate_set(
...
def test_candidate_limit_cannot_exceed_source_candidate_count_or_hard_bound() -> None:
    exceeds_source = build_controlled_production_evidence_import_candidate_set(
        _valid_write_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=6,
    )
    _assert_blocked(exceeds_source, "blocked_candidate_limit_exceeds_source_evidence_layer_write_candidate_count")

    many_candidates = [_write_candidate(index) for index in range(1, 12)]
    exceeds_bound = build_controlled_production_evidence_import_candidate_set(
        _valid_write_candidate_set(
            evidence_layer_write_candidates=many_candidates,
            evidence_layer_write_candidate_count=11,
            source_evidence_layer_import_candidate_count=11,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=11,
    )
...
def test_forbidden_fields_in_source_candidates_block_or_never_emit() -> None:
    ...
    _assert_blocked(candidate_set, "forbidden_source_evidence_layer_write_candidate_field:raw_author_id")
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_production_evidence_import_candidate_summary(
        _valid_write_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == "sentigraph_controlled_production_evidence_import_candidate_summary_v0_1"
    assert summary["phase"] == "8W-22"
    assert summary["production_evidence_import_candidate_set_schema"] == (
        "sentigraph_controlled_production_evidence_import_candidate_set_v0_1"
    )
    assert summary["production_evidence_import_candidate_set_status"] == (
        "production_evidence_import_candidate_set_warn_manual_review_required"
    )
    assert summary["production_evidence_import_candidate_count"] == 5
    assert summary["source_evidence_layer_write_candidate_count"] == 5
    assert summary["warning_count"] == 1
    assert summary["human_review_required"] is True
    assert "production_evidence_import_candidates" not in summary
