...
from app.services import controlled_analysis_result_candidate as module
from app.services.controlled_analysis_result_candidate import (
    APPROVAL_PHRASE,
    build_controlled_analysis_result_candidate_set,
    build_safe_controlled_analysis_result_candidate_summary,
)


EXPECTED_APPROVAL_PHRASE = "APPROVE_8W_40_CONTROLLED_ANALYSIS_RESULT_CANDIDATE_HELPER_IMPLEMENTATION"
NON_ASCII_APPROVAL_PHRASE = (
    "\u6279\u51c6 8W-40 Controlled Analysis Result Candidate Helper Implementation"
)
...
def _source_actual_analysis_execution_candidate(**overrides: object) -> dict[str, object]:
...
def _valid_source_actual_analysis_execution_candidate_set(**overrides: object) -> dict[str, object]:
    source: dict[str, object] = {
        ...
        "source_production_case_candidate_count": 1,
        "source_controlled_evidence_item_count": 5,
        "actual_analysis_execution_candidate_mode": (
        ...
        "human_review_required": True,
        "no_automatic_trust_upgrade": True,
        "production_analysis_run_candidate_created_upstream": True,
        "actual_analysis_execution_candidate_created": True,
        "actual_analysis_execution_candidate_only": True,
        "actual_analysis_execution_started": False,
        "analysis_execution_started": False,
        "analysis_result_created": False,
        ...
        "actual_analysis_execution_candidates": [_source_actual_analysis_execution_candidate()],
    }
...
def _assert_blocked(candidate_set: dict[str, object], expected_reason: str) -> None:
    assert str(candidate_set["analysis_result_candidate_set_status"]).startswith("blocked_")
    assert expected_reason in candidate_set["blockers"]
    assert candidate_set["analysis_result_candidate_created"] is False
    assert candidate_set["analysis_result_candidate_count"] == 0
    assert candidate_set["analysis_result_candidates"] == []
...
def test_ready_path_builds_one_controlled_analysis_result_candidate() -> None:
    ...
    candidate_set = build_controlled_analysis_result_candidate_set(
        _valid_source_actual_analysis_execution_candidate_set(),
        exact_approval_phrase=EXPECTED_APPROVAL_PHRASE,
    )

    assert candidate_set["analysis_result_candidate_set_schema"] == (
        "sentigraph_controlled_analysis_result_candidate_set_v0_1"
    )
    assert candidate_set["phase"] == "8W-40"
    assert candidate_set["analysis_result_candidate_set_status"] == (
        "analysis_result_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["input_source_kind"] == "controlled_actual_analysis_execution_candidate"
    assert candidate_set["source_actual_analysis_execution_candidate_set_schema"] == (
        "sentigraph_controlled_actual_analysis_execution_candidate_set_v0_1"
    )
    assert candidate_set["source_actual_analysis_execution_candidate_schema"] == (
        "sentigraph_controlled_actual_analysis_execution_candidate_v0_1"
    )
    ...
    assert candidate_set["analysis_result_candidate_mode"] == (
        "backend_only_local_analysis_result_candidate_boundary"
    )
    assert candidate_set["analysis_result_candidate_count"] == 1
    ...
    assert candidate_set["actual_analysis_execution_candidate_created_upstream"] is True
    assert candidate_set["analysis_result_candidate_created"] is True
    assert candidate_set["analysis_result_generation_executed"] is False
    assert candidate_set["analysis_result_created"] is False
    assert candidate_set["production_analysis_result_created"] is False
    assert candidate_set["actual_analysis_execution_started"] is False
    ...
    candidates = candidate_set["analysis_result_candidates"]
    assert isinstance(candidates, list)
    assert len(candidates) == 1
    candidate = candidates[0]
    allowed_fields = {
        "analysis_result_candidate_schema",
        "analysis_result_candidate_id",
    ...
    assert candidate["analysis_result_candidate_schema"] == (
    ...
    assert candidate["analysis_result_candidate_only"] is True
    ...
    assert candidate["boundary_flags"]["analysis_result_candidate_only"] is True
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "APPROVE_8W_40_CONTROLLED_ANALYSIS_RESULT_CANDIDATE_HELPER_IMPLEMENTATI0N",
...
def test_exact_ascii_approval_required_before_candidate_construction_and_file_access(
    ...
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-40 must not open files")

    def blocked_candidate(*args, **kwargs):
        raise AssertionError("8W-40 must not construct candidates on wrong approval")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            "actual_analysis_execution_candidate_set_status",
            "actual_analysis_execution_candidate_set_ready",
        ...
        (
            ...
            False,
            "source_actual_analysis_execution_candidate_created_not_true",
        ),
...
def test_source_validation_blocks_unsafe_candidate_set_fields(field: str, value: object, reason: str) -> None:
    candidate_set = build_controlled_analysis_result_candidate_set(
        _valid_source_actual_analysis_execution_candidate_set(**{field: value}),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "source_runtime_side_effect_true:created_production_analysis_result",
        ),
...
def test_source_runtime_side_effects_block(runtime_flag: str, reason: str) -> None:
    source = _valid_source_actual_analysis_execution_candidate_set()
...
def test_candidate_count_blocks_when_source_candidates_absent_or_malformed() -> None:
    absent = build_controlled_analysis_result_candidate_set(
        _valid_source_actual_analysis_execution_candidate_set(actual_analysis_execution_candidates=[]),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(absent, "source_actual_analysis_execution_candidates_count_not_one")

    malformed = build_controlled_analysis_result_candidate_set(
        _valid_source_actual_analysis_execution_candidate_set(
            actual_analysis_execution_candidates=[{"wrong": "shape"}]
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(malformed, "source_actual_analysis_execution_candidate_schema_wrong")


def test_forbidden_fields_in_source_candidate_block_or_never_emit() -> None:
    unsafe_candidate = _source_actual_analysis_execution_candidate(
    ...
    candidate_set = build_controlled_analysis_result_candidate_set(
        _valid_source_actual_analysis_execution_candidate_set(
            actual_analysis_execution_candidates=[unsafe_candidate]
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    _assert_blocked(candidate_set, "forbidden_source_actual_analysis_execution_candidate_field:analysis_result_id")
...
def test_requested_side_effects_block_and_keep_flags_false(requested_action: str) -> None:
    ...
    assert candidate_set["analysis_result_candidate_set_status"].startswith("blocked_")
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_analysis_result_candidate_summary(
        _valid_source_actual_analysis_execution_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == "sentigraph_controlled_analysis_result_candidate_summary_v0_1"
    assert summary["phase"] == "8W-40"
    assert summary["analysis_result_candidate_set_schema"] == (
        "sentigraph_controlled_analysis_result_candidate_set_v0_1"
    )
    assert summary["analysis_result_candidate_set_status"] == (
        "analysis_result_candidate_set_warn_manual_review_required"
    )
    assert summary["analysis_result_candidate_count"] == 1
    ...
    assert summary["analysis_result_candidate_created"] is True
    ...
    assert "analysis_result_candidates" not in summary
    assert "source_actual_analysis_execution_candidate_id" not in _serialized(summary)
