...
ROW_SENTINEL = "ROW_SENTINEL_SHOULD_NOT_BE_READ_OR_EXPOSED_8X6"
RAW_IDENTITY_SENTINEL = "RAW_IDENTITY_SHOULD_NOT_BE_EXPOSED_8X6"
ROW_LIKE_FILES = {
...
def _provider_result_payload(package_name: str = "synthetic_package") -> dict:
    return {
        ...
        "provider_result_id": "provider_result_8x6_fixture",
        "provider_job_id": "provider_job_8x6_fixture",
        "request_id": "analysis_request_8x6_fixture",
...
def _build_review_only_staging_summary(provider_result_path: Path, export_root: Path) -> dict:
    ...
    candidate = create_review_only_staging_candidate(handoff_summary, requested_by="8x6_fixture_metadata_smoke")
...
def _has_required_fixture_metadata_missing(generated_run: dict) -> bool:
    blockers = generated_run.get("blockers")
    if not isinstance(blockers, list):
        return False
    return any(
        isinstance(blocker, dict) and blocker.get("reason") == "required_fixture_metadata_missing"
        for blocker in blockers
    )


def test_synthetic_stage_metadata_clears_required_fixture_metadata_missing_without_trust_upgrade(
    tmp_path: Path,
    ...
    def guarded_read_text(self: Path, *args, **kwargs):
        if self.name in ROW_LIKE_FILES:
            raise AssertionError(f"{self.name} must not be opened or parsed in 8X-6 metadata bridge smoke")
        return original_read_text(self, *args, **kwargs)

    def fail_dense_graph(*args, **kwargs):
        raise AssertionError("dense graph must not be called in 8X-6")

    def fail_report_candidate(*args, **kwargs):
        raise AssertionError("report candidate must not be created in 8X-6")
    ...
    bridge = build_staging_candidate_generated_run_bridge(staging_summary, created_by="8x6_fixture_metadata_smoke")
    bridge["minimum_real_run_input_candidate"]["fixture_metadata"]["stage_id"] = "T0"

    execution = execute_minimum_real_run_from_bridge_candidate(bridge, created_by="8x6_fixture_metadata_smoke")
    ...
    assert _has_required_fixture_metadata_missing(generated_run) is False
