...
EXISTING_PRODUCTION_CASE_HELPER_PHRASE = (
    "APPROVE_8W_31_CONTROLLED_PRODUCTION_CASE_CANDIDATE_HELPER_IMPLEMENTATION"
)
MOJIBAKE_PRODUCTION_CASE_HELPER_PHRASE = (
    "\u9395\u7470\u567f 8W-31 Controlled Production Case Candidate Helper Implementation"
)

EXPECTED_SOURCE_RUNTIME_SCHEMA = "sentigraph_controlled_evidenceitem_evidence_layer_write_runtime_v0_1"
EXPECTED_SOURCE_CANDIDATE_SET_SCHEMA = (
...
SOURCE_TRUE_BLOCKERS = {
    ...
    "new_evidence_layer_write_performed": "source_new_evidence_layer_write_performed_true",
    ...
    "raw_author_ids_emitted": "source_raw_author_ids_emitted_true",
    "raw_author_names_emitted": "source_raw_author_names_emitted_true",
    "profile_urls_emitted": "source_profile_urls_emitted_true",
...
def _fail_if_called(*args: object, **kwargs: object) -> None:
    raise AssertionError("8Y-16 must not call production runtime, route, frontend, or delivery helpers")
...
def _patch_file_open_blocked(monkeypatch: pytest.MonkeyPatch) -> None:
    def blocked_open(*args: object, **kwargs: object) -> None:
        raise AssertionError("8Y-16 controlled smoke must not open files")

    monkeypatch.setattr(builtins, "open", blocked_open)
    monkeypatch.setattr(Path, "open", blocked_open)


def _safe_8y14_evidenceitem_runtime() -> dict[str, Any]:
    ...
    assert smoke["evidence_write_result_schema"] == EXPECTED_SOURCE_RUNTIME_SCHEMA
    assert smoke["source_production_import_derived_write_candidate_schema"] == (
        EXPECTED_SOURCE_CANDIDATE_SET_SCHEMA
    )
    runtime = copy.deepcopy(smoke["helper_runtime"])
    ...
    for field in (
        ...
        runtime.setdefault(field, False)
    return runtime


def _blocked_8y16_output(reason: str, source_runtime: dict[str, Any] | None = None) -> dict[str, Any]:
    return {
        "schema": "sentigraph_8y16_controlled_production_case_candidate_smoke_v0_1",
        "phase": "8Y-16",
        "smoke_status": "blocked",
        "source_path_step": "evidenceitem_write_result_to_production_case_candidate",
        "outer_8y16_phrase": None,
        "production_case_candidate_created": False,
        "production_case_candidate_schema": None,
        "production_case_candidate_mode": None,
        "source_evidence_write_result_schema": (
            source_runtime.get("runtime_schema") if isinstance(source_runtime, dict) else None
        ),
        "source_production_import_derived_write_candidate_schema": (
            source_runtime.get("source_candidate_set_schema") if isinstance(source_runtime, dict) else None
        ),
        "candidate_only": True,
        "backend_only": True,
        "local_only": True,
...
def _source_blockers(source_runtime: dict[str, Any] | None) -> list[str]:
    if not isinstance(source_runtime, dict):
        return ["source_evidenceitem_write_runtime_missing_or_not_object"]

    blockers: list[str] = []
    if source_runtime.get("runtime_schema") != EXPECTED_SOURCE_RUNTIME_SCHEMA:
        blockers.append("source_evidence_write_result_schema_wrong")
    if source_runtime.get("source_candidate_set_schema") != EXPECTED_SOURCE_CANDIDATE_SET_SCHEMA:
        blockers.append("source_production_import_derived_write_candidate_schema_wrong")
    if source_runtime.get("human_review_required") is not True:
        blockers.append("source_human_review_required_not_true")
    if source_runtime.get("no_automatic_trust_upgrade") is not True:
        blockers.append("source_no_automatic_trust_upgrade_not_true")
    for field, reason in SOURCE_TRUE_BLOCKERS.items():
        if source_runtime.get(field) is True:
            blockers.append(reason)
    return blockers


def _build_8y16_smoke(
    *,
    ...
    if outer_approval_phrase is None or outer_approval_phrase == "":
        return _blocked_8y16_output("blocked_missing_exact_8y16_approval")
    if outer_approval_phrase != EIGHT_Y16_APPROVAL_PHRASE:
        return _blocked_8y16_output("blocked_wrong_exact_8y16_approval")

    source = source_runtime or _safe_8y14_evidenceitem_runtime()
    source_blockers = _source_blockers(source)
    if source_blockers:
        return _blocked_8y16_output(source_blockers[0], source)

    candidate_set = production_case_module.build_controlled_production_case_candidate_set(
        source,
        exact_approval_phrase=production_case_module.APPROVAL_PHRASE,
    )
    candidates = candidate_set["production_case_candidates"]
    candidate = candidates[0] if candidates else {}
    created = candidate_set["production_case_candidate_created"] is True
    return {
        "schema": "sentigraph_8y16_controlled_production_case_candidate_smoke_v0_1",
        "phase": "8Y-16",
        "smoke_status": "ready" if created else "blocked",
        "source_path_step": "evidenceitem_write_result_to_production_case_candidate",
        "outer_8y16_phrase": EIGHT_Y16_APPROVAL_PHRASE,
        "production_case_candidate_created": created,
        "production_case_candidate_schema": candidate.get("production_case_candidate_schema"),
        "production_case_candidate_mode": candidate_set["production_case_candidate_mode"],
        "source_evidence_write_result_schema": candidate_set["source_runtime_schema"],
        "source_production_import_derived_write_candidate_schema": source.get("source_candidate_set_schema"),
        "candidate_only": candidate_set["production_case_candidate_only"],
...
def _assert_blocked_before_candidate_helper(
    smoke: dict[str, Any],
    expected_reason: str,
) -> None:
    assert smoke["smoke_status"] == "blocked"
    assert smoke["production_case_candidate_created"] is False
    assert smoke["production_case_candidate_schema"] is None
    assert smoke["helper_candidate_set"] is None
    assert smoke["blockers"] == [expected_reason]
    _assert_false_side_effects(smoke)


def test_8y16_ready_path_builds_local_production_case_candidate_only(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert production_case_module.APPROVAL_PHRASE == EXISTING_PRODUCTION_CASE_HELPER_PHRASE
    assert production_case_module.APPROVAL_PHRASE.isascii()
    ...
    assert smoke["production_case_candidate_mode"] == "backend_only_local_production_case_candidate_boundary"
    assert smoke["source_evidence_write_result_schema"] == EXPECTED_SOURCE_RUNTIME_SCHEMA
    assert smoke["source_production_import_derived_write_candidate_schema"] == EXPECTED_SOURCE_CANDIDATE_SET_SCHEMA
...
def test_exact_8y16_phrase_required_before_production_case_candidate_helper(
    ...
    smoke = _build_8y16_smoke(outer_approval_phrase=outer_phrase)

    expected = "blocked_missing_exact_8y16_approval" if outer_phrase in {None, ""} else (
        "blocked_wrong_exact_8y16_approval"
    )
    _assert_blocked_before_candidate_helper(smoke, expected)


def test_existing_production_case_helper_rejects_mojibake_phrase() -> None:
    assert production_case_module.APPROVAL_PHRASE == EXISTING_PRODUCTION_CASE_HELPER_PHRASE
    assert production_case_module.APPROVAL_PHRASE.isascii()
    assert MOJIBAKE_PRODUCTION_CASE_HELPER_PHRASE != production_case_module.APPROVAL_PHRASE

    candidate_set = production_case_module.build_controlled_production_case_candidate_set(
        _safe_8y14_evidenceitem_runtime(),
        exact_approval_phrase=MOJIBAKE_PRODUCTION_CASE_HELPER_PHRASE,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("runtime_schema", "wrong", "source_evidence_write_result_schema_wrong"),
        (
            "source_candidate_set_schema",
            "wrong",
            "source_production_import_derived_write_candidate_schema_wrong",
        ),
        ...
        ("new_evidence_layer_write_performed", True, "source_new_evidence_layer_write_performed_true"),
...
def test_unsafe_source_blocks_before_production_case_candidate_helper(
    field: str,
    value: object,
    expected_reason: str,
    monkeypatch: pytest.MonkeyPatch,
    ...
    _patch_forbidden_runtime_entrypoints(monkeypatch)
    source = _safe_8y14_evidenceitem_runtime()
    source[field] = value

    smoke = _build_8y16_smoke(
        outer_approval_phrase=EIGHT_Y16_APPROVAL_PHRASE,
        source_runtime=source,
    )

    _assert_blocked_before_candidate_helper(smoke, expected_reason)


def test_requested_production_case_or_analysis_actions_block_at_existing_helper_layer() -> None:
    candidate_set = production_case_module.build_controlled_production_case_candidate_set(
        _safe_8y14_evidenceitem_runtime(),
        exact_approval_phrase=production_case_module.APPROVAL_PHRASE,
        requested_actions=["production_case", "production_analysis_run"],
    )

    assert candidate_set["production_case_candidate_created"] is False
    assert "requested_action_blocked:production_case" in candidate_set["blockers"]
...
def test_8y16_smoke_output_is_json_serializable_without_raw_or_absolute_values() -> None:
    smoke = _build_8y16_smoke(outer_approval_phrase=EIGHT_Y16_APPROVAL_PHRASE)
