...
from app.services.controlled_evidence_layer_write_candidate_from_production_import_candidate import (
    APPROVAL_PHRASE,
    build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set,
    build_safe_controlled_evidence_layer_write_candidate_from_production_import_candidate_summary,
)


EXPECTED_APPROVAL_PHRASE = "批准 8W-25 Controlled Evidence Layer Write Candidate Helper Implementation"
MOJIBAKE_APPROVAL_PHRASE = "鎵瑰噯 8W-25 Controlled Evidence Layer Write Candidate Helper Implementation"
...
def _production_import_candidate(index: int, **overrides: object) -> dict[str, object]:
...
def _valid_production_import_candidate_set(**overrides: object) -> dict[str, object]:
    candidates = [_production_import_candidate(index) for index in range(1, 6)]
    candidate_set: dict[str, object] = {
        ...
        "source_evidence_layer_write_candidate_set_status": (
        ...
        "write_candidate_only": True,
        "production_import_candidate_only": True,
        "production_evidence_import_candidate_helper_implementation_approved": True,
        ...
        "production_evidence_import_candidates": candidates,
        "blockers": [],
...
def test_ready_path_builds_local_evidence_layer_write_candidates_from_production_import_candidates() -> None:
    ...
    candidate_set = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(),
    ...
    assert candidate_set["phase"] == "8W-25"
    assert candidate_set["evidence_layer_write_candidate_set_status"] == (
        "evidence_layer_write_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["input_source_kind"] == "controlled_production_evidence_import_candidate_set"
    assert candidate_set["source_production_evidence_import_candidate_set_schema"] == (
        "sentigraph_controlled_production_evidence_import_candidate_set_v0_1"
    )
    assert candidate_set["source_production_evidence_import_candidate_set_status"] == (
        "production_evidence_import_candidate_set_warn_manual_review_required"
    )
    assert candidate_set["source_production_evidence_import_candidate_count"] == 5
    assert candidate_set["evidence_layer_write_candidate_mode"] == (
    ...
    assert candidate_set["evidence_layer_write_candidate_only"] is True
...
def test_exact_approval_required_before_candidate_creation_and_file_access(
    ...
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-25 must not open files")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("phase", "wrong", "source_production_evidence_import_candidate_set_phase_wrong"),
        (
            "production_evidence_import_candidate_set_status",
            "production_evidence_import_candidate_set_ready_for_manual_review",
            "source_production_evidence_import_candidate_set_status_not_warn_manual_review",
        ),
        (
            "production_evidence_import_candidate_count",
            4,
            "source_production_evidence_import_candidate_count_inconsistent",
        ),
        (
        ...
        (
            ...
            True,
            "source_evidence_layer_write_candidate_created_true",
        ),
...
def test_source_validation_blocks_unsafe_production_import_candidate_set_fields(
    field: str,
    value: object,
    reason: str,
) -> None:
    candidate_set = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(**{field: value}),
...
def test_source_runtime_side_effects_block(runtime_flag: str, reason: str) -> None:
    source = _valid_production_import_candidate_set()
...
def test_candidate_limit_cannot_exceed_source_candidate_count_or_hard_bound() -> None:
    exceeds_source = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=6,
    )
    _assert_blocked(exceeds_source, "blocked_candidate_limit_exceeds_source_production_evidence_import_candidate_count")

    many_candidates = [_production_import_candidate(index) for index in range(1, 12)]
    exceeds_bound = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(
            production_evidence_import_candidates=many_candidates,
            production_evidence_import_candidate_count=11,
            source_evidence_layer_write_candidate_count=11,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=11,
    )
...
def test_candidates_absent_or_malformed_block() -> None:
    missing = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(
            production_evidence_import_candidates=[],
            production_evidence_import_candidate_count=0,
            source_evidence_layer_write_candidate_count=0,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(missing, "source_production_evidence_import_candidates_missing")

    malformed = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(
            production_evidence_import_candidates=[{"production_evidence_import_candidate_id": "candidate-001"}],
            production_evidence_import_candidate_count=1,
            source_evidence_layer_write_candidate_count=1,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(malformed, "source_production_evidence_import_candidate_missing_required_field:evidence_id_hash")


def test_forbidden_fields_in_source_candidates_block_or_never_emit() -> None:
    unsafe_candidate = _production_import_candidate(
        1,
    ...
    candidate_set = build_controlled_evidence_layer_write_candidate_from_production_import_candidate_set(
        _valid_production_import_candidate_set(
            production_evidence_import_candidates=[unsafe_candidate],
            production_evidence_import_candidate_count=1,
            source_evidence_layer_write_candidate_count=1,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    ...
    _assert_blocked(candidate_set, "forbidden_source_production_evidence_import_candidate_field:raw_author_id")
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_evidence_layer_write_candidate_from_production_import_candidate_summary(
        _valid_production_import_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    ...
    assert summary["phase"] == "8W-25"
    assert summary["evidence_layer_write_candidate_set_schema"] == (
        "sentigraph_controlled_evidence_layer_write_candidate_from_production_import_candidate_set_v0_1"
    )
    assert summary["evidence_layer_write_candidate_set_status"] == (
        "evidence_layer_write_candidate_set_warn_manual_review_required"
    )
    assert summary["evidence_layer_write_candidate_count"] == 5
    assert summary["source_production_evidence_import_candidate_count"] == 5
    ...
    assert "production_evidence_item_id" not in _serialized(summary)
