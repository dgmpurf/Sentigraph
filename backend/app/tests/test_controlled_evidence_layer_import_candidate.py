...
from app.services.controlled_evidence_layer_import_candidate import (
    APPROVAL_PHRASE,
    build_controlled_evidence_layer_import_candidate_set,
    build_safe_controlled_evidence_layer_import_candidate_summary,
)


EXPECTED_APPROVAL_PHRASE = "APPROVE_8W_16_CONTROLLED_EVIDENCE_LAYER_IMPORT_CANDIDATE_IMPLEMENTATION"
OLD_CHINESE_APPROVAL_PHRASE = "批准 8W-16 Controlled Evidence Layer Import Candidate Helper Implementation"
MOJIBAKE_APPROVAL_PHRASE = "鎵瑰噯 8W-16 Controlled Evidence Layer Import Candidate Helper Implementation"
ALT_MOJIBAKE_APPROVAL_PHRASE = "閹电懓鍣?8W-16 Controlled Evidence Layer Import Candidate Helper Implementation"
...
def _review_queue_candidate(index: int, **overrides: object) -> dict[str, object]:
    candidate: dict[str, object] = {
        "review_queue_candidate_schema": "sentigraph_controlled_review_queue_candidate_v0_1",
        "review_queue_candidate_id": f"review-queue-candidate-{index:03d}-hash-{index:03d}",
...
def _valid_review_queue_candidate_set(**overrides: object) -> dict[str, object]:
    candidates = [_review_queue_candidate(index) for index in range(1, 6)]
    candidate_set: dict[str, object] = {
        ...
        "source_candidate_set_schema": "sentigraph_controlled_evidence_candidate_set_v0_1",
        "source_candidate_set_status": "evidence_candidate_set_warn_manual_review_required",
        ...
        "preview_only": True,
        "queue_candidate_only": True,
        "review_queue_candidate_helper_implementation_approved": True,
        ...
        "review_queue_candidates": candidates,
        "blockers": [],
...
def _assert_blocked(candidate_set: dict[str, object], expected_reason: str) -> None:
    assert str(candidate_set["evidence_layer_import_candidate_set_status"]).startswith("blocked_")
    assert candidate_set["evidence_layer_import_candidate_helper_implementation_approved"] is False
    assert candidate_set["evidence_layer_import_candidate_created"] is False
    assert candidate_set["evidence_layer_import_candidate_count"] == 0
    assert candidate_set["evidence_layer_import_candidates"] == []
...
def test_ready_path_builds_local_evidence_layer_import_candidates_without_production_side_effects() -> None:
    assert APPROVAL_PHRASE == EXPECTED_APPROVAL_PHRASE

    candidate_set = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(),
        exact_approval_phrase=EXPECTED_APPROVAL_PHRASE,
    )

    assert candidate_set["evidence_layer_import_candidate_set_schema"] == "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1"
    assert candidate_set["phase"] == "8W-16"
    assert candidate_set["evidence_layer_import_candidate_set_status"] == "evidence_layer_import_candidate_set_warn_manual_review_required"
    assert candidate_set["input_source_kind"] == "controlled_review_queue_candidate_set"
    assert candidate_set["source_review_queue_candidate_set_schema"] == "sentigraph_controlled_review_queue_candidate_set_v0_1"
    assert candidate_set["source_review_queue_candidate_set_status"] == "review_queue_candidate_set_warn_manual_review_required"
    assert candidate_set["source_review_queue_candidate_count"] == 5
    assert candidate_set["evidence_layer_import_candidate_mode"] == "backend_only_local_evidence_layer_import_candidate_boundary"
    assert candidate_set["evidence_layer_import_candidate_count"] == 5
    ...
    assert candidate_set["evidence_layer_import_candidate_helper_implementation_approved"] is True
    assert candidate_set["evidence_layer_import_candidate_created"] is True
    assert candidate_set["evidence_item_created"] is False
    assert candidate_set["evidence_items_created"] is False
    assert candidate_set["production_evidence_item_created"] is False
    assert candidate_set["evidence_layer_write"] is False
    ...
    for candidate in candidate_set["evidence_layer_import_candidates"]:
        ...
        assert candidate["evidence_layer_import_candidate_schema"] == "sentigraph_controlled_evidence_layer_import_candidate_v0_1"
        assert candidate["source_review_queue_candidate_set_schema"] == "sentigraph_controlled_review_queue_candidate_set_v0_1"
        assert candidate["source_review_queue_candidate_schema"] == "sentigraph_controlled_review_queue_candidate_v0_1"
...
def test_exact_approval_required_before_candidate_creation_and_file_access(
    ...
    def blocked_open(*args, **kwargs):
        raise AssertionError("8W-16 must not open files")
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("phase", "wrong", "source_review_queue_candidate_set_phase_wrong"),
        ("review_queue_candidate_set_status", "review_queue_candidate_set_ready_for_manual_review", "source_review_queue_candidate_set_status_not_warn_manual_review"),
        ("review_queue_candidate_count", 4, "source_review_queue_candidate_count_inconsistent"),
        ("source_evidence_candidate_count", 4, "source_evidence_candidate_count_less_than_review_queue_candidate_count"),
        ...
        ("queue_candidate_only", False, "source_queue_candidate_only_not_true"),
        ("review_queue_candidate_created", False, "source_review_queue_candidate_created_not_true"),
...
def test_source_validation_blocks_unsafe_review_queue_candidate_set_fields(field: str, value: object, reason: str) -> None:
    candidate_set = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(**{field: value}),
...
def test_source_runtime_side_effects_block(runtime_flag: str, reason: str) -> None:
    source = _valid_review_queue_candidate_set()
...
def test_candidate_limit_cannot_exceed_source_candidate_count_or_hard_bound() -> None:
    exceeds_source = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=6,
    )
    _assert_blocked(exceeds_source, "blocked_candidate_limit_exceeds_source_review_queue_candidate_count")

    many_candidates = [_review_queue_candidate(index) for index in range(1, 12)]
    exceeds_bound = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(
            review_queue_candidates=many_candidates,
            review_queue_candidate_count=11,
            source_candidate_count=11,
            source_evidence_candidate_count=11,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
        candidate_limit=11,
    )
...
def test_candidates_absent_or_malformed_block() -> None:
    missing = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(
            review_queue_candidates=[],
            review_queue_candidate_count=0,
            source_candidate_count=0,
            source_evidence_candidate_count=0,
        ),
        exact_approval_phrase=APPROVAL_PHRASE,
    )
    _assert_blocked(missing, "source_review_queue_candidates_missing")

    malformed = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(
            review_queue_candidates=[{"review_queue_candidate_id": "candidate-001"}],
    ...
    _assert_blocked(malformed, "source_review_queue_candidate_missing_required_field:evidence_id_hash")


def test_forbidden_fields_in_source_candidates_block_or_never_emit() -> None:
    unsafe_candidate = _review_queue_candidate(
        1,
    ...
    candidate_set = build_controlled_evidence_layer_import_candidate_set(
        _valid_review_queue_candidate_set(
            review_queue_candidates=[unsafe_candidate],
            review_queue_candidate_count=1,
            source_candidate_count=1,
            source_evidence_candidate_count=1,
        ),
    ...
    _assert_blocked(candidate_set, "forbidden_source_review_queue_candidate_field:raw_author_id")
...
def test_safe_summary_is_counts_and_boundaries_only() -> None:
    summary = build_safe_controlled_evidence_layer_import_candidate_summary(
        _valid_review_queue_candidate_set(),
        exact_approval_phrase=APPROVAL_PHRASE,
    )

    assert summary["summary_schema"] == "sentigraph_controlled_evidence_layer_import_candidate_summary_v0_1"
    assert summary["phase"] == "8W-16"
    assert summary["evidence_layer_import_candidate_set_schema"] == "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1"
    assert summary["evidence_layer_import_candidate_set_status"] == "evidence_layer_import_candidate_set_warn_manual_review_required"
    assert summary["evidence_layer_import_candidate_count"] == 5
    assert summary["source_review_queue_candidate_count"] == 5
    assert summary["warning_count"] == 1
    assert summary["human_review_required"] is True
    assert "evidence_layer_import_candidates" not in summary
