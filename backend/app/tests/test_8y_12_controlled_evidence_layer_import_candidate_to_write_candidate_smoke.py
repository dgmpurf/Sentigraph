...
SOURCE_IMPORT_BLOCKED_TRUE_FLAGS = {
...
def _safe_import_candidate_set() -> dict[str, Any]:
    ...
    import_candidate_set["no_automatic_trust_upgrade"] = True
    import_candidate_set["actual_evidence_layer_write_used"] = False
    import_candidate_set["persisted_evidence_layer_record_created"] = False
    import_candidate_set["actual_review_queue_runtime_used"] = False
    import_candidate_set["raw_rows_exposed"] = False
    import_candidate_set["raw_comments_exposed"] = False
    import_candidate_set["raw_identities_exposed"] = False
    import_candidate_set["raw_author_ids_emitted"] = False
    import_candidate_set["raw_author_names_emitted"] = False
    import_candidate_set["profile_urls_emitted"] = False
    import_candidate_set["author_names_or_profile_urls_exposed"] = False
    import_candidate_set["secrets_read"] = False
    return import_candidate_set
...
def _blocked_8y12_output(reason: str) -> dict[str, Any]:
    return {
        "schema": "sentigraph_8y12_import_candidate_to_write_candidate_smoke_v0_1",
        "phase": "8Y-12",
        ...
        "evidence_layer_write_candidate_schema": None,
        "write_candidate_mode": None,
        "source_import_candidate_set_schema": None,
        "source_import_candidate_schema": None,
        "source_import_candidate_count": 0,
        ...
        "raw_author_ids_emitted": False,
        "raw_author_names_emitted": False,
        "profile_urls_emitted": False,
        "author_names_or_profile_urls_exposed": False,
        "secrets_read": False,
    }


def _source_import_gate_blockers(source_candidate_set: dict[str, Any]) -> list[str]:
    blockers: list[str] = []
    if (
        source_candidate_set.get("evidence_layer_import_candidate_set_schema")
        != "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1"
    ):
        blockers.append("source_import_candidate_set_schema_wrong")
    ...
    for flag in SOURCE_IMPORT_BLOCKED_TRUE_FLAGS:
        if source_candidate_set.get(flag) is True:
            blockers.append(f"blocked_source_import_candidate_{flag}")
    ...
    if isinstance(runtime_side_effects, dict):
        for flag in SOURCE_RUNTIME_BLOCKED_TRUE_FLAGS:
            if runtime_side_effects.get(flag) is True:
                blockers.append(f"blocked_source_import_candidate_runtime_{flag}")
    return blockers


def _build_8y12_source_path_smoke(
    *,
    approval_phrase: str | None,
    source_import_candidate_set: dict[str, Any] | None = None,
) -> dict[str, Any]:
    if approval_phrase != EIGHT_Y12_APPROVAL_PHRASE:
        return _blocked_8y12_output("blocked_missing_exact_8y12_approval")

    candidate_source = source_import_candidate_set or _safe_import_candidate_set()
    source_blockers = _source_import_gate_blockers(candidate_source)
    if source_blockers:
        return _blocked_8y12_output(source_blockers[0])
    ...
    created = write_candidate_set["evidence_layer_write_candidate_created"]
    return {
        "schema": "sentigraph_8y12_import_candidate_to_write_candidate_smoke_v0_1",
        "phase": "8Y-12",
        "source_path_step": "evidence_layer_import_candidate_to_write_candidate",
        ...
        "write_candidate_created": created,
        ...
        "source_import_candidate_set_schema": write_candidate_set[
            "source_evidence_layer_import_candidate_set_schema"
        ],
        "source_import_candidate_schema": "sentigraph_controlled_evidence_layer_import_candidate_v0_1",
        "source_import_candidate_count": write_candidate_set["source_evidence_layer_import_candidate_count"],
        "evidence_layer_write_candidate_count": write_candidate_set["evidence_layer_write_candidate_count"],
        "evidence_layer_write_candidate_set": write_candidate_set,
        "blockers": list(write_candidate_set["blockers"]),
        "warnings": list(write_candidate_set["warnings"]),
        "human_review_required": write_candidate_set["human_review_required"],
        "no_automatic_trust_upgrade": True,
        "actual_evidence_layer_write_used": False,
        "evidence_layer_write": write_candidate_set["evidence_layer_write"],
        "persisted_evidence_layer_record_created": False,
        "production_evidence_item_created": write_candidate_set["production_evidence_item_created"],
        "production_case_created": write_candidate_set["production_case_created"],
        "production_analysis_run_created": write_candidate_set["production_analysis_run_created"],
        "evidence_import_service_called": False,
        "evidence_ingestion_service_called": False,
        "production_evidenceitem_write_runtime_used": False,
        "actual_review_queue_runtime_used": False,
        "production_review_queue_item_created": write_candidate_set["production_review_queue_item_created"],
        "source11_runtime_called": False,
        "actual_final_summary_report_created": False,
        "b_end_report_runtime_generated": False,
        "sandbox_public_event_runtime_generated": False,
        "export_download_public_delivery_created": False,
        "generated_response_text": False,
        "route_ready": write_candidate_set["route_ready"],
        "frontend_ready": write_candidate_set["frontend_ready"],
        "production_ready": write_candidate_set["production_ready"],
        "customer_ready": write_candidate_set["customer_ready"],
        "public_ready": write_candidate_set["public_ready"],
        "raw_rows_exposed": False,
        "raw_comments_exposed": False,
        "raw_identities_exposed": False,
        "raw_author_ids_emitted": False,
...
def _assert_no_production_side_effects(smoke: dict[str, Any]) -> None:
    ...
    candidate_set = smoke.get("evidence_layer_write_candidate_set")
...
def _fail_if_called(*args: object, **kwargs: object) -> None:
    raise AssertionError("8Y-12 must not call write runtime, production, route, frontend, or delivery helpers")
...
def test_8y12_builds_local_write_candidate_from_controlled_import_candidate(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    assert ROW_PREVIEW_APPROVAL_PHRASE == "APPROVE_8W_7_CONTROLLED_ROW_PREVIEW_IMPLEMENTATION"
    assert [hex(ord(ch)) for ch in write_candidate_module.APPROVAL_PHRASE[:2]] == ["0x6279", "0x51c6"]
    assert "8W-19 Controlled Evidence Layer Write Candidate Helper Implementation" in (
        write_candidate_module.APPROVAL_PHRASE
    )
    _patch_forbidden_entrypoints(monkeypatch)

    smoke = _build_8y12_source_path_smoke(approval_phrase=EIGHT_Y12_APPROVAL_PHRASE)

    assert smoke["smoke_status"] == "ready"
    assert smoke["source_path_step"] == "evidence_layer_import_candidate_to_write_candidate"
    assert smoke["evidence_layer_write_candidate_created"] is True
    assert smoke["write_candidate_created"] is True
    assert smoke["evidence_layer_write_candidate_schema"] == (
        "sentigraph_controlled_evidence_layer_write_candidate_set_v0_1"
    )
    assert smoke["write_candidate_mode"] == "backend_only_local_evidence_layer_write_candidate_boundary"
    assert smoke["source_import_candidate_set_schema"] == (
        "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1"
    )
    assert smoke["source_import_candidate_schema"] == "sentigraph_controlled_evidence_layer_import_candidate_v0_1"
    assert smoke["evidence_layer_write_candidate_count"] > 0
    assert smoke["evidence_layer_write_candidate_count"] <= smoke["source_import_candidate_count"] <= 10
    ...
    candidate_set = smoke["evidence_layer_write_candidate_set"]
    assert candidate_set["evidence_layer_write_candidate_set_status"] == (
    ...
    for candidate in candidate_set["evidence_layer_write_candidates"]:
        assert candidate["evidence_layer_write_candidate_schema"] == (
            "sentigraph_controlled_evidence_layer_write_candidate_v0_1"
        )
        assert candidate["source_evidence_layer_import_candidate_set_schema"] == (
            "sentigraph_controlled_evidence_layer_import_candidate_set_v0_1"
        )
        assert candidate["source_evidence_layer_import_candidate_schema"] == (
...
def test_8y12_phrase_required_before_write_candidate_creation(
    approval_phrase: str | None,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_write_builder(*args: object, **kwargs: object) -> None:
        raise AssertionError("write candidate helper must not be called without 8Y-12 approval")

    monkeypatch.setattr(write_candidate_module, "build_controlled_evidence_layer_write_candidate_set", fail_write_builder)
    _patch_forbidden_entrypoints(monkeypatch)

    smoke = _build_8y12_source_path_smoke(approval_phrase=approval_phrase)

    assert smoke["smoke_status"] == "blocked"
    assert smoke["evidence_layer_write_candidate_created"] is False
    assert smoke["write_candidate_created"] is False
    assert smoke["evidence_layer_write_candidate_count"] == 0
    assert smoke["blockers"] == ["blocked_missing_exact_8y12_approval"]
    _assert_no_production_side_effects(smoke)
    _assert_no_forbidden_output(smoke)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("actual_evidence_layer_write_used", True, "blocked_source_import_candidate_actual_evidence_layer_write_used"),
        ("evidence_layer_write", True, "blocked_source_import_candidate_evidence_layer_write"),
        (
            "persisted_evidence_layer_record_created",
            True,
            "blocked_source_import_candidate_persisted_evidence_layer_record_created",
        ),
        ("production_evidence_item_created", True, "blocked_source_import_candidate_production_evidence_item_created"),
        ("production_case_created", True, "blocked_source_import_candidate_production_case_created"),
        ("production_analysis_run_created", True, "blocked_source_import_candidate_production_analysis_run_created"),
        (
            "actual_review_queue_runtime_used",
            True,
            "blocked_source_import_candidate_actual_review_queue_runtime_used",
        ),
        (
            "production_review_queue_item_created",
            True,
            "blocked_source_import_candidate_production_review_queue_item_created",
        ),
        ("raw_rows_exposed", True, "blocked_source_import_candidate_raw_rows_exposed"),
        ("raw_comments_exposed", True, "blocked_source_import_candidate_raw_comments_exposed"),
        ("raw_identities_exposed", True, "blocked_source_import_candidate_raw_identities_exposed"),
        ("raw_author_ids_emitted", True, "blocked_source_import_candidate_raw_author_ids_emitted"),
        ("raw_author_names_emitted", True, "blocked_source_import_candidate_raw_author_names_emitted"),
        ("profile_urls_emitted", True, "blocked_source_import_candidate_profile_urls_emitted"),
        (
            "author_names_or_profile_urls_exposed",
            True,
            "blocked_source_import_candidate_author_names_or_profile_urls_exposed",
        ),
        ("no_automatic_trust_upgrade", False, "source_no_automatic_trust_upgrade_not_true"),
    ],
)
def test_unsafe_source_import_candidate_blocks_before_write_candidate_creation(
    field: str,
    value: object,
    ...
    def fail_write_builder(*args: object, **kwargs: object) -> None:
        raise AssertionError("write candidate helper must not be called for unsafe import candidate source")

    source_candidate_set = _safe_import_candidate_set()
    source_candidate_set[field] = value
    monkeypatch.setattr(write_candidate_module, "build_controlled_evidence_layer_write_candidate_set", fail_write_builder)
    _patch_forbidden_entrypoints(monkeypatch)

    smoke = _build_8y12_source_path_smoke(
        approval_phrase=EIGHT_Y12_APPROVAL_PHRASE,
        source_import_candidate_set=source_candidate_set,
    )
...
@pytest.mark.parametrize(
    ...
    [
        ("emitted_raw_comments", "blocked_source_import_candidate_runtime_emitted_raw_comments"),
        ("emitted_raw_identities", "blocked_source_import_candidate_runtime_emitted_raw_identities"),
        ("emitted_profile_urls", "blocked_source_import_candidate_runtime_emitted_profile_urls"),
        ("wrote_evidence_layer", "blocked_source_import_candidate_runtime_wrote_evidence_layer"),
        ("created_evidence_items", "blocked_source_import_candidate_runtime_created_evidence_items"),
        (
            "created_production_evidence_items",
            "blocked_source_import_candidate_runtime_created_production_evidence_items",
        ),
        ("created_review_queue_items", "blocked_source_import_candidate_runtime_created_review_queue_items"),
        (
            "created_production_review_queue_items",
            "blocked_source_import_candidate_runtime_created_production_review_queue_items",
        ),
        ("created_production_case", "blocked_source_import_candidate_runtime_created_production_case"),
        ("created_production_analysis_run", "blocked_source_import_candidate_runtime_created_production_analysis_run"),
        ("read_original_package_rows", "blocked_source_import_candidate_runtime_read_original_package_rows"),
        ("parsed_evidence_items_csv", "blocked_source_import_candidate_runtime_parsed_evidence_items_csv"),
    ],
)
def test_unsafe_source_import_candidate_runtime_flags_block(
    runtime_flag: str,
    expected_blocker: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    def fail_write_builder(*args: object, **kwargs: object) -> None:
        raise AssertionError("write candidate helper must not be called for unsafe runtime flags")
