...
| Export-friendly report layout | MVP polish needed | Report layout is readable and export-friendly, but PDF export is not implemented. | Add print CSS and PDF/export task later. |
...
| Saved analysis cases | Next implementation | Not implemented. Current flow is session/local state oriented. | Add mock case model and local persistence before real DB. |
...
| Real Reddit adapter | Future real-data integration | Reddit remains mock-selectable and a future real adapter candidate. | Design safe official/public API adapter without bypass behavior. |
