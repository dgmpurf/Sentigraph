...
Large-Scale Evidence Ingestion scaffold demo (2026-05-26): in `Cases`, import a CSV/XLSX sample or attach manual evidence, then open the `Evidence Scale / Coverage` panel. Expected success signs: total evidence, unique evidence, duplicate evidence, rejected count, weak/unverified count, source/type/acquisition/trust/review distributions, latest ingestion job rows, and source coverage tags are visible. The panel must show `当前覆盖范围仅代表已导入/可用证据，不代表全平台全量覆盖。` and the English coverage note. Run the case and confirm `analysis_input_source=case_evidence_items` when no raw comments are attached. Safety proof points: no real APIs, no URL fetching, no scraping, no MediaCrawler integration, no real LLM calls, no raw-file persistence by default, and no full-platform coverage claim.
