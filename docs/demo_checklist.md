...
RSS/GDELT mock-provider Search Discovery demo path (2026-05-26): open `Search Discovery`, use the provider selector to choose `RSS Mock`, generate candidates, accept one, reject one, and attach the accepted candidate to a case. Repeat with `GDELT Mock`. Expected success signs: provider status shows `mock_only`, `live_fetch_enabled=false`, `metadata only`, and `full_content=false`; generated candidates have `provider=rss_mock` or `provider=gdelt_mock`; accepted candidates attach as `provenance_type=search_discovery_candidate`, `verification_status=source_url_provided_unverified`, and review-needed evidence. Safety proof points: RSS feeds are not fetched, GDELT APIs are not called, candidate URLs are not fetched, and full content extraction remains separate/future.
...
Large-Scale Evidence Ingestion browser-smoke QA / demo path (2026-05-26): in `Cases`, import a CSV/XLSX sample or attach manual evidence, add a duplicate item, reject one review-queue item, and open the `Evidence Scale / Coverage` panel. Expected success signs: total evidence, unique evidence, duplicate evidence, rejected count, weak/unverified count, source/type/acquisition/trust/review distributions, latest ingestion job rows, and source coverage tags are visible. The panel must show `当前覆盖范围仅代表已导入/可用证据，不代表全平台全量覆盖。` plus the English no-fetch/no-scrape note. Run the case and confirm `analysis_input_source=case_evidence_items` when no raw comments are attached; rejected evidence should be excluded from representative comments and duplicates should stay collapsed. Safety proof points: no real APIs, no URL fetching, no scraping, no MediaCrawler integration, no real LLM calls, no raw-file persistence by default, and no full-platform coverage claim.

Evidence Review History / Audit Timeline MVP (2026-05-26): in `Cases`, open a normalized-evidence case, enter an optional review note, and apply `Approve`, `Reject`, `Mark Weak`, `Request Source`, `Merge Duplicate`, and `Reset` on sample review-queue items. Expected success signs: the audit summary shows total review events plus approved/rejected/weak/source-requested/merged/reset counts; the `Review history / audit timeline` table shows latest decisions, previous status -> new status, reviewer label, reviewed time, redacted note/reason, and analysis effect. Rejected evidence remains stored but excluded from default analysis, weak evidence remains usable with warnings, duplicate decisions preserve collapsed counts, screenshots/transcriptions are not called verified, and the UI states that audit history is human review only with no URL fetching and no AI authenticity verification.
