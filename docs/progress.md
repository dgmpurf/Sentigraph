...
Latest Source Catalog planning checkpoint: added `docs/source_catalog.md` and `GET /api/v1/sources/catalog` as a static, metadata-only source planning layer for all-web public-opinion evidence ingestion. The catalog covers video platforms, news/media sites, forums/communities, Q&A sites, complaint/review sites, finance/investor forums, social platforms, search discovery, RSS, user-uploaded datasets, manual URL evidence, and future data-vendor integrations. It returns allowed acquisition modes, allowed/forbidden data types, current Sentigraph status, compliance notes, next action, priority, and green/yellow/red feasibility status without calling real APIs, reading credentials, fetching URLs, using cookies, calling real LLM APIs, or enabling scraping. Added `SourceCatalogEntry`, `SourceCatalogCategory`, and `SourceCatalogResponse` schema placeholders plus frontend API normalization helper `getSourceCatalog()`. Added tests verifying expected categories, YouTube green/real-capable status, Douyin OAuth/permission pending status, Bilibili official-permission pending status, no secret exposure, and no MediaCrawler integration in product code. Validation passed with `python -m pytest` (`571 passed in 5.88s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.27s`; existing non-blocking vendor chunk warning remains). Next recommended task: browser-smoke the Evidence attach flow and Source Catalog endpoint, then add CSV/Excel upload only if manual evidence demos need it.
