...
Recommended next development task: add a mock data source selector QA pass across Keyword Search and case creation so the frontend mock-selectable platforms (Reddit plus the eight official API scaffolds) stay aligned with backend registry status, `/crawl/start` metadata, and the case pipeline. Keep it UI/API consistency only: do not add real platform calls, do not enable live public fetching, and do not start real API integrations.
