...
Recommended next development task: run a cross-platform official adapter QA matrix for Weibo, Bilibili, Douyin, Kuaishou, Xiaohongshu, Zhihu, Douban, and Toutiao. Confirm all scaffolds expose the same mock-first interface, safe registry metadata, `.env.example` variables, `/crawl/start` schema fields, no real API calls, and consistent `api_pending` / `config_error` behavior before starting any approved real API application package.
