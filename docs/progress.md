...
Latest YouTube real-data demo UX polish: clarified real-data demo provenance across the app and report path. Completed YouTube cases with `analysis_input_source=case_raw_data` now display `Data source: YouTube Real`, `Analysis: Offline deterministic`, `LLM: Mock`, and `analysis_input_source=case_raw_data` in the app header, Analysis Result, Summary Report, and Demo Flow surfaces instead of flattening the experience into generic `Mock Mode`. Backend summary wording now says `Offline deterministic analysis from attached case raw data` when raw case data is used. Representative-comment selection keeps all raw YouTube comments stored but down-ranks obvious channel promotion/self-promo comments (`patreon`, `channel member`, `subscribe`, `referral`, `promo code`, `affiliate`, `join`) when substantive event-relevant alternatives exist. Validation passed with `python -m pytest` (`544 passed in 4.79s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.78s`; existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the manual YouTube real-data browser demo with local credentials and capture the badge screenshot plus end-to-end report screenshots.
