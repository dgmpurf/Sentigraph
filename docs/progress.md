...
Recommended next development task: run a browser QA pass for the new Platform Integration Overview page alongside the existing Public Parser Status page, then add a mock data source selector QA pass across Keyword Search and case creation so frontend mock-selectable platforms stay aligned with backend registry status, `/crawl/start` metadata, and the case pipeline. Keep it UI/API consistency only: do not add real platform calls, do not enable live public fetching, and do not start real API integrations.
