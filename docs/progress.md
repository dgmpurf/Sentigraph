...
Latest YouTube official API adapter QA update: the mock-first `youtube` platform adapter is stabilized after a local real-mode smoke check. With `YOUTUBE_ADAPTER_MODE=real` and a local `YOUTUBE_API_KEY`, `/api/v1/crawl/start` entered `adapter_mode=real`, returned `fallback_used=false`, `credential_present=true`, `post_count=3`, `comment_count=3`, and valid `RawPost` / `RawComment` schema metadata. The adapter normalizes public YouTube video metadata plus top-level/reply comments, keeps tiny real-mode limits, falls back safely to mock mode when the key is missing, and exposes credential presence only as booleans. Added QA coverage for UTF-8/emoji text normalization, top-level/reply comment parent/default URL handling, and `/crawl/start` missing-key safe metadata. Automated tests use mocked real API responses only; no real YouTube API call, scraping, cookie use, login bypass, captcha bypass, anti-bot evasion, API-key printing, `.env` modification, or LLM API call was introduced. Backend validation passed with `python -m pytest` (`522 passed in 4.94s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). Frontend build was not run because no frontend files changed. GitHub Actions CI remains intentionally disabled.
