...
Latest selector repair mock scaffold update: added a fixture-only public parser selector repair design and backend scaffold. New schemas cover `SelectorRepairRequest`, `SelectorCandidate`, `SelectorRepairSuggestion`, and `SelectorRepairPreviewResult`; the service sanitizes caller-provided public fixture HTML, removes scripts/styles/inline handlers/obvious secrets, caps request length, and uses deterministic offline `MockProvider.suggest_selector_repair()` candidates only. Added safe endpoints `POST /api/v1/public-parsers/selector-repair/suggest` and `POST /api/v1/public-parsers/selector-repair/preview`; they do not fetch live pages, call real LLM APIs, call real platform APIs, or automatically modify active parser profiles. `.env.example` now documents `SELECTOR_REPAIR_MODE=mock`, `SELECTOR_REPAIR_ENABLE_REAL_LLM=false`, and `SELECTOR_REPAIR_MAX_HTML_CHARS=20000`; `docs/selector_repair_design.md` documents the no-bypass and human-review workflow. Focused selector-repair validation passed with `11 passed`; full backend validation passed with `python -m pytest` (`374 passed in 3.43s`). Frontend build was not run because no frontend files changed. GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no real LLM APIs were called, no API keys were printed, no real platform APIs were called, and live public fetching remains disabled by default.
...
Recommended next development task: run a mock data-source selector QA pass across Keyword Search and case creation so frontend mock-selectable platforms stay aligned with backend registry status, `/crawl/start` metadata, and the case pipeline. Keep it UI/API consistency only: do not add real platform calls, do not enable live public fetching, do not start real API integrations, and keep the new LLM provider layer in mock mode.
