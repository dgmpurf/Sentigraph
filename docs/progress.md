...
Latest mock-only Search Discovery Candidate Review implementation: added a safe candidate-review UI and attach path on 2026-05-26. Backend schemas now include `SearchDiscoveryCandidateAttachRequest` and `SearchDiscoveryCandidateAttachResult`; `POST /api/v1/cases/{case_id}/search-discovery/candidates/attach` converts accepted mock/static candidates into normalized `EvidenceItem` records with `acquisition_mode=search_discovery`, `provenance_type=search_discovery_candidate`, `verification_status=source_url_provided_unverified`, conservative trust, safe metadata only, and review-needed behavior. Rejected candidates are skipped. The new `Search Discovery` page lets users generate mock candidates, accept/reject them, attach accepted candidates to a selected case, and optionally run analysis after attach. The UI explicitly states mock/static only, no real search API, no URL fetch, no scraping, metadata only, and human review required. Browser smoke generated mock candidates, accepted one, rejected one, attached the accepted candidate, then verified `search_discovery` evidence appears in Cases, Evidence Review Queue, and Evidence Scale / Coverage with no console errors or raw `[object Object]` rendering. Validation passed with `python -m pytest` (`604 passed in 7.40s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.28s`; existing non-blocking large chunk warning remains). No real search APIs, website APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced.
