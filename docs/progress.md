...
Latest v6.3 Screenshot / Recording Demo Package Final: prepared the final external-presentation docs for the YouTube real-data demo path. Added `docs/demo_recording_script.md` with 3-minute and 8-minute scripts, exact page order, what to say on each page, what each page proves, and presenter guardrails. Added `docs/demo_screenshot_checklist.md` with the canonical 13-screen capture sequence from Dashboard through Platform Integration Overview. Updated `docs/demo_package.md` as the v6.3 package overview, refreshed `README.md` with a short Demo Package section, linked the new package docs from `docs/demo_story.md`, and updated `docs/demo_checklist.md` with the v6.3 capture target. The docs explicitly distinguish real YouTube public video/comment data, offline deterministic analysis/risk/report/forecast/Simulation Lab, mock LLM provider, and mock/scaffold status for other platforms. Validation passed with `python -m pytest` (`544 passed in 6.00s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.05s`; existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no real YouTube API calls were made in automated tests, no real LLM APIs were called, no scraping was implemented, and no product features were added. Next recommended task: produce the actual screenshot set and short recording from `docs/demo_screenshot_checklist.md` and `docs/demo_recording_script.md`, then consider the Douyin Web App permission audit.
