...
Latest versioned B05 v0.2 identity-ready review-surface G3 working-tree update: the bounded Goal `SENTIGRAPH_B05_V02_IDENTITY_READY_REVIEW_SURFACE_G3_BOUNDED_IMPLEMENTATION_FIXTURE_VALIDATION_V01` implements the independently accepted six-path change surface against `main` checkpoint `0ccd63cc8b8147088f3b105b9b963f24f259f91e` / tree `9c623a77f4e1b3feded7cb243858c764ad1a54c0`. The backend adds exactly one default-disabled GET route, `/api/v1/internal/alpha/review-console/v0.2/local-exchange-projections/{sample_handle}`, behind `SENTIGRAPH_INTERNAL_ALPHA_LOCAL_EXCHANGE_IDENTITY_READY_REVIEW_PROJECTION_V0_2_ENABLED` with the first allowlist fixed to `helldivers2-psn-demo`; it reuses the existing 53-field v0.2 service and fails closed before protected service access when either route gate or the allowlist check fails. The frontend adds a distinct strict 53/13-field v0.2 normalizer, one exact-handle GET helper, and a separately selected display-only identity panel with a once-per-mount request guard and the complete safe binding hash. The historical v0.1 route, 52-field normalizer, helper, and review view remain separate with no fallback. Validation for this candidate is fixture-only and limited to the exact focused backend pytest pair plus `InternalAlphaReviewConsole.test.jsx`; it uses synthetic/mocked data and authorizes no server, live HTTP, browser, B05 current-sample access, Provider Result or metadata read, database, Source access, production/public/export/delivery action, or dependency installation. Commit, push, publication, live HTTP acceptance, and browser-visible acceptance remain unestablished. The next gate after a passing candidate package is Mainline independent G3 candidate acceptance; G4 publication, G5 real-current-sample HTTP, G6 browser-visible acceptance, and G7 human review/formal decision remain separate future authorities.

Latest post-repair Class 2 current-sample acceptance update: the fresh Goal `SENTIGRAPH_V0_4_CLASS2_ONE_REAL_CURRENT_SAMPLE_POST_REPAIR_IN_PROCESS_ASGI_ACCEPTANCE_V0_4` completed exactly once against repository checkpoint `56b1d5a075de5091aaff45ba69813f0df129df75` / tree `641ddf44ad79108f400c049d4155e06ee351e5a3` and was independently accepted by mainline ChatGPT on 2026-08-08. The accepted request returned HTTP 200 with `application/json`, the exact ordered 52-field projection, `projection_status=ready_for_human_review`, `candidate_count=1`, and coverage counts `7 / 3 / 4 / 3`. The governed read ledger recorded Provider Result read/reopen `1 / 0`; exactly one read each for `README.md`, `coverage_note.md`, `manifest.json`, `validation_report.json`, and `validation_report.md`; zero `package_index.json` body reads; zero forbidden row reads; zero alternate Provider Result/package/receipt reads; and zero retry/second GET. Counter, privacy, read-primitive restoration, and gate restoration contracts all passed. The repository and Project Source remained unchanged, with no production, persistence, public, export, or delivery action. The mainline acceptance receipt is `4663` bytes with SHA-256 `41a17ffbf641af0e410781ce2a3438c6f64e6a8861ff25cc1344d9c94369d11f`. At that Class 2 checkpoint, acceptance classes were Class 1 established, Class B established, Class 2 established, and Class C not established. The historical V0.3R1 terminal stopped ASGI-request-failure result remains preserved without reclassification. This acceptance creates no reusable Goal, retry, second-GET, browser/Class-C, persistence, production, public, export, or delivery authority.

Latest V0.3R5 Class C browser-visible acceptance update: the fresh Goal `SENTIGRAPH_V0_4_CLASSC_ONE_REAL_CURRENT_SAMPLE_BROWSER_VISIBLE_ACCEPTANCE_V0_3R5` completed exactly once against repository checkpoint `8a47267c6d5b97bfa18b2a50defde7246b6392e5` / tree `77d4cbe7f6e9d80aa175cf29d94a194002e64763` and was independently accepted by mainline ChatGPT on 2026-08-08. The exact bounded evidence package `SENTIGRAPH_V0_4_CLASSC_V0_3R5_RUNTIME_ATTEMPT_EVIDENCE_V0_1.zip` is `6327` bytes with SHA-256 `980632548bd5bbd69af8acfdc9ab57722829910e0d1175aac7f3760b43078312`; its seven-member evidence chain independently passed integrity review. The one-time V0.3R5 authority is fully consumed and nonreusable (`created / consumed / remaining = 1 / 1 / 0`): PRE-GOAL passed with exit `0`, launcher exited `0`, and the bounded browser interaction completed one trial click, one actual pointer-target click, one Local-exchange selection, and exactly one current B05 GET with zero retry, zero second B05, and zero screenshots. The accepted B05 response returned HTTP `200`; raw response bytes/SHA remained observation-only while the bounded JSON/object, exact ordered 52-field, stable-semantic, and volatile-ID format contracts all passed. The browser API ledger recorded catalog `1 x 200`, governed read-only projection `2 x 200` within the accepted `1..2` development-StrictMode bound, and current B05 `1 x 200`, with zero unexpected API paths. Browser, frontend, and backend cleanup passed; ports `8000/5173` were free; repository and Project Source remained unchanged; Codex Goal-tool calls/mutations remained `0/0`. Mainline acceptance therefore establishes Class C and browser-visible acceptance while preserving Class 1, Class B, and Class 2 as established; screenshot acceptance remains NOT ESTABLISHED because screenshot calls remained zero. The mainline Class C acceptance receipt is `7402` bytes with SHA-256 `b04ae9f54110189f9e589ac4e91c1141e9fd9f0e29c8a814e2e60f3d444ca1e3`. Historical V0.3, V0.3R1, V0.3R2, V0.3R3, and V0.3R4 terminal STOP results remain preserved without reclassification, and this acceptance creates no reusable browser runtime, retry, second-B05, screenshot, repository-mutation, production, persistence, public, export, or delivery authority. The next governed project step is Post-ClassC bounded Project Source synchronization; Project Source replacement remains a separate manual human boundary.

Historical V0.3R1 Corrected Route A Class 2 preparation checkpoint: the exact ten-file corrected Route A repair was independently accepted by mainline ChatGPT, committed, pushed to `origin/main`, and independently accepted as a stable repository repair checkpoint on 2026-08-07 (`commit 2711e687b4c3ee39f922d39651b0bb6a7f4a1580`; `tree b63044812b1c3fa4fccaafc3ed0d97f10dab4841`; parent `4e50ec3949c701730f945b6865556a7a7f6c9a91`). The governed B05 path selects the exact five-file metadata-read profile (`README.md`, `coverage_note.md`, `manifest.json`, `validation_report.json`, and `validation_report.md`) while the generic private-collector resolver retains its existing six-file behavior; B05 validates `package_index_ref` provenance but does not open the `package_index.json` body. B01/B03 lazy included-router compatibility tests were corrected without changing product routing. Fresh bounded validation before publication passed with `git diff --check`, compilation of the four production files, and the exact five-file focused pytest run (`175 passed, 0 failed, 0 skipped, 0 errors`; one existing `StarletteDeprecationWarning`). The subsequent staged-byte hard stop was resolved by read-only proof and independent acceptance that the three byte differences were solely deterministic CRLF-to-LF Git canonicalization; no historical execution result was reclassified. No governed runtime retry, second GET, Goal mutation, Project Source access/change, or production, persistence, public, export, or delivery action occurred at that checkpoint. At that time the repair was published but had not yet been live-validated, so Class 2 remained not established until the later fresh V0_4 execution recorded above.
...
Latest Vendor POC Scoring Template update: completed on 2026-05-27. Added `docs/vendor_scoring_rubric.md` with a 100-point vendor scoring model covering Compliance / contract readiness (40), technical field completeness (25), data quality (20), and risk control (15). Added `docs/vendor_poc_scorecard_template.md` as a fillable scorecard for source/rights review, sample import results, Sentigraph mapping, risk flags, scoring, classification, next action, adapter gates, and sign-off. The template standardizes vendor classifications as `approved_poc`, `limited_poc`, `internal_research_only`, and `reject`, and maps accepted vendor samples to `acquisition_mode=data_vendor`, `provenance_type=data_vendor`, `verification_status=vendor_attested` only when documented, and default `trust_label=medium_low`. Standard risk flags are now documented for `self_crawled_public_web`, `source_unclear`, `deletion_sync_unknown`, and `personal_data_unknown`. Validation passed with `python -m pytest` (`609 passed in 7.04s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.22s`; existing non-blocking large chunk warning remains). No vendor APIs, real APIs, adapter implementation, scraping, MediaCrawler integration, credential storage, or secret exposure were introduced. Next recommended task: apply the scorecard to a concrete vendor sample only when one is available, using CSV/Excel POC import rather than live API integration.

Latest Data Vendor Intake and Sample Import planning: completed on 2026-05-26. Added `docs/data_vendor_intake_checklist.md`, `docs/vendor_sample_data_schema.md`, and `docs/vendor_poc_plan.md` so third-party data vendors can be evaluated through identity, platform coverage, field completeness, pricing/SLA, commercial rights, retention/deletion, personal-data handling, contract/DPA, red-flag, sample-schema, and bounded POC checks before any adapter work. Updated `docs/source_feasibility_matrix.md` to distinguish contracted vendor sample import as a bounded green path, prospective vendors as yellow until POC/compliance gates pass, and unofficial scrape APIs / source-code crawler solutions as red or internal-research-only. Updated `docs/evidence_import_guide.md` with vendor CSV/JSON-to-`EvidenceItem` guidance using `acquisition_mode=data_vendor`, conservative provenance/trust labeling, no-secret sample requirements, and CSV/Excel POC import only. Updated `docs/implementation_backlog.md` to keep a future Data Vendor Adapter blocked until vendor selection, POC, contract/DPA, retention/deletion, security, quota, mocked-fixture, and credential-handling gates are complete. Validation passed with `python -m pytest` (`609 passed in 6.58s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.11s`; existing non-blocking large chunk warning remains). No vendor APIs, real APIs, scraping, MediaCrawler integration, credential storage, or secret exposure were introduced. Next recommended task: use the checklist to evaluate one concrete vendor sample through CSV/Excel POC, or pause vendor work until a real vendor candidate is available.

Latest v6.27 Demo Screenshot / Recording Package preparation: completed on 2026-05-26. Added `docs/demo_asset_package_v6_27.md` as the final manual capture manifest for the v6.26 demo-ready MVP, covering the package purpose, target audiences, what the demo proves, what it does not claim, 21 required screenshot assets with recommended filenames, 3-minute and 8-minute recording scripts, talking points, local capture commands, optional YouTube-real capture note, and pre-recording privacy/safety checklist. Updated `docs/demo_recording_script.md` and `docs/demo_screenshot_checklist.md` to match the v6.27 package, including Evidence Review, Audit Timeline, Evidence Scale / Coverage, Search Discovery, RSS/GDELT mock provider selector, Benchmarks, and LLM Safety. Updated `docs/demo_package.md` and `README.md` to link the v6.27 asset package and clarify the 21-screen checklist. The package deliberately preserves the boundaries from the v6.26 audit: optional real YouTube public video/comment data only when locally configured; offline deterministic analysis/risk/report/forecast/simulation; mock/static Search Discovery and RSS/GDELT providers; mock LLM provider; pending Douyin/Bilibili/Xiaohongshu/Reddit/Weibo integrations; no MediaCrawler, scraping, cookies, captcha bypass, anti-bot bypass, URL fetching, real AI authenticity review, or full-web/full-platform coverage claim. Validation passed with `python -m pytest` (`609 passed in 6.74s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.87s`; existing non-blocking large chunk warning remains). No real APIs, real search APIs, real RSS/GDELT APIs, real YouTube/Douyin/Bilibili APIs, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI authenticity review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: manually produce the actual screenshot deck and short recording from `docs/demo_asset_package_v6_27.md`.

Latest v6.26 Full Demo Readiness Audit: completed on 2026-05-26. The audit refreshed the external demo package docs (`docs/demo_package.md`, `docs/demo_recording_script.md`, `docs/demo_screenshot_checklist.md`, and the top-level path in `docs/demo_checklist.md`) so the final presentation now covers the full current surface: optional YouTube real-data demo, CSV/Excel evidence import, manual URL evidence, Source Catalog / Feasibility Matrix, Search Discovery mock/static candidates, RSS Mock and GDELT Mock providers, Evidence Trust / Provenance / Deduplication, Evidence Review Queue, Evidence Review Audit Timeline, Evidence Scale / Coverage, Analysis Result, Summary Report / Markdown export, Risk Monitor / Forecast, Simulation Lab, Benchmarks, LLM Safety, and Platform Integration Overview. Local API smoke verified the Source Catalog returns 12 categories and 22 sources with static metadata only; Search Discovery providers return `mock_static`, `rss_mock`, and `gdelt_mock` with `live_fetch_enabled=false`, `requires_api_key=false`, `returns_title_snippet_url=true`, and `returns_full_content=false`; CSV import plus manual URL evidence can drive `analysis_input_source=case_evidence_items`; rejected evidence is excluded from representative comments; forecast and Simulation Lab initialization work from an evidence-backed case; Search Discovery mock/RSS/GDELT accepted candidates attach as review-needed evidence; and mock fallback still produces `analysis_input_source=mock_data_fallback`. Browser smoke opened Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmarks, LLM Safety, Platform Integration Overview, Search Discovery, and Keyword Search; it also generated and attached a mock Search Discovery candidate through the UI. No app-console errors, raw `[object Object]` rendering, secret markers, or ECharts duplicate-node regression were observed. Validation passed with `python -m pytest` (`609 passed in 8.20s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.89s`; existing non-blocking large chunk warning remains). No real APIs, real search APIs, real RSS/GDELT APIs, real YouTube/Douyin/Bilibili APIs, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI authenticity review, secret exposure, or GitHub Actions workflow were introduced. Current known limitations: YouTube real data remains optional and local-key based; Douyin/Bilibili/Xiaohongshu/Reddit/Weibo real integrations remain pending official API/OAuth/permission gates; RSS/GDELT/search discovery remain mock/static metadata providers; Evidence Scale / Coverage describes imported or available evidence, not full-platform/full-web capture; predictions and simulations remain decision-support outputs, not guaranteed outcomes or real-world actions. Next recommended task: produce the actual screenshots/recording from the v6.26 package, or pause ingestion expansion until official platform API gates or a real provider policy review is available.

Latest RSS/GDELT mock-provider browser smoke QA: completed on 2026-05-26. Local API smoke rechecked `GET /api/v1/search-discovery/status`, `GET /api/v1/search-discovery/providers`, the default mock candidate endpoint, `provider=mock_static`, `provider=rss_mock`, `provider=gdelt_mock`, and `POST /api/v1/cases/{case_id}/search-discovery/candidates/attach`. The provider taxonomy returns `mock_static`, `rss_mock`, and `gdelt_mock` as mock-only/static providers with `live_fetch_enabled=false`, `requires_network=false`, `requires_api_key=false`, `returns_title_snippet_url=true`, `returns_full_content=false`, and no exposed credentials. Browser smoke opened `Search Discovery`, used the provider selector for Mock Static, RSS Mock, and GDELT Mock, generated candidates, accepted one and rejected one per provider, attached accepted candidates to a local QA case, and confirmed the accepted candidates appear as `search_discovery_candidate` evidence in Cases, Evidence Review Queue, and Evidence Scale / Coverage. The provider status card now explicitly shows `candidate metadata only` and `No URL content extraction`; quick navigation smoke for Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmarks, LLM Safety, Platform Integration Overview, and Search Discovery found no app-console errors, no `[object Object]` rendering, and no ECharts duplicate-node regression. Validation passed with `python -m pytest` (`609 passed in 7.78s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.89s`; existing non-blocking large chunk warning remains). No real RSS feeds, GDELT APIs, search APIs, website APIs, real YouTube/Douyin/Bilibili APIs, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: plan a user-reviewed URL-list provider or pause ingestion work until official platform API gates are available.

Latest RSS/GDELT mock-provider planning update: implemented and validated on 2026-05-26. Search Discovery now exposes a static provider taxonomy through `GET /api/v1/search-discovery/providers` with `mock_static`, `rss_mock`, `gdelt_mock`, `search_api_future`, `user_url_list`, and `data_vendor_future`. The mock candidate endpoint accepts `provider=rss_mock` and `provider=gdelt_mock` and returns local `example.test` URL/title/snippet fixtures only. The Search Discovery page now includes a provider selector and provider status card showing mock-only, no-live-fetch, metadata-only boundaries. Accepted RSS/GDELT mock candidates still attach as conservative `EvidenceItem` records with `acquisition_mode=search_discovery`, `provenance_type=search_discovery_candidate`, `verification_status=source_url_provided_unverified`, and review-needed behavior. Validation passed with `python -m pytest` (`609 passed in 7.03s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.76s`; existing non-blocking large chunk warning remains). Local backend/frontend HTTP smoke reached `GET /api/v1/health` and the Vite dev page; rendered browser automation was not completed because the current Node REPL environment did not have the `playwright` module available. No real RSS feeds, GDELT APIs, search APIs, website APIs, real YouTube/Douyin/Bilibili APIs, URL fetching, scraping, MediaCrawler integration, real LLM calls, secret exposure, or GitHub Actions workflow were introduced.

Latest mock-only Search Discovery Candidate Review QA: stabilized on 2026-05-26. Rechecked `GET /api/v1/search-discovery/status`, `GET /api/v1/search-discovery/mock-candidates?query=Tesla`, and `POST /api/v1/cases/{case_id}/search-discovery/candidates/attach`; the status and mock candidate endpoints still return static metadata only, and accepted mock candidates attach as conservative `EvidenceItem` records with `acquisition_mode=search_discovery`, `provenance_type=search_discovery_candidate`, `verification_status=source_url_provided_unverified`, low trust, safe metadata only, and review-needed behavior. Browser smoke opened `Search Discovery`, generated mock Tesla candidates, accepted one, rejected one, attached the accepted candidate to a case, verified the candidate appears in Cases Evidence, Evidence Review Queue, and Evidence Scale / Coverage, then ran the case with `analysis_input_source=case_evidence_items`. QA also fixed a graph-stability edge case so evidence-derived post/comment nodes get distinct IDs and do not reintroduce the ECharts duplicate-node regression. Validation passed with `python -m pytest` (`604 passed in 7.36s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.34s`; existing non-blocking large chunk warning remains). No real search APIs, website APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: RSS/GDELT mock-provider planning, while real search provider integration and automatic parser fetch remain future work.

Latest Large-Scale Evidence Ingestion browser smoke QA: rechecked on 2026-05-26. Local API smoke created an evidence-only case with CSV import, manual URL evidence, duplicate submissions, one rejected evidence item, and one low-trust/missing-source evidence item. The new read-only endpoints `GET /api/v1/cases/{case_id}/evidence/summary`, `GET /api/v1/cases/{case_id}/evidence/jobs`, and `GET /api/v1/cases/{case_id}/evidence/coverage` returned total/unique/duplicate counts, rejected and weak evidence counts, latest local ingestion jobs, safe metadata only, and coverage warnings that this is imported/available evidence rather than full-platform coverage. Browser smoke selected the QA case in `Cases` and verified the `Evidence Scale / Coverage` panel shows total evidence `7`, unique `6`, duplicates `1`, rejected `1`, weak/unverified `1`, source/type/acquisition/trust/review distributions, latest CSV/manual ingestion jobs, source coverage, and the visible note `当前覆盖范围仅代表已导入/可用证据，不代表全平台全量覆盖。` Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmark Dashboard, LLM Safety, Platform Integration Overview, and Cases opened without raw `[object Object]` rendering or page-console errors. The evidence run produced `analysis_input_source=case_evidence_items`; rejected evidence was excluded from Markdown representative text, duplicates stayed collapsed, weak/unverified evidence produced warnings, `case_raw_data` still won over evidence when raw comments were attached, and mock fallback still worked when no raw/evidence existed. Validation passed with `python -m pytest` (`602 passed in 7.21s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.40s`; existing non-blocking large chunk warning remains). No real APIs, real search APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: chunked/resumable import planning only if larger demo datasets are needed; otherwise continue Search Discovery candidate-review planning or wait for official platform API gates.

Latest Evidence Review History / Audit Timeline MVP implementation: added append-only review decision history on 2026-05-26. `EvidenceItem` now carries `review_history`; review decisions append `EvidenceReviewHistoryEntry` records with previous/new status, decision, reason code, reviewer label, reviewed time, redacted note, trust/verification before-after fields, analysis effect, and safe-mode flags for no AI verification, no URL fetch, and no secret exposure. Added backend timeline/audit endpoints: `GET /api/v1/cases/{case_id}/evidence/{evidence_id}/review-history`, `GET /api/v1/cases/{case_id}/evidence/review-timeline`, and `GET /api/v1/cases/{case_id}/evidence/review-audit-summary`. Cases now shows a review note field, audit summary counts, and a latest-first review history table. Browser smoke opened the local Cases page, selected an audit-timeline evidence case, verified the review history/audit summary text rendered, and found no browser console errors or `[object Object]` rendering. Validation passed with `python -m pytest` (`600 passed in 6.81s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.99s`; existing non-blocking large chunk warning remains). No real APIs, URL fetching, scraping, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: plan large-scale evidence ingestion jobs with the same human-review/audit boundaries.

Latest Evidence Review Queue browser-smoke QA and validation-command stabilization: completed on 2026-05-26. Browser smoke covered the Cases `Evidence Review / 证据复核` queue, Analysis Result, Summary Report, Dashboard, Risk Monitor, Simulation Lab, Benchmark Dashboard, LLM Safety, Platform Integration Overview, and the static Search Discovery / Source Catalog surfaces. The queue is demo-ready for low-trust, unverified, screenshot/transcription, missing-source, attestation-missing, duplicate, secret-redacted, and HTML/script-like plain-text evidence. The UI shows queue counts, review-needed/low-trust/duplicate/approved/rejected/marked-weak/needs-more-source counts, filters, trust/provenance/verification labels, source URL status, duplicate indicators, risk flags, and human-review actions. Approve, reject, mark weak, request source, merge duplicate, and reset review were browser-smoked; rejected evidence remains stored but is excluded from default evidence-based analysis and representative comments, while marked-weak evidence stays usable with warnings. Analysis Result and Summary Report show low-trust/unverified evidence notes and rejected-excluded notes without claiming screenshots/transcriptions or AI authenticity review are verified. Fixed the pytest working-directory false failure by changing the Simulation Lab benchmark test to use the repository-root `BENCHMARK_DIR` instead of `Path("benchmarks")`; local validation commands remain documented as root-run commands: `python -m pytest`, `python scripts/run_offline_benchmarks.py`, and `npm --prefix frontend run build`. No real APIs, real search APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: large-scale evidence ingestion job planning or a review-history/audit-timeline polish pass, while keeping AI-assisted review future-only.

Latest Evidence Review Queue MVP implementation: added a human review queue for normalized evidence on 2026-05-25. Backend schemas now include review queue items, summaries, decision requests, and decision results; `EvidenceItem` carries review status, reason codes, reviewer label, review notes, and reviewed time. Added `GET /api/v1/cases/{case_id}/evidence/review-queue`, `GET /api/v1/cases/{case_id}/evidence/review-summary`, and `POST /api/v1/cases/{case_id}/evidence/{evidence_id}/review`. The queue includes low-trust, unverified, screenshot/transcription, missing-source, duplicate, attestation-missing, and risk-flagged evidence. Decisions support approve, reject, mark weak, request more source, merge duplicate, and reset review. Rejected evidence remains stored but is excluded from default `case_evidence_items` analysis and representative comments; duplicate evidence remains collapsed; `case_raw_data` still wins over evidence items and mock fallback still works. Cases now has an `Evidence Review / 证据复核` panel with queue filters and action buttons, while Analysis Result and Summary Report show review-needed and rejected-excluded notes. Validation passed with `python -m pytest` (`599 passed in 7.17s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.24s`; existing non-blocking vendor chunk warning remains). No real APIs, real search APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, real AI review, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: browser-smoke the Evidence Review Queue UI end to end.

Latest Evidence Trust / Provenance / Deduplication browser smoke QA: completed on 2026-05-25. Local API smoke created a manual-evidence QA case and verified the priority order `case_raw_data` > `case_evidence_items` > `mock_data_fallback`; duplicate manual URL/text submissions collapsed into one duplicate group, screenshot transcription remained `screenshot_unverified`, source URL presence/missing status was visible, and the case reran with `analysis_input_source=case_evidence_items`. Browser smoke covered Cases, Analysis Result, Summary Report, Propagation Graph, Risk Monitor, Simulation Lab, Benchmark Dashboard, LLM Safety, Platform Integration Overview, and Dashboard. Cases and Analysis Result now show verification-status distribution, source URL present/missing counts, explicit `attestation_missing`, review flags, trust labels, provenance labels, and duplicate indicators. Summary Report now labels evidence-based reports as `Normalized case evidence report` instead of raw-data reports and keeps the low-trust/unverified evidence warning. No raw `[object Object]` rendering or Sentigraph runtime console errors were observed; only Vite dev reconnect/React DevTools development messages appeared. Validation passed with `python -m pytest` (`595 passed in 7.21s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.28s`; existing non-blocking vendor chunk warning remains). No real APIs, real search APIs, real YouTube/Douyin/Bilibili calls, URL fetching, scraping, cookies, MediaCrawler integration, real LLM calls, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: design a compact evidence review queue or large-scale evidence ingestion job model with the same no-fetch/no-scrape boundaries.

Latest Evidence Trust / Provenance / Deduplication framework update: added conservative trust and provenance labels to `EvidenceItem` on 2026-05-25. Evidence now carries `provenance_type`, `verification_status`, `trust_score`, `trust_label`, source URL/capture metadata, user attestation status, hash fields, duplicate-group metadata, and review `risk_flags`. Added read-only summaries at `GET /api/v1/cases/{case_id}/evidence/trust-summary` and `GET /api/v1/cases/{case_id}/evidence/dedup-summary`. Manual URL and CSV/Excel evidence now support source capture method and user attestation; missing attestation, screenshot transcription, missing URL/timestamp, secret-like text, HTML/script-like text, unsupported platform claims, and high duplicate counts are flagged for review. Duplicate text/URL submissions are collapsed within a case so repeated or malicious uploads do not directly inflate sentiment/topic/risk counts; `duplicate_count` remains available as repetition signal. UI panels in Cases, Analysis Result, and Summary Report now show trust/provenance/review/dedup signals and warn when analysis uses low-trust or unverified evidence. Validation passed with `python -m pytest` (`595 passed in 6.38s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.83s`; existing non-blocking vendor chunk warning remains). No real APIs, real search APIs, URL fetching, scraping, cookies, MediaCrawler integration, real YouTube/Douyin/Bilibili calls, real LLM calls, or secret storage were added. Next recommended task: browser-smoke the trust/dedup UI flow, then consider a mock Search Discovery candidate-review UI only if demos need it.

Latest Search Discovery planning/static status update: added a safe Search Discovery design layer on 2026-05-25 without enabling live discovery or fetching. `docs/search_discovery_design.md` defines Search Discovery as URL/title/snippet candidate discovery, not crawling or scraping. `docs/search_discovery_source_matrix.md` classifies search engine APIs, news discovery APIs, RSS, site-specific public search, user-provided URL lists, data vendors, and mock fixtures. Added static backend schemas and endpoints: `GET /api/v1/search-discovery/status` and `GET /api/v1/search-discovery/mock-candidates?query=Tesla`. Both endpoints return mock/static metadata only, never call real search APIs, never fetch URLs, never scrape, never inspect `.env`, and never expose secrets. Platform Integration Overview now includes a read-only Search Discovery panel with provider status, review flow, and safety tags. Source Catalog and Source Feasibility Matrix now clarify that Search Discovery is green only for metadata discovery or user-reviewed URL lists; full content extraction requires official API permission, user-provided text, licensed vendor data, or reviewed public parser rules. Validation passed with `python -m pytest` (`589 passed in 6.78s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.54s`; existing non-blocking vendor chunk warning remains). Next recommended task: mock Search Discovery candidate-review UI only if needed for demos; otherwise wait for RSS/GDELT/provider research or official Douyin/Bilibili API gates.

Latest Manual URL Evidence Import UI browser smoke QA: stabilized the end-to-end manual evidence flow on 2026-05-25. Browser smoke on the Cases page verified a draft case with no `raw_comments`, the `手动添加证据` panel, article/video/comment/reply/interaction-metric evidence entry, readable attached-evidence previews, `evidence_items=5`, `acquisition_mode=manual_url`, and no raw `[object Object]` rendering. QA found and fixed two blocking polish issues: repeated single-evidence attach calls now append instead of replacing prior evidence, and the manual form resets after successful attach to avoid stale values. Added a safe `打开` action so a draft case can be selected without running mock analysis first. Running the case after manual attach now produces `analysis_input_source=case_evidence_items`; Analysis Result and Summary Report/Markdown wording now say normalized case evidence rather than mock fallback or attached raw data. API smoke also confirmed missing text returns `400`, secret-like pasted text is redacted, invalid numeric metrics produce warnings, and Simulation Lab initialization works from the manual-evidence case. Browser smoke covered Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmarks, LLM Safety, and Platform Integration Overview with no Sentigraph console/runtime errors. Validation passed with `python -m pytest` (`585 passed in 7.05s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.57s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube calls in automated tests, real Douyin/Bilibili calls, real LLM calls, URL fetching, scraping, cookies, MediaCrawler integration, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: Search Discovery planning only, or pause real-source implementation until Douyin/Bilibili official API gates are available.

Latest Manual URL Evidence Import UI implementation: added a `手动添加证据` panel on the Cases page so a user can attach a single lawful public URL/title/body/comment evidence item without CSV/Excel. The form supports article, video, post, comment, reply, and `interaction_metric` evidence; shows the required safety copy that Sentigraph does not auto-fetch URLs, does not save credentials/Cookies/keys, and requires lawful data sources; and returns evidence count, source/type distribution, `acquisition_mode=manual_url`, latest preview text, and backend warnings. The backend reuses `POST /api/v1/cases/{case_id}/evidence/attach`, keeps URLs as plain text only, requires at least one of `title`, `body_text`, or `comment_text`, redacts secret-like pasted text values, coerces invalid numeric metrics to `0` with warnings, and preserves the run priority `case_raw_data` > `case_evidence_items` > `mock_data_fallback`. Validation passed with `python -m pytest` (`584 passed in 6.73s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.43s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube API calls in automated tests, real Douyin/Bilibili calls, real LLM calls, URL fetching, scraping, cookies, MediaCrawler integration, secret exposure, or GitHub Actions workflow were introduced. Next recommended task: browser-smoke the Manual URL Evidence UI end-to-end, then plan search discovery or wait for Douyin/Bilibili official API gates.

Latest Evidence Import Template browser smoke QA: verified the CSV template download/import demo path locally. `GET /api/v1/evidence/import/template.csv` returned `200`, `Content-Type: text/csv; charset=utf-8`, `Content-Disposition: attachment; filename="sentigraph_evidence_import_template.csv"`, the expected 18-column header, three safe sample rows, and no secret-like values. The downloaded template was parsed by `POST /api/v1/cases/{case_id}/evidence/import/preview` with `preview_ready`, three valid rows, and evidence types `article`, `video`, and `comment`; commit stored three normalized `EvidenceItem` records with source distribution `news_site:1`, `youtube:1`, `uploaded_dataset:1`; rerunning the case produced `analysis_input_source=case_evidence_items`. Browser smoke on `Cases` confirmed the `下载 CSV 模板` button, template-use helper copy, safety copy (`按模板填写后上传`, lawful data source, formulas not executed, raw file not saved, credentials not read), visible evidence counts, source/type/acquisition tags, and no raw `[object Object]` rendering. `Summary Report` showed imported evidence text, including the Chinese sample comment. Quick page smoke passed for Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmark Dashboard, LLM Safety, and Platform Integration Overview; browser console errors were empty. No real APIs, real YouTube API calls in automated tests, real Douyin/Bilibili calls, real LLM calls, cookies, scraping bypasses, raw-file persistence, secret storage, MediaCrawler integration, or GitHub Actions workflow were introduced. Next recommended task: Manual URL Evidence Import UI, or search discovery planning if manual URL can stay docs-only for now.

Latest Evidence Import Template Download implementation: added a static `GET /api/v1/evidence/import/template.csv` endpoint that returns `sentigraph_evidence_import_template.csv` as a UTF-8 CSV attachment with the full evidence-import header plus safe article, video, and comment sample rows. The template contains no credentials, cookies, API keys, tokens, or private data, and it can be parsed by the existing preview endpoint. The Cases page `导入证据数据` panel now includes `下载 CSV 模板` and clarifies that users should fill the template, ensure lawful data sources, and that Sentigraph does not execute formulas, save raw files by default, or read credentials. Documentation now covers template download in the import guide, API contract, data schema, demo checklist, README boundary table, and backlog. Validation passed with `python -m pytest` (`582 passed in 6.45s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.84s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube API calls in automated tests, real Douyin/Bilibili calls, real LLM calls, scraping, cookies, MediaCrawler integration, secret storage, or GitHub Actions workflow were introduced. Next recommended task: browser-smoke the template download plus upload/preview/commit loop once, then consider a small sample-data gallery only if repeated demos need richer scenarios.

Latest CSV / Excel Evidence Import browser smoke QA: stabilized the user-upload evidence import flow in the local browser before adding template download helpers. Backend endpoint smoke verified `POST /api/v1/cases/{case_id}/evidence/import/preview`, `POST /api/v1/cases/{case_id}/evidence/import/commit`, `GET /api/v1/cases/{case_id}/evidence`, and `POST /api/v1/cases/{case_id}/run` with `backend/app/tests/fixtures/evidence/sample_evidence_import.csv`; preview returned `preview_ready`, commit stored 2 normalized `EvidenceItem` records, `acquisition_mode=user_upload`, safe mode showed `raw_file_persisted=false`, `formulas_executed=false`, and `secrets_exposed=false`, and the rerun case used `analysis_input_source=case_evidence_items`. Browser smoke covered Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmark Dashboard, LLM Safety, and Platform Integration Overview. The Cases page now clearly shows imported evidence source/type/acquisition summaries and representative imported comments without raw `[object Object]` rendering. QA found and fixed two frontend polish issues: imported `case_evidence_items` on cases whose platform list includes YouTube now display `Data: Evidence` instead of incorrectly showing `Data: YouTube Real`, and `PropagationGraphChart` now emits explicit graph node IDs so evidence-backed graphs do not trigger new ECharts duplicate-node console errors. Validation passed with `python -m pytest` (`580 passed in 6.01s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.94s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube calls in automated tests, real Douyin/Bilibili calls, real LLM calls, cookies, scraping bypasses, raw-file persistence, secret storage, MediaCrawler integration, or GitHub Actions workflow were introduced. Next recommended task: add a downloadable CSV import template and a small sample-data helper if repeated demos need an easier starting file.

Latest CSV / Excel Evidence Import implementation: added a safe user-upload import flow so lawful CSV/XLSX datasets can become case-level `EvidenceItem` records without using platform APIs. Backend schemas now include `EvidenceImportPreviewRequest`, `EvidenceImportPreviewResult`, `EvidenceImportCommitRequest`, `EvidenceImportCommitResult`, `EvidenceImportColumnMapping`, `EvidenceImportValidationWarning`, and `EvidenceImportRowPreview`. Added `POST /api/v1/cases/{case_id}/evidence/import/preview` and `POST /api/v1/cases/{case_id}/evidence/import/commit`; both parse file bytes in memory from JSON base64, support UTF-8/UTF-8-BOM/GB18030/GBK CSV and macro-free `.xlsx`, reject unknown binaries and macro-style workbooks, treat formulas as plain text, redact secret-like fields, dedupe rows by deterministic content hash, and persist only normalized evidence items plus safe metadata. Cases now include a `导入证据数据` panel for upload, field mapping, preview, warnings, commit, and `导入后运行分析`. The case-analysis priority remains `case_raw_data` > `case_evidence_items` > `mock_data_fallback`, preserving the YouTube real-data flow and mock fallback. Validation passed with `python -m pytest` (`580 passed in 5.96s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.91s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube calls in automated tests, real Douyin/Bilibili calls, real LLM calls, cookies, scraping bypasses, raw-file persistence, or secret storage were added. Next recommended task: browser-smoke CSV/XLSX import with a local case, then optionally add a richer import template/download helper.

Latest Source Catalog and Evidence browser smoke QA: local backend/frontend browser smoke verified the new Source Catalog and Evidence attach flow before CSV/Excel import. `GET /api/v1/sources/catalog` returned 12 categories and 22 sources as static metadata only, with no credentials, crawler behavior, or real API calls. A manual evidence case (`case_010`) attached article/body evidence, video metadata, comment, reply, and `interaction_metric` records through `POST /api/v1/cases/{case_id}/evidence/attach`; the case then ran with `analysis_input_source=case_evidence_items`, `raw_comment_count=0`, and no secret marker exposed. Browser smoke covered Dashboard, Platform Integration Overview, Cases, Summary Report, Analysis Result, Risk Monitor, Simulation Lab, Benchmark Dashboard, and LLM Safety. The Platform Integration Overview now shows the Source Catalog section directly, including green/yellow/red feasibility labels and the third-party-crawler/no-cookie/no-captcha/no-anti-bot boundary. The Evidence UI shows source distribution, evidence type counts, acquisition modes, top titles, representative public text, and the normalized-evidence safety note without raw `[object Object]` rendering. A small Propagation Graph UI fix now deduplicates visual graph nodes before passing data to ECharts, removing duplicate-node console errors for small/evidence-backed graph data. Validation passed with `python -m pytest` (`573 passed in 5.66s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.71s`; existing non-blocking vendor chunk warning remains). No real APIs, real YouTube API calls in automated tests, real Douyin/Bilibili calls, real LLM calls, cookies, scraping bypasses, secrets, or GitHub Actions workflow were introduced. Next recommended task: add CSV/Excel Evidence Import only if manual evidence demos need it; otherwise continue waiting for Douyin/Bilibili official API gates.

Latest Source Catalog and Evidence Attach QA checkpoint: stabilized `GET /api/v1/sources/catalog` and the case evidence attach/read path before CSV/Excel import. The source catalog remains static metadata only and now exposes separate categories for video platforms, news/media sites, forums/communities, Q&A sites, complaint/review sites, finance/investor forums, social platforms, search discovery, RSS, user-uploaded datasets, manual URL evidence, and future data-vendor integration. Evidence attach QA now covers article/body evidence, video metadata, comments, replies, standalone `interaction_metric` records, `manual_url` and `user_upload` acquisition labels, secret-key redaction from `raw_data_safe`, and the case-run priority order: `case_raw_data` when raw comments exist, `case_evidence_items` when normalized evidence exists without raw comments, and `mock_data_fallback` otherwise. MediaCrawler remains explicitly out of product code, and no cookies, captcha bypass, anti-bot bypass, scraping, real Douyin/Bilibili calls, real YouTube calls in automated tests, or real LLM calls are enabled. Validation passed with `python -m pytest` (`573 passed in 5.91s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.79s`; existing non-blocking vendor chunk warning remains). Next recommended task: browser-smoke the local pages around Cases / Analysis Result evidence summaries, then add CSV/Excel upload only if manual evidence demos need it.

Latest Source Feasibility Matrix and Evidence Layer checkpoint: added `docs/source_feasibility_matrix.md` to classify current/preset sources as green, yellow, or red for compliant evidence acquisition. The matrix covers YouTube, Douyin, Bilibili, Xiaohongshu, Weibo, Reddit, Kuaishou, Zhihu, Douban, Toutiao, The Paper, Jiemian, Hupu, Tieba, NGA, Maimai, user-uploaded CSV/Excel, manual URL import, and search discovery. It records allowed acquisition modes, forbidden modes, currently collectable data, unavailable data, Sentigraph status, and next action for each source. The matrix explicitly states that MediaCrawler must not be integrated as a core data source and that third-party crawler exports can only be treated as user-provided datasets with lawful-source attestation. The Evidence schema now accepts `interaction_metric` as the preferred standalone metric evidence type while preserving `interaction_metrics` as a backward-compatible alias. Cases and Analysis Result evidence panels now show acquisition mode labels and a normalization-only safety note. Added regression coverage that confirms the Evidence layer accepts standalone interaction metrics and that MediaCrawler is not referenced by product source code. Validation passed with `python -m pytest` (`567 passed in 5.28s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 7.61s`; existing non-blocking vendor chunk warning remains). Next recommended task: QA-stabilize evidence attach in the browser, then add CSV/Excel import only if manual evidence UX requires it.

Latest Universal Evidence Ingestion Layer: implemented a source-neutral evidence normalization layer so Sentigraph can attach event evidence from official API outputs, OAuth-authorized data, public-parser fixtures, search discovery, CSV/Excel/manual imports, future data vendors, or mock fixtures without changing downstream analysis contracts. Added `EvidenceItem`, `EvidenceSource`, `EvidenceType`, `EvidenceIngestionBatch`, `EvidenceIngestionResult`, and `EvidenceNormalizationMetadata`; added converters for YouTube `RawPost` / `RawComment`, public-parser article/forum records, and manual/user-provided evidence payloads; added case fields for `evidence_items` and `evidence_item_count`; added `GET /api/v1/cases/{case_id}/evidence` and `POST /api/v1/cases/{case_id}/evidence/attach`; and updated the case pipeline so attached raw comments still preserve the existing YouTube `analysis_input_source=case_raw_data` flow, while manual normalized evidence can feed analysis as `analysis_input_source=case_evidence_items` when no raw comments are present. Cases and Analysis Result now show a compact Evidence summary with source distribution, evidence type counts, top titles, and representative public text. `raw_data_safe` removes secret-like keys, and the feature does not call real APIs, real Douyin/Bilibili APIs, real LLM APIs, crawlers, or live public fetch. Validation passed with `python -m pytest` (`565 passed in 5.59s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.38s`; existing non-blocking vendor chunk warning remains). Next recommended task: QA-stabilize the Evidence layer with browser smoke on a manually attached evidence case, then add CSV/Excel import only if needed.

Latest README and project-status consolidation: refreshed `README.md` so the first screen now reflects the current true state before historical MVP notes. The README now opens with what Sentigraph is, current demo-ready status, the optional local YouTube real-data path, offline deterministic analysis/report/forecast/Simulation Lab boundaries, mock LLM status, pending platform work, local run commands, and demo-doc links. Added a concise real-vs-mock boundary table covering YouTube data, analysis pipeline, V1.5 risk, Chinese report, forecasting, Simulation Lab, LLM, Douyin, Xiaohongshu, Reddit/Bilibili/Weibo, and public parsers. Older mock-first notes are now clearly labeled as historical/default-mode context so they do not override the current YouTube real-data demo state. `docs/implementation_backlog.md` now records this consolidation as complete. Documentation was checked for overclaims: it does not say all platforms are real, does not say real LLM is active, does not imply guaranteed predictions, does not imply Simulation Lab or content moderation executes real-world actions, does not imply individual targeting, and does not imply Douyin real API integration is complete. Validation passed with `python -m pytest` (`559 passed in 5.93s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`; safety line says no real LLM calls, no real platform API calls, live fetch disabled), and `npm run build` from `frontend/` (`built in 8.15s`; existing non-blocking vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no real YouTube or Douyin APIs were called in automated validation, no API keys or `.env` values were printed, no scraping was implemented, and no real LLM APIs were called. Next recommended task: produce the actual screenshots/recording while continuing Douyin console verification.

Latest v6.3 Demo Package Final: refreshed the external-presentation package for the YouTube real-data end-to-end demo. `docs/demo_recording_script.md` now includes the 3-minute and 8-minute scripts, page order, talking points, proof notes, safety guardrails, and explicit Douyin readiness wording. `docs/demo_screenshot_checklist.md` now defines the canonical 14-screen capture sequence: Dashboard, Keyword Search, Cases, Analysis Result, Summary Report, Propagation Graph, Risk Monitor / Forecast, Simulation Lab initialized from case, Simulation Lab A/B comparison, strategy report export, Benchmark Dashboard, LLM Safety, Platform Integration Overview, and Douyin readiness / platform status if visible. `docs/demo_package.md`, `docs/demo_story.md`, `docs/demo_checklist.md`, `README.md`, and `docs/implementation_backlog.md` now clearly distinguish real YouTube public video/comment data, offline deterministic analysis/risk/report/forecast/Simulation Lab, mock LLM provider, mock/scaffold/pending status for other platform APIs, and Douyin Web App OAuth / `item.comment` pending status. Documentation was checked for overclaims: it does not say all platforms are real, does not say real LLMs are integrated, does not imply guaranteed predictions, does not imply Simulation Lab or content moderation executes real-world actions, does not imply individual targeting, and does not imply Douyin real API integration is complete. Validation passed with `python -m pytest` (`559 passed in 5.62s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm run build` from `frontend/` (`built in 7.96s`; existing non-blocking vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no real YouTube or Douyin APIs were called in automated validation, no API keys or `.env` values were printed, no scraping was implemented, and no real LLM APIs were called. Next recommended task: produce the actual screenshot set and short recording from the v6.3 package, then continue Douyin console verification.

Latest final YouTube real-data demo UI polish QA stabilization: verified the screenshot-ready provenance labels and report wording after the final polish pass. YouTube raw-data cases route through the shared source-status helper and surface `Data: YouTube Real`, `Analysis: Offline`, `LLM: Mock`, and `analysis_input_source=case_raw_data` on the header, Analysis Result, Summary Report, Cases, Demo Flow, and YouTube-only Keyword Search flow; mock fallback cases surface `Data: Mock`, `Analysis: Offline`, and `LLM: Mock`. The UI no longer collapses real-data YouTube cases into a generic `Mock Mode` label, raw-data wording uses "Offline deterministic analysis from attached case raw data" and "YouTube public video/comment data", Summary Report Markdown downloads use `text/markdown;charset=utf-8`, and the shortened Simulation sidebar label retains the full tooltip for recordings. Representative-comment QA coverage is present for down-ranking promo/self-promo comments while preserving all attached raw YouTube comments. Frontend validation passed with `npm run build` from `frontend/` (`built in 7.95s`; existing non-blocking vendor chunk warning remains). Backend tests and offline benchmarks were not rerun in this QA pass because no backend code changed. GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no real YouTube or Douyin APIs were called, no API keys or `.env` values were printed, no scraping was implemented, and no real LLM APIs were called. Next recommended task: capture the final screenshot/recording sequence, then continue Douyin console verification.

Latest final YouTube real-data demo UI polish: tightened screenshot/recording labels without changing backend ingestion or adapter behavior. The global app header fallback now uses the same provenance vocabulary as selected cases (`Data: Mock`, `Analysis: Offline`, `LLM: Mock`) instead of a single generic `Mock Mode` chip, the YouTube-only Keyword Search panel now matches the screenshot target with `Data: YouTube Real`, `Analysis: Offline`, and `LLM: Mock`, Summary Report distinguishes `Attached raw-data report` from `Mock fallback report`, and the Simulation Lab sidebar label is shortened to `Simulation / 沙盘` while preserving the full tooltip. The demo checklist now explicitly notes that data source and analysis mode are separate, that Douyin remains OAuth/scope-pending, and that real Douyin API calls remain disabled. Frontend validation passed with `npm run build` from `frontend/` (`built in 7.81s`; existing non-blocking vendor chunk warning remains). Backend tests and offline benchmarks were not rerun because this pass changed frontend/docs only. GitHub Actions CI remains intentionally disabled, no workflow was recreated, no real YouTube or Douyin APIs were called in automated validation, no API keys or `.env` values were printed, no scraping was implemented, and no real LLM APIs were called. Next recommended task: capture the final YouTube real-data screenshot/recording sequence, then continue Douyin console verification.

Latest Real Data Source Readiness Framework QA stabilization: verified the unified readiness/status response fields across `/api/v1/platforms/readiness`, `/api/v1/platforms/status`, and the legacy `/api/v1/platforms` registry surface. Added regression coverage that asserts every platform returns the required readiness fields (`platform`, `integration_type`, developer/API/OAuth/scope status, `credential_present`, required credentials/scopes, real-mode gates, data access level, notes, and next user action), confirms YouTube is `integration_type=official_api` with `data_access_level=public_video_comment_data` and quota/cache protection, confirms Douyin is `integration_type=official_api_oauth` with Web App developer access obtained but `item.comment`, `redirect_uri`, whitelist/test account, and `item_id` source still pending, and confirms Weibo, Reddit/Bilibili, and Xiaohongshu blockers. The response still exposes credential presence only as booleans and does not return API keys, client secrets, access tokens, refresh tokens, or `.env` values. Validation passed with `python -m pytest` (`559 passed in 5.41s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm run build` from `frontend/` (`built in 7.82s`; existing non-blocking vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no real YouTube or Douyin APIs were called in automated tests, no scraping was implemented, and no real LLM APIs were called. Next recommended task: browser-smoke Platform Integration Overview with a local backend/frontend, then continue Douyin console verification.

Latest Real Data Source Readiness Framework update: implemented and validated a unified platform readiness/status layer for official API, OAuth, permission, credential, and quota/cache gates. `GET /api/v1/platforms/readiness` now returns the same safe response shape as `/platforms/status` plus normalized fields such as `integration_type`, `required_credentials`, `required_scopes`, `scope_status`, `oauth_required`, `oauth_status`, `real_mode_configured`, `credential_present`, `data_access_level`, `next_user_action`, and `quota_cache_protected`; credential values remain redacted as booleans only. YouTube is marked `integration_type=official_api`, `data_access_level=public_video_comment_data`, real-capable, and real-selectable only when `YOUTUBE_ADAPTER_MODE=real` and `YOUTUBE_API_KEY` are both configured; Douyin is marked `integration_type=official_api_oauth` with Web App developer access obtained but OAuth and `item.comment` still pending; Weibo records `company_age_requirement_pending`; Reddit/Bilibili record approval-pending blockers; Xiaohongshu records unknown/unconfirmed comment API status. Platform Integration Overview now groups sources into Real Ready, Configured but Guarded, Permission Pending, OAuth Pending, and Mock / Scaffold Only. Validation passed with `python -m pytest` (`558 passed in 6.02s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 7.94s`; existing non-blocking vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no secrets or `.env` values were printed, no real platform APIs were called in automated tests, no scraping was implemented, and no real LLM APIs were called. Next recommended task: run a browser smoke pass on Platform Integration Overview and then continue with Douyin console verification or screenshot production.

Latest Douyin Web App OAuth and item.comment readiness QA: stabilized the readiness scaffold without enabling real Douyin calls. Added the missing `.env.example` placeholders for `DOUYIN_CLIENT_TOKEN`, `DOUYIN_STABLE_CLIENT_TOKEN`, `DOUYIN_ENABLE_REAL_CALLS=false`, and `DOUYIN_SCOPE_STATUS=unverified`; added safe readiness fields for `recommended_comment_scope="item.comment"` and `video_comment_scope_status="not_recommended_for_mvp"`; added a non-network OAuth helper scaffold for authorization URL construction, redirect URI validation, callback parsing, and token-exchange/refresh placeholders; and added `docs/douyin_console_checklist.md` for console-side verification. Real mode remains blocked in mock fallback with safe `permission_not_verified` / `config_error` metadata, and the token placeholder functions intentionally raise no-network placeholder errors. Focused Douyin OAuth/adapter/registry/crawl validation passed with `69 passed in 0.97s`; full backend validation passed with `python -m pytest` (`556 passed in 5.13s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). Frontend build was not run because no frontend files changed. GitHub Actions CI remains intentionally disabled, no workflow was recreated, no credentials or `.env` values were printed, no real Douyin API was called, no scraping or bypass path was added, and no real LLM API was called. Next recommended task: verify the Douyin console gates (Web App redirect URI, test-account OAuth, `item.comment` approval, `video.list.bind` / `video.data.bind`, token flow, daily test quota, and lawful `item_id` source) and capture sanitized official payload fixtures before any real-mode implementation.

Latest v6.3 Screenshot / Recording Demo Package Final: prepared the final external-presentation docs for the YouTube real-data demo path. Added `docs/demo_recording_script.md` with 3-minute and 8-minute scripts, exact page order, what to say on each page, what each page proves, and presenter guardrails. Added `docs/demo_screenshot_checklist.md` with a capture sequence that is now superseded by the 2026-05-20 canonical 14-screen checklist including Douyin readiness / platform status. Updated `docs/demo_package.md` as the v6.3 package overview, refreshed `README.md` with a short Demo Package section, linked the new package docs from `docs/demo_story.md`, and updated `docs/demo_checklist.md` with the v6.3 capture target. The docs explicitly distinguish real YouTube public video/comment data, offline deterministic analysis/risk/report/forecast/Simulation Lab, mock LLM provider, mock/scaffold status for other platforms, and Douyin readiness as pending rather than integrated. Validation passed with `python -m pytest` (`544 passed in 6.00s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.05s`; existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no real YouTube API calls were made in automated tests, no real LLM APIs were called, no scraping was implemented, and no product features were added. Next recommended task: produce the actual screenshot set and short recording from `docs/demo_screenshot_checklist.md` and `docs/demo_recording_script.md`, then consider the Douyin Web App permission audit.

Latest v6.2 YouTube real-data demo polish QA: revalidated the final screenshot/recording polish for provenance badges, raw-data wording, representative-comment quality, Keyword Search YouTube-only flow, Markdown report wording, and local validation health. YouTube cases with `analysis_input_source=case_raw_data` show `Data: YouTube Real`, `Analysis: Offline`, and `LLM: Mock`; mock-fallback cases show `Data: Mock`, `Analysis: Offline`, and `LLM: Mock`. Analysis Result, Summary Report, Cases, and Demo Flow expose `analysis_input_source` or equivalent data-source context where useful, and raw-data wording uses offline deterministic analysis language without implying real LLM usage. Keyword Search keeps the default mock flow, shows the explicit YouTube real-data flow only for YouTube-only selection, and warns `For YouTube real-data demo, select YouTube only.` for mixed selections. Representative-comment QA confirms raw comments remain stored while promo/self-promo terms such as `patreon`, `channel member`, `subscribe`, `referral`, `promo code`, `affiliate`, `join`, `merch`, and `discount code` are down-ranked when substantive alternatives exist. Validation passed with `npm --prefix frontend run build` (`built in 8.08s`; existing non-blocking large vendor chunk warning remains), `python -m pytest` (`544 passed in 5.99s`), and `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the manual YouTube real-data browser demo with local credentials and capture the final screenshot/recording sequence.

Latest final YouTube real-data demo polish: completed screenshot/recording polish for data-source, analysis-mode, and LLM-mode wording. The app header now shows `Data: YouTube Real`, `Analysis: Offline`, and `LLM: Mock` for YouTube cases with `analysis_input_source=case_raw_data`; mock-fallback cases show `Data: Mock`, `Analysis: Offline`, and `LLM: Mock` instead of a single generic `Mock Mode` label. Cases now includes a current-case data-source card, Dashboard wording switches between attached raw-data analysis and mock visualization, Keyword Search warns `For YouTube real-data demo, select YouTube only.` for mixed selections, and Markdown report export labels raw-data cases as attached case raw data offline deterministic analysis with no real LLM call. Representative-comment ranking keeps all raw YouTube comments stored but now down-ranks `patreon`, `channel member`, `subscribe`, `referral`, `promo code`, `affiliate`, `join`, `merch`, and `discount code` when substantive event/topic comments exist. Validation passed with `python -m pytest` (`544 passed in 6.40s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.57s`; existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the manual YouTube real-data browser demo with local credentials and capture the final screenshot/recording sequence.

Latest Keyword Search YouTube real-data frontend QA stabilization: rechecked the browser-only YouTube case flow against the case crawl/run API contract and stabilized the post-run UX. The existing `Create Case & Run Mock Analysis` path remains unchanged. When the platform selector is exactly `youtube`, Keyword Search shows `Create YouTube Real Case`, `Crawl YouTube & Attach Raw Data`, and `Run Case Analysis`; when YouTube is mixed with other platforms it warns `For YouTube real-data demo, select YouTube only.` The YouTube panel displays safe crawl metadata (`raw_data_status`, raw counts, `adapter_mode`, `fallback_used`, `fallback_reason_category`, `cache_hit`, `quota_guardrail_status`) and post-run metadata (`analysis_input_source`, raw counts, representative-comments preview). The run step now stays on Keyword Search so users can verify `analysis_input_source=case_raw_data`, then use explicit shortcuts to Analysis Result, Summary Report, Risk Monitor, or Simulation Lab. Frontend validation passed with `npm --prefix frontend run build` (`built in 7.81s`; existing non-blocking large vendor chunk warning remains). Backend tests and offline benchmarks were not rerun because no backend code changed. No API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated validation, no real LLM APIs were called, and GitHub Actions CI remains intentionally disabled.

Latest YouTube real-data demo UX polish: clarified real-data demo provenance across the app and report path. Completed YouTube cases with `analysis_input_source=case_raw_data` now display `Data: YouTube Real`, `Analysis: Offline`, `LLM: Mock`, and `analysis_input_source=case_raw_data` in the app header, Analysis Result, Summary Report, and Demo Flow surfaces instead of flattening the experience into generic `Mock Mode`. The Simulation Lab sidebar item is shortened to `Simulation Lab` with a tooltip for `Simulation Lab / 舆情预演沙盘`, reducing awkward truncation in screenshots. Backend summary wording now says `Offline deterministic analysis from attached case raw data` when raw case data is used. Representative-comment selection keeps all raw YouTube comments stored but down-ranks obvious channel promotion/self-promo comments (`patreon`, `channel member`, `subscribe`, `referral`, `promo code`, `affiliate`, `join`) when substantive event-relevant alternatives exist. Validation passed with `python -m pytest` (`544 passed in 5.95s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.11s`; existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the manual YouTube real-data browser demo with local credentials and capture the badge screenshot plus end-to-end report screenshots.

Latest YouTube real-data demo browser-smoke documentation QA: verified `docs/youtube_real_data_demo.md`, `docs/demo_story.md`, and `docs/demo_checklist.md` document the full manual path after quota-safe cache stabilization: start backend/frontend, run a tiny `platforms=["youtube"]` crawl, confirm `adapter_mode=real`, confirm cache behavior, attach/store YouTube `raw_posts` / `raw_comments` on a case, run case analysis with `analysis_input_source=case_raw_data`, confirm YouTube-derived representative comments, view the Chinese Summary Report, export Markdown, open Risk Monitor/Forecast, initialize Simulation Lab from the YouTube-based case, run A/B strategy comparison, export the Simulation Lab strategy report, and open Benchmark Dashboard plus LLM Safety. Tightened the concise demo story so cache-hit verification is explicit. Validation passed with `python -m pytest` (`542 passed in 5.18s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm run build` from `frontend/` (`built in 7.59s`; existing non-blocking large chunk warning remains). GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: perform the manual YouTube real-data browser smoke with local credentials and capture the documented screenshot sequence.

Latest YouTube quota-safe cache/guardrail implementation pass: rechecked the existing credential-gated YouTube Data API v3 adapter, project-local cache, `.env.example` guardrail defaults, `.gitignore` runtime cache protection, safe cache keys, safe crawl metadata, mocked-only error handling, and case raw-data ingestion compatibility. No extra API surface was added because manual cache clearing is already documented and avoids exposing a new operator endpoint. Validation passed with `python -m pytest` (`542 passed in 5.42s`) and `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). Frontend build was not run because no frontend files changed. `backend/data/youtube_cache.json` was absent after automated tests, `backend/data/.gitkeep` remains preserved, and `.gitignore` covers the runtime cache file. GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed or stored, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the documented manual YouTube real-data demo with local credentials and capture the screenshot/recording sequence.

Latest YouTube Real Data Demo Story update: created `docs/youtube_real_data_demo.md` as the manual local walkthrough for the credential-gated YouTube path from tiny official API crawl through case raw-data attachment, `analysis_input_source=case_raw_data`, V1.5 topic risk, Chinese Markdown report export, Risk Monitor/Forecast, Simulation Lab initialization, A/B strategy rehearsal, strategy report export, Benchmark Dashboard, LLM Safety, and Platform Integration Overview. Updated `docs/demo_story.md`, `docs/demo_checklist.md`, `README.md`, and `docs/implementation_backlog.md` to clearly separate the default mock/offline demo from the optional YouTube real-data demo that requires a local ignored `.env` key. Validation passed with `python -m pytest` (`542 passed in 6.32s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm --prefix frontend run build` (`built in 8.26s`, existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: run the manual YouTube real-data demo locally with the configured key and capture the screenshot/recording sequence from `docs/youtube_real_data_demo.md`.

Latest YouTube quota-safe cache/guardrail QA stabilization: revalidated the project-local YouTube Data API cache at ignored runtime path `backend/data/youtube_cache.json`, `.env.example` guardrail defaults, safe cache key fields, safe crawl metadata, and case raw-data ingestion compatibility. Focused QA passed for cache miss/hit/TTL expiry, mocked-client call avoidance on cache hit, limit clamping, disabled deep replies by default, total-comment caps, comments-disabled partial output, quota-error fallback, API-key redaction, local JSON/MongoDB-shaped raw-data persistence, and YouTube-derived case analysis. Full validation passed with `python -m pytest` (`542 passed in 5.14s`) and `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). Frontend build was not run because no frontend files changed. `backend/data/youtube_cache.json` was absent after automated tests, `backend/data/.gitkeep` remains preserved, and `.gitignore` covers the runtime cache file. GitHub Actions CI remains intentionally disabled, no workflow was recreated, no API keys or `.env` values were printed or stored, no scraping was implemented, no real YouTube API calls were made in automated tests, and no real LLM APIs were called. Next recommended task: capture the manual YouTube real-data demo screenshots using the cache-enabled local path, then consider a tiny read-only quota/cache status panel if repeated demos need it.

Latest YouTube Real Data Full Demo QA update: verified the credential-gated YouTube path through normalized crawl output, explicit case raw-data attachment, case analysis, report export, monitoring/forecasting, and Simulation Lab initialization/report surfaces without making automated real YouTube calls. The supported real-data path is explicit: `/api/v1/cases/{case_id}/crawl/start` attaches normalized YouTube `RawPost` / `RawComment` output, then `/api/v1/cases/{case_id}/run` uses stored raw comments and records `analysis_input_source="case_raw_data"` plus raw counts; cases without attached raw comments still fall back to `mock_data_fallback`. Added mocked-real fallback coverage for comments-disabled/quota/auth-style errors and network errors, plus docs noting `search.list`, `videos.list`, `commentThreads.list`, future `comments.list` reply expansion, tiny limits, key-redacted metadata, and the manual Windows full-demo command path. Validation passed with `python -m pytest` (`533 passed in 5.30s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), and `npm run build` from `frontend/` (`built in 7.66s`, existing non-blocking large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no API keys or `.env` values were printed, no scraping was implemented, no real YouTube API calls were made in automated tests, no real LLM APIs were called, and live public fetching was not enabled. Next recommended task: capture the manual YouTube real-data demo screenshots, then consider a small guarded frontend attach-control if repeated real-data demos become common.

Latest Case Raw Data Ingestion QA update: revalidated the explicit case-scoped crawl attachment flow and tightened regression coverage for persistence and report provenance. `POST /api/v1/cases/{case_id}/crawl/start` uses the case keyword/platforms by default, calls the existing adapter layer, stores `raw_posts`, `raw_comments`, safe `crawl_metadata`, `crawl_attached_at`, and `raw_data_status` on `AnalysisCaseDetail`, and returns credential presence only as booleans. `POST /api/v1/cases/{case_id}/run` uses stored case `raw_comments` when present and records `analysis_input_source="case_raw_data"` plus raw counts; if no raw comments are attached, it falls back to the deterministic mock dataset with `analysis_input_source="mock_data_fallback"`. Added QA tests that verify local JSON reload persistence, MongoDB-shaped persistence through the fake store, YouTube fixture comments appearing in Markdown, old mock comments not appearing when raw case comments exist, and no raw-data JSON dump in the Markdown report. YouTube public comments can feed V1.5 topic risk, Chinese reports, representative comments, monitoring snapshots, forecasting inputs, and Simulation Lab initialization after explicit attachment. Automated tests use mocked crawl output only; no real YouTube API call, scraping, cookie use, login bypass, captcha bypass, anti-bot evasion, API-key printing, `.env` modification, live public fetch, or LLM API call was introduced. Backend validation passed with `python -m pytest` (`531 passed in 5.22s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). Frontend build was not run because no frontend files changed. GitHub Actions CI remains intentionally disabled.
...
Latest v5.4 Demo Polish and One-Click Demo Flow update: added a `Demo Flow / 演示流程` sidebar page that guides a local end-to-end Sentigraph demo through Tesla mock case preparation, mock analysis, V1.5 risk review, Chinese report and Markdown export, Simulation Lab initialization, A/B strategy comparison, Simulation Lab strategy report export, Benchmark Dashboard, and LLM Safety status. The page includes one-click demo prep using existing local APIs, status badges, page-opening buttons, local Windows demo commands, and explicit mock/offline/no-real-API safety copy. `scripts/seed_demo_cases.py` now reports forecast and case-to-simulation initialization readiness, and `scripts/api_smoke_check.py` covers forecast endpoints plus Simulation Lab demo scenario/run/report export. Backend validation passed with `python -m pytest` (`511 passed in 4.88s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`); frontend validation passed with `npm run build` from `frontend/` (`built in 8.06s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains); local API smoke passed against a temporary backend on port 8010 (`38 passed, 0 failed`). GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no real APIs or real LLM APIs were called, live public fetching remains disabled, and no manipulation tactics were implemented. Next recommended task: browser smoke the new Demo Flow page and capture a short screenshot/demo story, then consider PDF export or a policy/legal review checklist.

Latest Simulation Lab completion audit: audited the Simulation Lab design, backend services, frontend page, Demo Flow page, API helpers, routes, scripts, and benchmark coverage without adding product features. The current Simulation Lab MVP is demo-ready for local mock/offline use: research/design docs, backend MVP, bubble visualization, A/B comparison, lawful content visibility backlash modeling, case-to-simulation initialization, Markdown strategy report export, Demo Flow integration, ethics policy enforcement, and offline benchmark coverage are present. Backend validation passed with `python -m pytest` (`511 passed in 5.27s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, including `simulation_lab`, `simulation_strategy_report`, and `case_to_simulation_initializer`, `no_regression`); frontend validation passed with `npm run build` from `frontend/` (`built in 7.78s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains). `.github/workflows/ci.yml` is absent and GitHub Actions CI remains intentionally disabled. A safety scan found only local smoke URLs and fixture-only `example.invalid` benchmark data; no real APIs, real LLM APIs, live public fetching, manipulation tactics, individual targeting, or account-level influenceability scoring were introduced or exposed as usable UI options. Remaining non-blocking gaps are richer animation, empirical calibration, browser screenshot demo capture, optional PDF export, and a policy/legal review workflow. Exact next task: browser-smoke the Demo Flow plus Simulation Lab path and capture a concise demo story/screenshots.

Latest browser smoke and demo story update: validated the local demo path in a real browser runtime and created `docs/demo_story.md`. Deterministic demo seeding produced `case_001` with forecast and case-to-simulation readiness; local API smoke passed against `http://127.0.0.1:8000` (`38 passed, 0 failed`). Browser smoke covered Demo Flow, Dashboard, Cases, Analysis Result, Summary Report, Risk Monitor, Simulation Lab, Benchmarks, LLM Safety, Platform Integration Overview, Public Parser Status, and Selector Repair Tool. Simulation Lab case initialization from `case_001`, single-scenario run, A/B strategy comparison, lawful content visibility tradeoff display, strategy report export, and scoped allowed-intervention dropdown were verified; forbidden tactics were not selectable and no raw `[object Object]` rendering was observed. Captured local screenshots under `.benchmarks/demo_smoke_screenshots/` and documented the recommended screenshot story in `docs/demo_story.md`. Fixed a small LLM Safety table polish issue by replacing the deprecated index-based Ant Design `rowKey` pattern with a stable `record_key`. Validation passed with `npm run build` from `frontend/` (`built in 8.17s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains), `python -m pytest` (`511 passed in 5.19s`), and `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`). GitHub Actions CI remains intentionally disabled and `.github/workflows/ci.yml` was not recreated. No real APIs, real LLM APIs, live public fetching, real crawlers, real notifications, or manipulation tactics were enabled. Next recommended task: package the demo handoff with selected screenshots and then choose between optional PDF export or a policy/legal review checklist.

Latest v5.6 demo package and release-readiness update: packaged Sentigraph as a local mock/offline demo MVP for collaborator and reviewer walkthroughs. Added `docs/demo_package.md` with the demo story, screenshot sequence, exact Windows run commands, talking points, screen-by-screen proof notes, known limitations, and safety/ethics boundaries. Added `docs/release_readiness.md` with completed capabilities, demo-ready features, mock/scaffold-only features, real API pending items, real LLM pending items, non-blocking known issues, validation expectations, and next-stage options. Updated `README.md` with a top-level v5.6 demo-ready snapshot that clearly states what the demo can show, what remains mock/offline, what needs future platform approval, how to run locally, how to validate, and that GitHub Actions CI remains intentionally disabled. Fresh validation passed with `python -m pytest` (`511 passed in 5.25s`), `python scripts/run_offline_benchmarks.py` (`522 passed, 0 failed, 0 warnings`, `no_regression`), `npm run build` from `frontend/` (`built in 7.93s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains), and optional local API smoke against the already-running backend at `http://127.0.0.1:8000` (`38 passed, 0 failed`). The current phase is demo package / release-readiness documentation; docs intentionally avoid claiming real platform APIs, real LLMs, live crawlers, guaranteed predictions, or real-world action execution. Recommended next task: produce the final screenshot deck or short demo recording from `docs/demo_package.md` and `.benchmarks/demo_smoke_screenshots/`, then decide whether to do Douyin/Xiaohongshu permission verification or a Simulation Lab policy/legal review checklist.
...
Latest Simulation Lab content visibility intervention QA stabilization update: revalidated the deterministic, offline content visibility backlash model, backend schemas, ethics policy, `POST /api/v1/simulation/run` visibility-result shape, Simulation Lab A/B UI, and benchmark coverage. Added regression tests for supported visibility intervention registration, high-reach exposure reduction sensitivity, reactance-driven opposition impact, 0-100 score clamping, required high-reach scenario groups, API visibility payload handling, aggregate-only output, and target-list absence. Browser smoke against local backend/frontend verified the A/B default `no_response` vs `透明说明后内容移除` flow, the `内容可见性干预` panel labels, human-review copy, no real API/LLM copy, no automatic platform-action execution, and no forbidden tactic values in selectable options. A small frontend polish changed the Suspense fallback loader to avoid noisy Ant Design `Spin` fallback warnings. Backend validation passed with `python -m pytest` (`488 passed in 4.17s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`483 passed, 0 failed, 0 warnings`, including `simulation_lab: 36 cases`); frontend validation passed with `npm run build` from `frontend` (`built in 7.90s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no real APIs or real LLM APIs were called, live public fetching remains disabled, no illegal/covert suppression or manipulation tactics were implemented, and outputs remain aggregate-level and human-review-oriented. Next recommended task: implement a safe aggregate assumption editor or A/B strategy report export before empirical calibration or real-data replay.

Latest Simulation Lab content visibility intervention update: implemented deterministic, offline backlash modeling for lawful/platform-authorized content visibility actions. Backend schemas now include `VisibilityIntervention`, `BacklashModel`, `AudienceImpactBreakdown`, and `VisibilityInterventionResult`; the ethics policy allows `content_removal`, `comment_closure`, `account_restriction`, `visibility_reduction`, `platform_labeling`, `policy_enforcement_notice`, and `content_removal_with_explanation`, while explicitly rejecting illegal/covert suppression, fake consensus, bot amplification, fake events, deceptive diversion, covert seeding, individual targeting, and platform-governance evasion. The model estimates exposure reduction, backlash cost, trust loss, spillover risk, net risk change, removal legitimacy, transparent explanation quality, neutral-audience impact, and hard-opposition impact, and always returns human-review-oriented aggregate output rather than platform actions or target lists. The Simulation Lab A/B UI now exposes safe visibility options only when allowed by the backend policy and shows a `内容可见性干预` tradeoff panel. Browser smoke against local backend/frontend verified the A/B default `no_response` vs `透明说明后内容移除` flow, the visibility tradeoff labels, and the absence of forbidden tactic values from selectable options. Backend validation passed with `python -m pytest` (`482 passed in 4.47s`); offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`483 passed, 0 failed, 0 warnings`, including `simulation_lab: 36 cases`); frontend validation passed with `npm run build` from `frontend` (`built in 7.67s`, existing non-blocking Ant Design/ECharts large vendor chunk warning remains). GitHub Actions CI remains intentionally disabled, `.github/workflows/ci.yml` was not recreated, no real APIs or real LLM APIs were called, live public fetching remains disabled, and no illegal/covert suppression or manipulation tactics were implemented. Next recommended task: add a safe aggregate assumption editor or richer A/B replay controls.
...
Latest platform selection update: Sentigraph now exposes Reddit, the Chinese official-API-planned platforms, and YouTube as mock-selectable MVP choices. YouTube is also real-capable when explicitly configured through the official YouTube Data API v3; other official API platforms remain mock/scaffold-only until permissions and compliant implementations are confirmed.
...
| Platform registry | complete | 10 mock-selectable platforms are active, including YouTube mock mode; crawler-later platforms remain inactive for real crawling, and YouTube real mode is credential-gated. |
...
- The previous frontend only showed Weibo because the backend registry had only `weibo` marked `enabled_in_mvp=true`, and `frontend/src/App.jsx` filtered platform options only by that field. This has been fixed by adding `selectable_for_mock` and marking Reddit, Weibo, Bilibili, Douyin, Kuaishou, Xiaohongshu, Zhihu, Douban, Toutiao, and YouTube as safe mock-selectable platforms.
- Crawler-later platforms remain visible but disabled with the note `Future crawler integration`. YouTube is active for mock selection and is the first real-capable official API adapter when explicitly configured with a local API key.
...
## 6.15 Simulation Lab Event Frame and Audience Initialization Design

Update date: 2026-05-18.

Status: documentation-only design complete.

What changed:

- Added `docs/simulation_event_frame_design.md` to define how one event is decomposed into sub-issues such as product quality, response delay, pricing dispute, safety/legal issue, brand trust, suspected manipulation, public figure controversy, and workplace/company issue.
- Added `docs/simulation_audience_initialization.md` to define aggregate audience segments, observed-community initialization, synthetic bubble/agent mappings, persona-cluster weights, and ordinary-public baseline assumptions.
- Added `docs/simulation_baseline_gap_analysis.md` to compare the observed frame with an external public baseline and classify frames as aligned, more negative, more positive, polarized, manipulation-suspected, or insufficient.
- Added `docs/simulation_strategy_policy.md` to define ethical strategy implications, lawful content visibility tradeoff modeling, and disclosed creator/expert communication modeling.
- Updated `docs/simulation_ethics.md` with Event Frame / Audience Initialization ethics and creator/expert communication boundaries.

QA stabilization:

- Verified the proposed schemas are present: `EventFrame`, `SubIssue`, `AudienceSegment`, `BaselinePublicProfile`, `ObservedFrameProfile`, `FrameGapAnalysis`, `PersonaCluster`, `StrategyImplication`, `ContentVisibilityTradeoff`, and `CreatorCommunicationPlan`.
- Clarified Event Frame relationships to V1.5 topic risk, monitoring, forecasting, Simulation Lab, and report generation.
- Clarified observed attention-level and action-threshold mapping into synthetic aggregate audience initialization.
- Tightened creator/expert communication wording to allow only disclosed, truthful, transparent collaboration and to forbid fake consensus, bot amplification, fake grassroots, covert paid praise, undisclosed paid defense, false testimonials, harassment campaigns, and individual targeting.

Validation:

- Backend tests passed with `python -m pytest` (`488 passed in 4.03s`).
- Offline benchmarks passed with `python scripts/run_offline_benchmarks.py` (`483 passed, 0 failed, 0 warnings`; latest regression status `no_regression`).
- Frontend build was not run because this task did not change frontend files.

Safety and scope:

- This was documentation only.
- No product code, schemas, dependencies, real APIs, real LLM APIs, live public fetching, crawlers, platform actions, or automatic interventions were added.
- The design remains aggregate-only and forbids individual persuasion profiles, account-level influenceability scoring, covert creator seeding, fake grassroots, fake consensus, bot amplification, deceptive diversion, illegal/covert suppression, and target lists.
...
- Keep the Simulation Lab frontend visualization limited to existing backend toy simulator endpoints unless adding a safe report-export endpoint that reads existing aggregate results only.
- Do not use real accounts, real platform APIs, real LLM APIs, live public fetching, or crawler behavior.
- Keep the simulator synthetic, deterministic, aggregate-only, human-review-oriented, and benchmarked.
