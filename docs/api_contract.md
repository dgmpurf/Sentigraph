...
- When `platforms` explicitly contains a registered public-parser scaffold such as `the_paper`, `jiemian`, or `hupu`, the endpoint calls the public parser adapter through `adapter_factory.get_adapter(platform_id)`.
- The Paper, Jiemian, and Hupu public parsers currently run in `fixture_only` mode and return safe fixture/mock `RawPost` data by default.
