...
## 10.3 Offline Benchmark Summary

The benchmark summary endpoint is a read-only local diagnostics endpoint. It reads the latest JSON summary generated by `scripts/run_offline_benchmarks.py` and never starts a benchmark run from the API.
...
- The endpoint does not run benchmarks automatically.
- It returns suite-level summary fields only. It intentionally omits per-case benchmark payloads.
- It must not expose raw fixture text, raw HTML, prompts, raw user content, API keys, `.env` values, or external request bodies.
- Malformed summary files return a safe `status="malformed"` response with zero totals and no traceback.
