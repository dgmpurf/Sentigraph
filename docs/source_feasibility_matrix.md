...
| search discovery | `search_discovery` metadata after approved provider or user-provided export; `manual_url` | Search scraping, SERP scraping without permission, anti-bot bypass | Schema-level placeholder via manual evidence | Automatic all-web search ingestion | Evidence layer supports acquisition mode; no connector implemented | Design provider/vendor or manual-review workflow later |
