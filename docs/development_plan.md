...
- Douyin official API adapter scaffold exists under `backend/app/services/crawling/douyin_adapter.py`; it is mock-only, returns Douyin-style normalized short-video posts/comments, records developer access as obtained and `app_type="web_app"`, and keeps real API mode disabled as `api_pending:permission_not_verified` / `config_error` until Web App OAuth, `item.comment`, token flow, and lawful `item_id` source are verified.
...
- Xiaohongshu official API adapter scaffold exists under `backend/app/services/crawling/xiaohongshu_adapter.py`; it is mock-only, returns Xiaohongshu-style normalized lifestyle/community note posts/comments, records developer access as obtained, and keeps real API mode disabled as `api_pending:permission_not_verified` / `config_error` until note/comment API availability is verified.
