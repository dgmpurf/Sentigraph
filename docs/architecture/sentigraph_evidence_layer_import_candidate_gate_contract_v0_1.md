# Sentigraph Evidence Layer Import Candidate Gate Contract v0.1

## A. Purpose

This contract defines the 8Y-9 governance boundary between the completed 8Y-8 local controlled review-only / review queue candidate smoke and any future controlled Evidence Layer import candidate smoke.

It is docs-only and gate-only. It does not implement a helper, route, API, frontend view, runtime persistence, import candidate creation, Evidence Layer write, production EvidenceItem, production case, production `analysis_run`, Source 11 runtime, actual FinalSummaryReport runtime, report, Sandbox/public event, export/download/public/final-delivery runtime, provider job, collector job, real API call, real LLM call, URL fetch, or scrape.
...
8Y-8 completed only the helper hop from controlled evidence candidate to controlled review queue candidate.

8Y-9 decides that the next candidate boundary may be considered because an existing safe backend helper surface is present:

```text
backend/app/services/controlled_evidence_layer_import_candidate.py
```

This is not Evidence Layer write. It is a local Evidence-Layer-import-candidate-shaped boundary helper.

## C. Accepted Source Object

Future 8Y-10, if separately approved, may only accept:
...
## D. Existing Evidence Layer Import Candidate Helper Surface

Observed existing helper:

```text
service = backend/app/services/controlled_evidence_layer_import_candidate.py
phase = 8W-16
...
evidence_layer_import_candidate_created = true only when called with exact helper approval
production_evidence_item_created = false
evidence_layer_write = false
review_queue_item_created = false
production_review_queue_item_created = false
...
| --- | --- | --- | --- |
| `controlled_review_queue_candidate.py` | backend_helper | review_queue_candidate_source | no_persistence |
...
| `controlled_production_evidence_import_candidate.py` | backend_helper | production_import | no_persistence |
...
| `evidence_import.py` | backend_service | production_import | Evidence_Layer_write_possible if called |
| `evidence_ingestion.py` | backend_service | production_import | Evidence_Layer_write_possible if called |
...
| `test_controlled_evidence_layer_import_candidate.py` | test_only | import_candidate | no_persistence |
...
| `sentigraph_8w_15_evidence_layer_import_gate_decision_v0_1.md` | docs_only | import_candidate | no_persistence |

## F. Future 8Y-10 Output Contract

If future 8Y-10 is separately approved, it may produce only a local controlled Evidence Layer import candidate object.

Minimum expected output constraints:

```text
evidence_layer_import_candidate_created = true only inside controlled backend test path
import_candidate_created = true only inside controlled backend test path
...
## G. Future 8Y-10 Approval Phrase
...
This phrase is inactive in 8Y-9. It is a future placeholder only. It does not authorize implementation, Evidence Layer import candidate creation in 8Y-9, Evidence Layer write, production EvidenceItem creation, production case creation, production `analysis_run` creation, actual Review Queue runtime, production Review Queue item creation, Source 11 runtime, actual FinalSummaryReport runtime, route/API/frontend behavior, delivery runtime, provider/collector execution, real API calls, real LLM calls, URL fetching, or scraping.

## H. Related Phrase Status

The previous direct import phrase from 8Y-5 remains inactive and not selected:
...
Neither phrase may authorize 8Y-10 work.

## I. Required Stop Rules

Future 8Y-10 must stop if any request requires:
...
Evidence Layer import candidate is not Evidence Layer write.

Evidence Layer write, production EvidenceItem, production case, production `analysis_run`, actual analysis execution, production Analysis Result, Source 11 runtime, actual FinalSummaryReport runtime, report generation, export/download/public access, and final delivery each remain separate gates.

## K. Selected Next Boundary Option

Because a safe backend helper surface exists and it is separate from Evidence Layer write and production EvidenceItem creation, 8Y-9 selects:

```text
ready_for_8Y_10_controlled_review_queue_candidate_to_evidence_layer_import_candidate_smoke
```

This is only a planning state. Future 8Y-10 still requires a new task with the exact inactive phrase promoted by the user. No implementation is approved by this contract.
