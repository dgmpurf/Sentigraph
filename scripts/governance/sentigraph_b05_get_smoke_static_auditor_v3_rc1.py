...
AUDITOR_SCHEMA = "sentigraph_b05_get_smoke_runner_static_audit_v3_rc1_result_v0_1"
SELF_TEST_SCHEMA = "sentigraph_b05_get_smoke_static_auditor_v3_rc1_self_test_result_v0_1"
QUALIFICATION_SCHEMA = "sentigraph_b05_get_smoke_auditor_v3_rc1_runner_v3_rc1_current_product_contract_compatibility_qualification_result_v0_1"
VERSION = "0.1"
EXPECTED_RUNNER_BASENAME = ".sentigraph_b05_get_smoke_runner_v3_rc1.py"
...
CHECK_NAMES = (
    ...
    "PROJECTION_FIELDS_MODULE_IDENTITY_EXACT",
...
COMPATIBILITY_CONTRACT = {
    ...
    "receipt_public_constants": {
        ...
        "route_mode": "internal_alpha_read_only_local_exchange_projection_operator",
        "capability_label": "b05_local_exchange_projection_read_only",
        "variable_names": ("SENTIGRAPH_LOCAL_EXCHANGE_RESULTS_DIR", "SENTIGRAPH_PRIVATE_COLLECTOR_EXPORT_ROOT", "SENTIGRAPH_LOCAL_EXCHANGE_ADAPTER_ID"),
    ...
    "approved_environment_names": ("SENTIGRAPH_LOCAL_EXCHANGE_RESULTS_DIR", "SENTIGRAPH_PRIVATE_COLLECTOR_EXPORT_ROOT", "SENTIGRAPH_LOCAL_EXCHANGE_ADAPTER_ID"),
    "product_sources": {
        "b05_service": {
            "path": "backend/app/services/internal_alpha_local_exchange_review_projection.py",
            "blob": "f0c4a8768060a840ea1921aeba47a97f2e41f9e3",
        },
        "b05_route": {
            "path": "backend/app/api/v1/routes/internal_alpha_review_console.py",
            "blob": "8445b4595ea7edc9d9878e99b35ce0554b841c94",
        },
        "b03_projection": {
            "path": "backend/app/services/local_exchange_review_only_projection_bridge.py",
            "blob": "534bdf02e211134b52b2e7714d01a0dd615210b4",
        },
    },
    "projection_module": "app.services.local_exchange_review_only_projection_bridge",
    "projection_fields": ("projection_schema", "projection_version", "projection_mode", "projection_status", "projection_error_code", "source_chain_boundary", "result_file_name", "upstream_schema", "upstream_status", "reader_status", "adapter_status", "provider_result_status", "package_resolution_status", "candidate_count", "staging_candidate_id", "gate_result_id", "analysis_request_id", "provider_result_id", "package_name", "case_id_hint", "case_title_hint", "validation_summary", "coverage_summary", "review_status", "promotion_status", "staging_status", "gate_summary", "warnings", "blockers", "allowed_actions", "blocked_actions", "metadata_only", "review_only", "human_review_required", "no_automatic_trust_upgrade", "candidate_persistence", "persistent_staging_write", "review_decision_write", "evidence_layer_write", "production_evidenceitem_created", "production_case_created", "analysis_run_created", "analysis_result_created", "frontend_action_enabled", "public_output_enabled", "export_delivery_enabled", "path_exposed", "raw_metadata_exposed", "trust_approved", "production_ready", "promotion_completed", "mutable_authority_granted"),
    "target_route": "/api/v1/internal/alpha/review-console/local-exchange-projections/helldivers2-psn-demo",
    "forbidden_projection_modules": ("app.api.v1.endpoints.internal_alpha_review_console", "app.api.v1.routes.internal_alpha_review_console"),
    "forbidden_receipt_fields": ("status", "privacy_issue_stop", "configuration_names", "configuration_value_reads", "canonical_objects", "canonical_serializations", "combined_sha256_computations", "new_salt_created", "new_receipt_created", "receipt_reopens", "warnings"),
}
...
def _receipt_subscript_keys(function):
    keys = []
    for node in ast.walk(function):
        if isinstance(node, ast.Subscript) and isinstance(node.value, ast.Name) and node.value.id == "receipt":
...
def _check_receipt_contract(tree):
    expected_fields = tuple(COMPATIBILITY_CONTRACT["receipt_fields"])
    if _literal_global(tree, "RECEIPT_FIELDS") != expected_fields:
        return False
    expected_false = (
        "raw_values_exposed",
        "per_variable_hashes_created",
    ...
    if _literal_global(tree, "RECEIPT_FALSE_FIELDS") != expected_false:
        return False
    function = _function(tree, "_read_receipt")
    constants = {node.value for node in ast.walk(function) if isinstance(node, ast.Constant) and isinstance(node.value, (str, int, bool))}
    public = COMPATIBILITY_CONTRACT["receipt_public_constants"]
    required_literals = {
        public["schema"], public["version"], public["binding_scope"], public["service_blob"],
        public["registry_schema"], public["sample_handle"], public["result_file_name"],
        public["route_mode"], public["capability_label"], public["canonicalization_label"],
        public["configuration_source"], public["environment_read_count"], public["binding_status"],
    }
    if not required_literals.issubset(constants):
        return False
    direct_keys = set(_receipt_subscript_keys(function))
    required_keys = set(expected_fields) - set(expected_false)
    if not required_keys.issubset(direct_keys):
        return False
    if direct_keys.intersection(COMPATIBILITY_CONTRACT["forbidden_receipt_fields"]):
        return False
    digest_calls = _calls(function, "_validate_sha256")
    digest_keys = []
    for call in digest_calls:
        if len(call.args) == 1 and isinstance(call.args[0], ast.Subscript):
            digest_keys.append(_subscript_key(call.args[0]))
    if digest_keys != ["salt_hex", "combined_binding_sha256"]:
        return False
    return True


def _check_projection_module(tree):
    if _direct_constant(tree, "PROJECTION_FIELDS_MODULE") != COMPATIBILITY_CONTRACT["projection_module"]:
        return False
    if _literal_global(tree, "EXPECTED_PROJECTION_FIELDS") != tuple(COMPATIBILITY_CONTRACT["projection_fields"]):
        return False
    function = _function(tree, "_validate_response")
    module_calls = _calls(function, "sys.modules.get")
    if len(module_calls) != 1 or len(module_calls[0].args) != 1 or module_calls[0].keywords:
        return False
    argument = module_calls[0].args[0]
    if not isinstance(argument, ast.Constant) or argument.value != COMPATIBILITY_CONTRACT["projection_module"]:
        return False
    constants = {node.value for node in ast.walk(function) if isinstance(node, ast.Constant) and isinstance(node.value, str)}
    if constants.intersection(COMPATIBILITY_CONTRACT["forbidden_projection_modules"]):
        return False
    getters = _calls(function, "getattr")
    projection_getters = [call for call in getters if len(call.args) == 2 and isinstance(call.args[0], ast.Name) and call.args[0].id == "projection_module" and isinstance(call.args[1], ast.Constant) and call.args[1].value == "PROJECTION_FIELDS"]
    if len(projection_getters) != 1:
        return False
    exact_compares = []
    for node in ast.walk(function):
        if isinstance(node, ast.Compare) and len(node.ops) == 1 and isinstance(node.ops[0], ast.NotEq) and isinstance(node.left, ast.Name) and node.left.id == "projection_fields" and len(node.comparators) == 1 and isinstance(node.comparators[0], ast.Name) and node.comparators[0].id == "EXPECTED_PROJECTION_FIELDS":
            exact_compares.append(node)
    return len(exact_compares) == 1
...
def _inventory(source):
    ...
    receipt_contract = int(_literal_global(tree, "RECEIPT_FIELDS") == tuple(COMPATIBILITY_CONTRACT["receipt_fields"]))
    response_function = _function(tree, "_validate_response")
    projection_module = sum(
        1
        for call in _calls(response_function, "sys.modules.get")
        if len(call.args) == 1 and isinstance(call.args[0], ast.Constant) and call.args[0].value == COMPATIBILITY_CONTRACT["projection_module"]
    )
    return {
...
EXPECTED_DELTAS = {
    ...
    "receipt_field_contract_substitution": {"receipt_contract": -1},
    "projection_fields_module_endpoints_substitution": {"projection_module": -1},
}
...
def _mutate_fixture(name):
    ...
    if name == "receipt_field_contract_substitution":
        receipt_tuple = _global_assign(tree, "RECEIPT_FIELDS").value
        matches = [node for node in ast.walk(receipt_tuple) if isinstance(node, ast.Constant) and node.value == "variable_names"]
        if len(matches) != 1:
            raise ValueError("receipt field anchor")
        return _replace_node(source, matches[0], '"configuration_names"')
    if name == "projection_fields_module_endpoints_substitution":
        function = _function(tree, "_validate_response")
        calls = _calls(function, "sys.modules.get")
        matches = [call for call in calls if len(call.args) == 1 and isinstance(call.args[0], ast.Constant) and call.args[0].value == COMPATIBILITY_CONTRACT["projection_module"]]
        if len(matches) != 1:
            raise ValueError("projection module anchor")
        return _replace_node(source, matches[0].args[0], '"app.api.v1.endpoints.internal_alpha_review_console"')
...
def _compatibility_contract_result():
    receipt_fields = COMPATIBILITY_CONTRACT["receipt_fields"]
    projection_fields = COMPATIBILITY_CONTRACT["projection_fields"]
    public = COMPATIBILITY_CONTRACT["receipt_public_constants"]
    exact = (
        COMPATIBILITY_CONTRACT["capture_runner"] == {
            "basename": ".sentigraph_cib_capture_risk_prompt_3_v1.py",
            "bytes": 4751,
            "sha256": "dd1c63882a90b95956555ffe2d9ae6a866f788135cba70cf58743996ef54a0fc",
        }
        and len(receipt_fields) == 23
        and tuple(receipt_fields) == (
            "schema", "version", "binding_scope", "service_blob", "registry_schema", "sample_handle",
            "result_file_name", "route_mode", "capability_label", "variable_names", "salt_hex",
            "combined_binding_sha256", "canonicalization_label", "configuration_source",
            "environment_read_count", "binding_status", "raw_values_exposed",
            "per_variable_hashes_created", "path_operations_performed", "application_imported",
            "artifact_accessed", "endpoint_called", "runtime_authorized",
        )
        and tuple(public["variable_names"]) == tuple(COMPATIBILITY_CONTRACT["approved_environment_names"])
        and public["environment_read_count"] == 3
        and public["runtime_authorized"] is False
        and COMPATIBILITY_CONTRACT["product_sources"]["b05_service"]["blob"] == "f0c4a8768060a840ea1921aeba47a97f2e41f9e3"
        and COMPATIBILITY_CONTRACT["product_sources"]["b05_route"]["blob"] == "8445b4595ea7edc9d9878e99b35ce0554b841c94"
        and COMPATIBILITY_CONTRACT["product_sources"]["b03_projection"]["blob"] == "534bdf02e211134b52b2e7714d01a0dd615210b4"
        and COMPATIBILITY_CONTRACT["projection_module"] == "app.services.local_exchange_review_only_projection_bridge"
        and len(projection_fields) == 52
        and len(set(projection_fields)) == 52
        and COMPATIBILITY_CONTRACT["target_route"] == "/api/v1/internal/alpha/review-console/local-exchange-projections/helldivers2-psn-demo"
    )
    return {
        "status": "pass" if exact else "fail",
        "compatibility_contract_exact": exact,
        "receipt_contract_field_count": len(receipt_fields),
        "projection_field_count": len(projection_fields),
        "projection_fields_unique": len(set(projection_fields)) == len(projection_fields),
    }


def _self_test():
    preflight = _asyncio_preflight()
    compatibility = _compatibility_contract_result()
    ...
    order_exact = tuple(CHECK_NAMES) == CHECK_NAMES and len(set(CHECK_NAMES)) == 32
    ...
    return {
        ...
        "receipt_access": 0,
        "product_access": 0,
        "compatibility_contract_exact": compatibility["compatibility_contract_exact"],
        "receipt_contract_field_count": compatibility["receipt_contract_field_count"],
        "projection_field_count": compatibility["projection_field_count"],
        "projection_fields_unique": compatibility["projection_fields_unique"],
...
def _qualify_runner(path_text):
    ...
    return {
        ...
        "status": "pass" if passed else "fail",
        "self_test": self_test,
        "compatibility_contract": self_test["compatibility_contract"],
