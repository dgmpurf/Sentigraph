...
- Universal evidence layer: case-level evidence can now normalize official API output, public-parser fixtures, manual/user-provided article/video/comment payloads, CSV/Excel uploads, and future vendor/search-discovery data into `EvidenceItem` records without calling external services. Evidence now carries trust/provenance, verification, attestation, and duplicate-collapse metadata.
- Evidence scale scaffold: CSV/Excel commits and manual evidence attaches record lightweight local ingestion jobs, evidence summaries, and coverage notes. These summaries describe imported/available case evidence only; they do not claim full-platform or all-web coverage.
...
- Search Discovery status: `GET /api/v1/search-discovery/status`, `GET /api/v1/search-discovery/providers`, and the mock candidates endpoint expose planning/static URL-title-snippet discovery metadata only. `mock_static`, `rss_mock`, and `gdelt_mock` are local fixtures; no real RSS, GDELT, search provider, URL fetch, scraping, or automatic attach is enabled.
...
| Evidence ingestion | Working as a source-neutral normalization layer with CSV/Excel upload import, manual URL entry, trust/provenance labels, and duplicate collapse. | Already attached public/authorized/manual/user-uploaded evidence can be represented as `EvidenceItem` records with source/trust metadata. | Evidence attach/import is offline normalization, does not persist raw uploaded files by default, never fetches external sources, and does not treat screenshots/user text as automatically verified. Search Discovery is static/mock candidate metadata only. | Real search/RSS/provider connectors and production review workflows. |
| Evidence scale scaffold | Lightweight local job and coverage summaries are implemented for imports/manual attach. | Counts, distributions, review status, trust status, latest local jobs, and coverage ranges for imported/available evidence. | No background worker, no real crawler, no URL fetching, no full-platform coverage claim. | Chunked/resumable imports, durable workers/storage, and vendor/official large-scale ingestion. |
