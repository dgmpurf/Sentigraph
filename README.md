...
| Evidence ingestion | Working as a source-neutral normalization layer with CSV/Excel upload import and a downloadable CSV template. | Already attached public/authorized/manual/user-uploaded evidence can be represented as `EvidenceItem` records. | Evidence attach/import is offline normalization, does not persist raw uploaded files by default, and never fetches external sources. Search Discovery is static/mock candidate metadata only. | Real search/RSS/provider connectors and production review workflows. |
